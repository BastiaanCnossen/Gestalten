...
\begin{document}
	...
	\begin{definition}
		A map $\Spec(B) \to \Spec(A)$ in $\Aff_{\S}$ is called \emph{countably presented} if $B$ is $\aleph_1$-compact as an object of $\CAlg_A(\Sp)$. Equivalently, the subcategory of $\CAlg(\Sp)_{A/}$ spanned by the countably presented maps is the smallest subcategory containing the polynomial algebra on one generator over $A$ which is closed under countable colimits.
	\end{definition}

	\begin{observation}
		The basic closure properties of countably presented maps follow from the compact algebra lemmas in \Cref{sec:Compact_Algebras}, applied with $\kappa = \aleph_1$ and $C = \Sp$. We will spell out the particular consequences needed later in \Cref{lem:Affine_Countably_Presented_Calculus}.
	\end{observation}
	...
	\begin{theorem}
		...
		\[
			\Span(\Aff_{\S}, \mathrm{all}, \textup{countably presented maps}) \to 1\Pr, \qquad \Spec(A) \mapsto n\Pr_A.
		\]
	\end{theorem}

	On a span in $\Aff_{\S}$, equivalently a cospan in $\CAlg(\Sp)$ of the form
	\[
	...
	We now construct the six-functor formalism for every $n \geq 0$. For $n = 0$, we have $\Vv = \Sp$, and we have the functor $\Aff_{\S}\catop \to \CAlg(1\Pr)$, $\Spec(A) \mapsto 0\Pr_A = \Mod_A(\Sp)$. We then established all the relevant properties to deduce that the assignment $\Spec(A) \mapsto 0\Pr_A = \Mod_A(\Sp)$ extends to a six-functor formalism
	\[
		\Span(\Aff_{\S}) \to \PrL, \qquad \Spec(A) \mapsto 0\Pr_A
	\]
	where all maps in $\Aff_{\S}$ are treated as ``proper'' maps (i.e.\ we always pick the right adjoint). Moreover, if we restrict the right-way maps to the countably presented maps, then the functors $f_*$ preserve $\aleph_1$-compact objects, so this functor restricts to
	\[
		\Span(\Aff_{\S}, \mathrm{all}, \textup{countably presented}) \to 1\Pr.
	\]
	...
	\[
		n\Pr_{(-)}\colon \Aff_{\S}\catop \xrightarrow{(n-1)\Pr_{(-)}} \CAlg(n\Pr) \xrightarrow{\Mod_{(-)}(n\Pr)} \CAlg((n+1)\Pr) \to \CAlg(1\Pr).
	\]
	Then we may apply properties (a)--(f) to get the desired six-functor formalism
	\[
		\Span(\Aff_{\S}, \mathrm{all}, \textup{countably presented}) \to 1\Pr, \qquad \Spec(A) \mapsto n\Pr_A
	\]
	...
	\begin{theorem}
		The functor $n\Pr_{(-)}^!\colon \Aff_{\S} \to 1\Pr$ (defined on maps by the maps $f^!$) is a hypersheaf with respect to the fpqc topology for each $n \geq 0$. (Note that for $n \geq 1$, the maps $f^!$ agree with $f^*$ by ambidexterity.)
	\end{theorem}
	...
	\]
	giving rise to a functor $\Shv_{\fpqc}(\Aff_{\S})\catop \to \StRing$.
	...
	\begin{proof}
		...
		$S'_{\bullet}$ for the Cech nerve of $S' \to S$. Since
	...
	\begin{lemma}\label[lemma]{lem:fpqc_Inclusion_Aleph1_Filtered_Colimits}
		The inclusion
		\[
	...
	\begin{proof}
		...
		for fpqc Cech nerves. Each such Cech nerve is indexed by the countable
		category $\simp\catop$, so the corresponding descent maps are maps
		between $\aleph_1$-compact presheaves. Hence this localization is
		generated by maps between $\aleph_1$-compact objects, and the fully
		faithful right adjoint preserves $\aleph_1$-filtered colimits. The
		argument over an affine base is identical.
	\end{proof}

	\begin{lemma}\label[lemma]{lem:Affines_Aleph1_Compact_fpqc}
		Every affine object $\Spec(A) \in \Shv_{\fpqc}(\Aff_{\S})$ is
		$\aleph_1$-compact. More generally, for every affine
		$X = \Spec(A)$, every affine object $\Spec(B) \to X$ is
		$\aleph_1$-compact in the slice
		$\Shv_{\fpqc}(\Aff_{\S})_{/X}$. In particular,
		$\Shv_{\fpqc}(\Aff_{\S})_{/X}$ is $\aleph_1$-compactly generated by
		affines, and every $\aleph_1$-compact object in this slice is a
		countable colimit of affine objects.
	\end{lemma}
	\begin{proof}
		Representables are compact in presheaves. By
		\Cref{lem:fpqc_Inclusion_Aleph1_Filtered_Colimits}, fpqc
		sheafification sends $\aleph_1$-compact presheaves to
		$\aleph_1$-compact sheaves. This proves the first assertion, and the
		same argument on the fpqc site of affines over $X$ proves the slice
		assertion.

		The sheaf category over $X$ is generated under colimits by affine
		objects over $X$. Thus it is $\aleph_1$-compactly generated by affines.
		In any $\aleph_1$-compactly generated category, the
		$\aleph_1$-compact objects are retracts of countable colimits of
		$\aleph_1$-compact generators; absorbing the retract into the indexing
		category gives the stated countable affine presentation.
	\end{proof}

	\begin{lemma}\label[lemma]{lem:Aleph1_Compact_fpqc_Finite_Limits}
		The $\aleph_1$-compact objects of $\Shv_{\fpqc}(\Aff_{\S})$ are closed
		under finite limits.
	\end{lemma}
	\begin{proof}
		Let
		\[
			L\colon \PSh(\Aff_{\S}) \rightleftarrows \Shv_{\fpqc}(\Aff_{\S}) : \iota
		\]
		be sheafification and the inclusion. By
		\Cref{lem:fpqc_Inclusion_Aleph1_Filtered_Colimits}, the inclusion
		$\iota$ preserves $\aleph_1$-filtered colimits. The functor $L$
		preserves finite limits, and
		\Cref{prop:Kappa_Compact_Presheaves_Finite_Limits} shows that
		$\aleph_1$-compact presheaves on $\Aff_{\S}$ are closed under finite
		limits. Therefore \Cref{lem:Compact_Finite_Limits_Localization},
		applied with $\kappa=\aleph_1$, gives the claim.
	\end{proof}
	...
	% 		Z \simeq \colim_{i \in I} \Spec(B_i)
	% 	\]
	% 	be a countable colimit in $\Shv_{\fpqc}(\Aff_{\S})_{/\Spec(A)}$, where $I$ is a countable category and each $A \to B_i$ is countably presented. Then for any two objects $i,j \in I$, the intersection
	% 	\[
	...
	We now collect the compactness facts for animae, presheaves, and left
	exact localizations which are used in the main text.

	\begin{proposition}\label[proposition]{prop:Kappa_Compact_Anima_Kappa_Small_Simplicial}
		Let $\kappa$ be an uncountable regular cardinal. An anima is
		$\kappa$-compact if and only if it can be represented by a simplicial
		set with fewer than $\kappa$ simplices.
	\end{proposition}
	\begin{proof}
		First suppose that $X$ is represented by a simplicial set $K$ with
		fewer than $\kappa$ simplices. Then $K$ is the colimit of its finite
		simplicial subsets, indexed by a $\kappa$-small filtered category.
		Finite simplicial sets are compact in $\An$, and $\kappa$-small
		colimits of $\kappa$-compact objects are again $\kappa$-compact,
		because $\kappa$-filtered colimits in $\An$ commute with
		$\kappa$-small limits. Thus $X$ is $\kappa$-compact.

		Conversely, let $X$ be $\kappa$-compact, and choose a simplicial set
		$K$ representing it. Let $I$ be the poset of simplicial subsets of $K$
		with fewer than $\kappa$ simplices. This poset is $\kappa$-filtered,
		since $\kappa$ is regular, and
		\[
			K \simeq \colim_{L \in I} L
		\]
		in simplicial sets, hence also in $\An$. By compactness, the identity
		of $X$ factors through one of the maps $\Piinfty{L} \to X$. Thus $X$ is
		a retract of an anima represented by a simplicial set with fewer than
		$\kappa$ simplices. It follows, after applying
		$\operatorname{Ex}^{\infty}$ if desired, that $\pi_0(X)$ and all
		higher homotopy groups of $X$ have cardinality $<\kappa$.

		We now use the usual cellular approximation argument. Start with one
		vertex for each connected component of $X$. Inductively, after the
		$n$-skeleton has been constructed, attach fewer than $\kappa$ many
		cells to hit the remaining elements of $\pi_n(X)$ and fewer than
		$\kappa$ many further cells to kill the kernel on $\pi_n$. Since
		$\kappa$ is uncountable and regular, the resulting simplicial set $Y$
		has fewer than $\kappa$ simplices. The map $\Piinfty{Y} \to X$
		constructed in this way induces an isomorphism on $\pi_0$ and on all
		higher homotopy groups, hence is an equivalence.
	\end{proof}

	\begin{proposition}\label[proposition]{prop:Kappa_Compact_Anima_Finite_Limits}
		Let $\kappa$ be an uncountable regular cardinal. The $\kappa$-compact
		objects of $\An$ are closed under finite limits.
	\end{proposition}
	\begin{proof}
		By \Cref{prop:Kappa_Compact_Anima_Kappa_Small_Simplicial}, it is enough
		to prove that finite limits of animae represented by simplicial sets
		with fewer than $\kappa$ simplices can again be represented by such
		simplicial sets. The terminal anima is represented by $\Delta^0$, so
		it remains to consider pullbacks. Let
		\[
			X \to Z \leftarrow Y
		\]
		be a diagram of such animae, represented by simplicial sets $K,L,M$.
		Replace $M$ by the Kan complex
		$M^{\fib}:=\operatorname{Ex}^{\infty}(M)$. This still has fewer than
		$\kappa$ simplices: for each $n$, the simplicial set
		$\operatorname{sd}\Delta^n$ is finite, so
		\[
			(\operatorname{Ex} N)_n
			=
			\Hom_{\sSet}(\operatorname{sd}\Delta^n,N)
		\]
		has cardinality $<\kappa$ whenever $N$ has fewer than $\kappa$
		simplices, and the sequential colimit defining
		$\operatorname{Ex}^{\infty}$ is still $\kappa$-small because $\kappa$
		is uncountable.

		Since $M^{\fib}$ is fibrant, the two maps to $Z$ may be represented by
		actual maps of simplicial sets $K \to M^{\fib}$ and $L \to M^{\fib}$,
		and the homotopy pullback is represented by
		\[
			K
			\times_{M^{\fib}}
			(M^{\fib})^{\Delta^1}
			...
			L.
		\]
		The simplicial set $(M^{\fib})^{\Delta^1}$ has fewer than $\kappa$
		simplices because
		\[
			((M^{\fib})^{\Delta^1})_n
			=
			\Hom_{\sSet}(\Delta^n \times \Delta^1,M^{\fib}),
		\]
		and $\Delta^n \times \Delta^1$ is finite. Finite strict limits of
		simplicial sets with fewer than $\kappa$ simplices again have fewer
		than $\kappa$ simplices. Hence the pullback is represented by a
		$\kappa$-small simplicial set.
	\end{proof}

	\begin{definition}\label[definition]{def:Necklaces_BS}
		A \emph{necklace} is a bipointed simplicial set of the form
		\[
			N = \Delta^{n_1} \vee \cdots \vee \Delta^{n_r},
		\]
		where we glue the final vertex of each bead to the initial vertex of
		the next one. We write $\mathrm{Nec}$ for the category of necklaces
		and maps of bipointed simplicial sets.
	\end{definition}

	Let
	\[
		\mathrm{L}_{\mathrm{Seg}}\colon \PSh(\simp) \to \PSh(\simp)
	\]
	denote Segalification, i.e. the left adjoint to the inclusion of Segal
	animae into all simplicial animae. We will use the following form of
	the Barkan--Steinebrunner necklace formula.

	\begin{proposition}[Barkan--Steinebrunner]\label[proposition]{prop:BS_Mapping_Anima_Formula}
		Let $X$ be a simplicial anima. Then there is a canonical equivalence
		\[
			\mathrm{L}_{\mathrm{Seg}}(X)_1
			\simeq
			\colim_{N\in \mathrm{Nec}\catop}\Map_{\PSh(\simp)}(N,X),
		\]
		naturally as an object over $X_0\times X_0$. Consequently, if
		$\mathrm{Cat}(X)$ denotes the $\infty$-category presented by the
		Segalification of $X$, followed by completion, then for two points
		$x,y\colon \ast \to X_0$ there is a canonical equivalence
		\[
			\Map_{\mathrm{Cat}(X)}(x,y)
			\simeq
			\abs{\mathrm{Nec}_{/(X,x,y)}}.
		\]
		Here $\mathrm{Nec}_{/(X,x,y)}$ is the $\infty$-category of necklaces
		$N$ equipped with a map $N\to X$ whose initial vertex is $x$ and whose
		final vertex is $y$.
	\end{proposition}
	\begin{proof}
		The first assertion is the $1$-simplices case of the
		Barkan--Steinebrunner formula for Segalification by necklaces. The
		second assertion is the fiber over $(x,y)$: the right fibration
		$\mathrm{Nec}_{/X}\to\mathrm{Nec}$ corresponds to the functor
		$N\mapsto\Map_{\PSh(\simp)}(N,X)$, so its groupoid completion is the
		colimit of this functor. Since colimits in animae are universal, we
		may take the fiber over $(x,y)\in X_0\times X_0$ after forming this
		colimit. Completion of a Segal anima does not change its mapping
		animae between fixed objects, so the result computes the mapping
		anima in the $\infty$-category presented by $X$.
	\end{proof}
	...
	\begin{lemma}\label[lemma]{lem:Finite_Limits_Kappa_Coproducts_Representables}
		...
		small $\infty$-category with finite limits. Then finite limits of
	...
	For a small $\infty$-category $C$ and a simplicial object
	$Y_{\bullet}$ of $\PSh(C)$, we write $R_Y$ for the internal relative
	Rezk nerve of the category of simplices of $Y_{\bullet}$, with all
	morphisms inverted. Concretely, $R_Y$ is the simplicial object of
	$\PSh(C)$ obtained by the following objectwise formula:
	\[
		(R_Y)_m(c)
		=
		\abs{\Fun([m],C_{Y(c)})},
		\qquad
		C_{Y(c)}:=\int_{\simp\catop} Y_{\bullet}(c).
	\]
	Equivalently, $(R_Y)_m$ is the geometric realization of a simplicial
	object whose $q$-simplices are
	\[
		\coprod_{\alpha\colon [q]\times [m]\to \simp\catop}
		\lim_{(a,b)\in [q]\times [m]} Y_{\alpha(a,b)}.
	\]
	The coproduct is indexed by a countable set, and the limit is finite.
	When $N$ is a finite simplicial set, we write
	$\Map_{\PSh(\simp)}(N,R_Y)$ for the corresponding mapping object in
	$\PSh(C)$, computed objectwise in $C$.

	\begin{proposition}\label[proposition]{prop:Relation_Object_Kappa_Small_Colimit}
		Let $\kappa$ be an uncountable regular cardinal, let $C$ be a small
		$\infty$-category with finite limits, and let $Y_{\bullet}$ be a
	...
	\end{proposition}
	\begin{proof}
		By the displayed formula for $(R_Y)_m$, each simplicial degree in the
		geometric realization computing $(R_Y)_m$ is a countable coproduct of
		finite limits of the $Y_n$. By
		\Cref{lem:Finite_Limits_Kappa_Coproducts_Representables}, each such
		finite limit is a $\kappa$-small coproduct of representables. Hence
		each $(R_Y)_m$ is a $\kappa$-small colimit of $\kappa$-small
		coproducts of representables, and therefore a $\kappa$-small colimit
		of representables.

		The same argument applies after mapping out of a finite necklace. Let
		$N$ be a necklace. Since $N$ is finite,
		\[
			\Map_{\PSh(\simp)}(N,R_Y)
		\]
		is a finite limit of the values $(R_Y)_m$. Each $(R_Y)_m$ is itself a
		geometric realization as above. Since $\PSh(C)$ is an $\infty$-topos,
		colimits are universal, so this finite limit can be computed as a
		$\kappa$-small colimit of finite limits of the displayed simplicial
		degrees. These finite limits are again $\kappa$-small coproducts of
		representables by
		\Cref{lem:Finite_Limits_Kappa_Coproducts_Representables}. Thus
		$\Map_{\PSh(\simp)}(N,R_Y)$ is a $\kappa$-small colimit of
		representables.

		We need the version with endpoints in $Y_0$. Since
		\[
			Y_0\times_{(R_Y)_0}\Map_{\PSh(\simp)}(N,R_Y)\times_{(R_Y)_0}Y_0
		\]
		is again a finite limit of $Y_0$ and finitely many values $(R_Y)_m$,
		the same universal-colimit argument shows that it is a
		$\kappa$-small colimit of representables.

		Now apply \Cref{prop:BS_Mapping_Anima_Formula} to the simplicial anima
		$R_Y$, objectwise in $C$. The relative Rezk nerve, after
		Segalification and completion, presents the localization; in our case,
		with all morphisms inverted, this localization is the groupoid
		completion
		\[
			\abs{C_{Y(c)}}\simeq \abs{Y_{\bullet}(c)}
		\]
		for each $c\in C$. Hence, in $\PSh(C)$,
		\[
			Y_0\times_{\abs{Y_{\bullet}}}Y_0
			\simeq
			\colim_{N\in \mathrm{Nec}\catop}
			\left(
			Y_0\times_{(R_Y)_0}
			\Map_{\PSh(\simp)}(N,R_Y)
			\times_{(R_Y)_0}Y_0
			\right).
		\]
		The category $\mathrm{Nec}$ is countable. Since $\kappa$ is
		uncountable regular, this final colimit is still a $\kappa$-small
	...
	\begin{proposition}\label[proposition]{prop:Kappa_Compact_Presheaves_Finite_Limits}
		...
		$\infty$-category with finite limits. Then the $\kappa$-compact objects
	...
	\begin{proof}
		...
		colimits, then $X$ can be written as the colimit of the countable
		diagram $\bbB \N \to \PSh(C)$ encoding the idempotent on $Y$.
		...
		representables lift along effective epimorphisms after evaluation.
		Thus the two maps $X,Z\to\abs{Y_{\bullet}}$ admit lifts to $Y_0$.
		...
		lifts. By \Cref{prop:Relation_Object_Kappa_Small_Colimit}, the object
		$R$ is a $\kappa$-small colimit $\colim_{\ell} R_{\ell}$ of
		representables. Since colimits in a presheaf topos are universal,
		\[
			(X \times Z) \times_{Y_0 \times Y_0} R
			\simeq
			\colim_{\ell}
			\left((X \times Z) \times_{Y_0 \times Y_0} R_{\ell}\right).
		\]
		Each term on the right is a finite limit of $\kappa$-small coproducts
		of representables, hence is itself a $\kappa$-small coproduct of
		representables by
		\Cref{lem:Finite_Limits_Kappa_Coproducts_Representables}. Therefore
