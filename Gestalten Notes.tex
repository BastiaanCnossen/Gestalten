...
\begin{document}
	...
	\begin{definition}
		...
		to be the sequence
		\[
			\St(A) := (\Omega^{\infty}(A), \D(A), 1\Pr_A, 2\Pr_A, \ldots),
		\]
	...
	\begin{example}[Analytic rings and analytic stacks]
		...
		\[
			\AnRing \to \StRing, \qquad A \mapsto (\D(A), 1\Pr_{\D(A)}, 2\Pr_{\D(A)}, \dots).
		\]
	...
	\end{proposition}
	\begin{proof}
		In short, we apply \citep[Theorem~B]{CLL_Span2} (recalled in \Cref{thm:Span_2_Universal}) to obtain a once-categorified six-functor formalism $Y \mapsto \Oo(Y)_2$, and then pass to endomorphisms to get the desired functor $\Span(C) \to \Oo(X)_2$. We spell out the details.
	...
	\begin{question}
		Can the six-functor formalism on $\Gest$ constructed in \Cref{prop:6FF_Truncated_Gest} be improved to a functor
		\[
			\Oo(-)_1\colon \Span(\Gest, \Gest^{!}) \to 1\Pr,
		\]
		defined on the span category of \emph{all} $!$-able maps of Gestalten (\Cref{def:Quasi_Shrieksable}), and not only the truncated ones? Here $\Gest^{!} \subseteq \Gest$ denotes the wide subcategory of $!$-able maps.
	\end{question}
	...
	\emph{Essentially all material in this chapter is taken from \cite{Aoki_PhD_Thesis}.}
	...
	\begin{itemize}
		...
		\item The Gestalt $[\Spec(\Z)]_{\SH}$ is the free Gestalt $S$ equipped with a ring object $R \in \CRing(\Gest_{/S})$ which is 1-affine, 0-suave, homologically contractible and `sutured' (roughly: $0 \hookrightarrow R$ and $R^{\times} \hookrightarrow R$ form a recollement).
	\end{itemize}
	...
	\begin{definition}
		...
		\begin{enumerate}
			\item Smooth maps admit left adjoints $f_\natural$, and these left adjoints satisfy smooth base change and the projection formula.
			...
			\item If $s\colon X\to\bbA^1_X$ is the zero section, then $\pi_\natural s_*$ is an equivalence.
		\end{enumerate}
	...
	\begin{theorem}[Ayoub--Cisinski--D\'eglise {\citep{Ayoub2007SixOperations,CisinskiDeglise2019Triangulated}}]
		...
		\[
			g_\natural p'_*\longrightarrow p_*g'_\natural
		\]
		is an equivalence.
	...
	\]
	is motivic if smooth pullback admits left adjoints satisfying smooth base change, closed--open decompositions give recollements, the projection $\bbA^1_X\to X$ is homotopy invariant, and the Tate operator $\pi_\natural s_*$ is invertible. Morphisms between motivic functors are symmetric monoidal natural transformations which are compatible with the left adjoints for smooth maps.
	...
	\begin{proof}
		...
		The only point worth spelling out is the projection formula. Let $f\colon Y\to X$ be smooth. The graph-square argument in the proof of \Cref{lem:Motivic_2_Functor_Extends_To_Spans}, applied now to the smooth left adjoint $f_\natural$, gives an equivalence in $\Mot^{(2)}(S)$
		\[
			f_\natural(M\otimes f^*N)
			\simeq
			f_\natural(M)\otimes N
