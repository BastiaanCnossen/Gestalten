...
\begin{document}
	...
	\begin{proposition}\label[proposition]{prop:Two_Sided_Bar_Relation_Object}
		Let $C$ be a small category, let $I$ be an ordinary small
		...
		\]
		defined naturally in $F$, with a natural equivalence
		\[
			\abs{B^{i,j}_{\bullet}(F)}
			\simeq
			F(i)\times_{\colim_I F}F(j).
		\]
		Moreover, each $B^{i,j}_q(F)$ is a coproduct, indexed by a set of
		finite necklace diagrams in $I$ from $i$ to $j$, of objects of the form
		\[
			\lim_{K} F,
		\]
		where $K\to I$ is the finite diagram attached to the necklace datum,
		and $\lim_K F$ denotes the limit of the composite
		$K\to I\xrightarrow{F}\PSh(C)$. In particular, if $I$ is countable,
		then each $B^{i,j}_q(F)$ is a countable coproduct of finite limits of
		values of $F$.
	\end{proposition}
	\begin{proof}
		We may alternatively interpret $F$ as a functor
		$C\catop \to \Fun(I,\An)$. Similarly, we will interpret the
		simplicial objects $B^{i,j}_{\bullet}$ as functors
		$C\catop \to \Fun(\simp\catop,\An)$. Since the construction we give
		is natural in $F$, and limits and colimits in $\PSh(C)$ are computed
		objectwise, we may assume that $F\colon I\to\An$.

		Let
		\[
			p\colon D:=\int_I F\longrightarrow I
		\]
		be the left fibration classifying $F$, given by the covariant Grothendieck
		construction. The fiber $D_i$ is the anima $F(i)$, and a
		morphism of $D$ over $\alpha\colon i\to i'$ in $I$ amounts to a point
		$x\in F(i)$ together with a path $F(\alpha)(x)\simeq x'$ in $F(i')$.
		The colimit of $F$ is the groupoid completion of its category
		of elements,
		\[
			\colim_I F\;\simeq\;\abs{D},
		\]
		where $\abs{D}$ denotes the geometric realization of the nerve of
		$D$. Under this equivalence
		the canonical map $F(i)\to\colim_I F$ is the composite
		$D_i\hookrightarrow D\to\abs{D}$. We must compute the pullback $D_i \times_{\abs{D}} D_j$.

		We now use the necklace formula for mapping spaces in $\abs{D}$. Let
		$\mathsf{Nec}$ denote the ordinary category of necklaces: its objects
		are bipointed simplicial sets of the form
		\[
			N=\Delta^{n_1}\vee\cdots\vee\Delta^{n_r},
		\]
		and its morphisms are maps of bipointed simplicial sets. If
		$X\colon\simp\catop\to\An$ is a simplicial anima, then a map
		$N\to X$ has two endpoint evaluations in $X_0$. For maps
		$A\to X_0$ and $B\to X_0$ of animae, we write
		$\mathsf{Nec}_{/(X,A,B)}$ for the Grothendieck construction of the
		functor
		\[
			\mathsf{Nec}\catop\longrightarrow\An,\qquad
			N\longmapsto
			A\times_{X_0}
			\Map_{\PSh(\simp)}(N,X)
			\times_{X_0}B .
		\]
		For points $a,b\in X_0$ this specializes to a category
		$\mathsf{Nec}_{/(X,a,b)}$ by taking $A=\{a\}$ and $B=\{b\}$.

		For a category $E$, let
		$\mathsf R(E)\colon \simp\catop \to \An$ denote the Rezk
		classification diagram of the relative category $(E,E)$, so that
		\[
			\mathsf R(E)_n
			=
			\abs{\Fun([n],E)} .
		\]
		Its associated complete Segal space presents the localization
		$E[E^{-1}]=\abs{E}$. If $a,b\in E$, the necklace formula of
		\citet[Theorem~A and the mapping-space formula of
		Section~2]{Barkan_Steinebrunner_Segalification}
		applied to $\mathsf R(E)$ gives
		\[
			\abs{\mathsf{Nec}_{/(\mathsf R(E),a,b)}}
			\simeq
			\Map_{\abs{E}}(a,b),
		\]
		where the objects $a,b\in E$ are viewed as points of
		$\mathsf R(E)_0=\abs{E}$. Letting the endpoints vary in arbitrary
		animae $A,B$ equipped with maps to $\abs E$, and using universality of
		colimits in $\An$, gives a natural equivalence
		\[
			\abs{\mathsf{Nec}_{/(\mathsf R(E),A,B)}}
			\simeq
			A\times_{\abs{E}}B .
		...
		Apply this with $E=D$, $A=D_i\simeq F(i)$, and
		$B=D_j\simeq F(j)$. Define $B^{i,j}_{\bullet}(F)$, after the present
		objectwise reduction, to be the simplicial anima underlying
		$\mathsf{Nec}_{/(\mathsf R(D),D_i,D_j)}$, expanded by writing each
		space $\abs{\Fun([n],D)}$ as the geometric realization of the nerve of
		$\Fun([n],D)$. Then
		\[
			\abs{B^{i,j}_{\bullet}(F)}
			\simeq
			F(i)\times_{\colim_I F}F(j).
		\]
		Doing the same construction objectwise gives the desired simplicial
		object in presheaves.

		It remains to identify the degrees. Unwinding the Grothendieck
		construction above, a $q$-simplex consists of a chain of necklaces
		\[
			N_0\to\cdots\to N_q
		\]
		in $\mathsf{Nec}$ and a $q$-simplex of
		\[
			A\times_{\mathsf R(E)_0}
			\Map_{\PSh(\simp)}(N_q,\mathsf R(E))
			\times_{\mathsf R(E)_0}B .
		\]
		Since $N_q$ is a finite gluing of simplices, the middle mapping anima
		is a finite limit of terms $\mathsf R(E)_n=\abs{\Fun([n],E)}$.
		After replacing each such realization by the nerve of
		$\Fun([n],E)$, a simplex over $I$ is therefore described by finitely
		many ordinals, maps between the corresponding necklaces, functors from
		these finite ordinals to $I$, and natural transformations between such
		functors, subject to the finitely many compatibility conditions imposed
		by the gluing of the necklace and by the endpoints $i,j$. Equivalently,
		it determines a finite category $K_\sigma$ and a functor
		$u_\sigma\colon K_\sigma\to I$.
		The summand of $B^{i,j}_q(F)$ lying over $\sigma$ is the anima of
		lifts of $u_\sigma$ along the left fibration $p$, or equivalently the
		anima of sections of $u_\sigma^{*}D\to K_\sigma$. By straightening over
		$K_\sigma$, this anima is
		\[
			\Map_{\Fun(K_\sigma,\An)}
			\bigl(\ast,F\circ u_\sigma\bigr)
			\simeq
			\lim_{K_\sigma}(F\circ u_\sigma).
		\]
		Therefore
		\[
			B^{i,j}_q(F)
			\simeq
			\coprod_{\sigma}\lim_{K_\sigma}(F\circ u_\sigma),
		\]
		where $\sigma$ runs over the finite necklace diagrams in $I$ of
		simplicial degree $q$ with endpoints $i$ and $j$. Each $K_\sigma$ is
		finite. If $I$ is countable, then there are only countably many such
		finite diagrams. Hence each $B^{i,j}_q(F)$ is a countable coproduct of
		finite limits of values of $F$.
	\end{proof}
	...
	\end{corollary}
	\begin{proof}
		Apply \Cref{prop:Two_Sided_Bar_Relation_Object} to the ordinary
	...
	\begin{proof}
		...
		lifts. By \Cref{cor:Relation_Object_Kappa_Small_Colimit}, the object
	...
	\begin{proof}
		...
		$R$ is the geometric realization of its Cech nerve. Since
		$R\to y(c)$ is a monomorphism, the terms of this Cech nerve are the
