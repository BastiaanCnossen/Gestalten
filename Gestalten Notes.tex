...
\begin{document}
	...
	\begin{observation}
		The basic closure properties of countably presented maps follow from the compact algebra lemmas in \Cref{sec:Basic_Properties_Compacts}, applied with $\kappa = \aleph_1$ and $C = \Sp$. We will spell out the particular consequences needed later in \Cref{lem:Affine_Countably_Presented_Calculus}.
	\end{observation}
	...
	\begin{definition}\label[definition]{def:Bi_Countably_Presented}
		Let $A \in \CAlg(\Sp)$. A map $Z \to \Spec(A)$ of fpqc sheaves is \emph{bi-countably presented over $\Spec(A)$} if $Z$ is a countable colimit, in $\Shv_{\fpqc}(\Aff)_{/\Spec(A)}$, of affines $\Spec(B_i)$ with $A \to B_i$ countably presented.
	...
	\begin{proposition}
		...
		The class $E$ is closed under composition and base change. Assuming \Cref{lem:Section_Of_Bicountably_Presented_Affine_Base}, it is also closed under sections and formation of diagonals.
	\end{proposition}
	\begin{proof}		
		...
		It remains to discuss sections and diagonals, conditional on the missing input \Cref{lem:Section_Of_Bicountably_Presented_Affine_Base}. The diagonal $\Delta_f\colon Y \to Y \times_X Y$ is a section of the first projection
		\[
			Y \times_X Y \to Y.
		\]
	...
	Consequently, the use of diagonal closure in the ambidexterity argument below should be read as conditional on the missing input \Cref{lem:Section_Of_Bicountably_Presented_Affine_Base}.
	...
	\begin{proof}
		...
		$\aleph_1$-compact objects of $\Shv_{\fpqc}(\Aff)$ are closed under
	...
	\begin{lemma}\label[lemma]{lem:Aleph1_Compact_Anima_Countable_Simplicial}
		An anima is $\aleph_1$-compact if and only if it can be represented by a
		countable simplicial set. In particular, $\aleph_1$-compact animae are
		closed under finite limits.
	\end{lemma}
	\begin{proof}
		First suppose that $X$ is represented by a countable simplicial set
		$K$. Then $K$ is the filtered colimit of its finite simplicial subsets.
		Since $K$ is countable, this is a countable colimit. Finite simplicial
		sets are compact in $\An$, and countable colimits of
		$\aleph_1$-compact objects are again $\aleph_1$-compact, because
		$\aleph_1$-filtered colimits in $\An$ commute with countable limits.
		Thus $X$ is $\aleph_1$-compact.

		Conversely, let $X$ be $\aleph_1$-compact, and choose a simplicial set
		$K$ representing it. Let $I$ be the poset of countable simplicial
		subsets of $K$. This poset is $\aleph_1$-filtered, since a countable
		union of countable simplicial subsets is again countable, and
		\[
		...
		a retract of an anima represented by a countable simplicial set. It
		follows that $\pi_0(X)$ is countable and that, for every base point,
		all higher homotopy groups of $X$ are countable.
		...
		$n$-skeleton has been constructed, attach countably many cells to hit
		the remaining elements of $\pi_n(X)$ and countably many further cells
		to kill the kernel on $\pi_n$. Since the homotopy groups of $X$ are
		countable, only countably many cells are needed at each stage. The
		result is a countable simplicial set $Y$ equipped with a map
		$\Piinfty{Y} \to X$ inducing an isomorphism on $\pi_0$ and on all
		higher homotopy groups. Hence $\Piinfty{Y} \to X$ is an equivalence.

		It remains to prove the closure under finite limits. The terminal
		anima is represented by $\Delta^0$, so it is enough to consider
		pullbacks. Let
		\[
			X \to Z \leftarrow Y
		\]
		be a diagram of $\aleph_1$-compact animae. Choose countable simplicial
		sets $K,L,M$ representing $X,Y,Z$. Replace $M$ by the countable Kan
		complex $M^{\fib}:=\operatorname{Ex}^{\infty}(M)$. This is still
		countable: for each $n$, the simplicial set $\operatorname{sd}\Delta^n$
		is finite, so
		\[
		...
		\]
		is countable whenever $N$ is countable, and then
		$\operatorname{Ex}^{\infty}(M)$ is a countable sequential colimit of
		countable simplicial sets. Since $M^{\fib}$ is fibrant, the two maps to
		$Z$ may be represented by actual maps of simplicial sets $K \to M^{\fib}$ and
		$L \to M^{\fib}$, and the homotopy pullback is represented by
		\[
			K
			\times_{M^{\fib}}
			(M^{\fib})^{\Delta^1}
			...
			L.
		\]
		This simplicial set is countable. Indeed,
		$(M^{\fib})^{\Delta^1}$ is countable because
		\[
			((M^{\fib})^{\Delta^1})_n
			=
		...
		countable simplicial sets are countable levelwise. Hence the pullback
		is represented by a countable simplicial set, and is therefore
		$\aleph_1$-compact.
	\end{proof}

	\begin{lemma}\label[lemma]{lem:Aleph1_Compact_Presheaves_Finite_Limits}
		The $\aleph_1$-compact objects of $\PSh(\Aff)$ are closed under finite
		limits.
	\end{lemma}
	\begin{proof}
		\red{TO DO.}
	...
	\begin{proof}
		...
		$L$ carries $\kappa$-compact objects of $C$ to $\kappa$-compact objects
		of $D$. Indeed, for $K \in C^{\kappa}$ and a $\kappa$-filtered diagram
		$(Y_j)$ in $D$, we have
		\[
			\Hom_D(LK,\colim_j Y_j)
			\simeq
			\Hom_C(K,\iota\colim_j Y_j)
			\simeq
			\Hom_C(K,\colim_j \iota Y_j)
			\simeq
			\colim_j\Hom_D(LK,Y_j).
		\]
		Since $L$ is essentially surjective and the $\kappa$-compact objects of
		$C$ generate $C$ under colimits, it follows that the objects $LK$ with
		$K \in C^{\kappa}$ generate $D$ under colimits. Thus $D$ is
		$\kappa$-compactly generated.

		Moreover, the $\kappa$-compact objects of $D$ are the retracts of
		objects of the form $LK$ with $K \in C^{\kappa}$. More precisely, the
		restriction $L^{\kappa}\colon C^{\kappa} \to D^{\kappa}$ exhibits
		$D^{\kappa}$ as the idempotent completion of the localization of
		$C^{\kappa}$ obtained by inverting the maps which become equivalences
		under $L$.

		We now use finite limits. Since $L$ preserves finite limits and
		$C^{\kappa}$ is closed under finite limits, the localized category
		$C^{\kappa}[L^{-1}]$ has finite limits and the functor
		$C^{\kappa} \to C^{\kappa}[L^{-1}]$ preserves them. Passing to
		idempotent completion preserves finite limits: equivalently, for every
		finite category $J$, idempotent completion commutes with
		$\Fun(J,-)$. Hence $D^{\kappa}$ is closed under finite limits in $D$.
	\end{proof}

	\begin{lemma}\label[lemma]{lem:Aleph1_Compact_fpqc_Finite_Limits}
		Assume \Cref{lem:Aleph1_Compact_Presheaves_Finite_Limits}. Then
		$\aleph_1$-compact objects of $\Shv_{\fpqc}(\Aff)$ are closed under
		finite limits.
	\end{lemma}
	\begin{proof}
		Let
		\[
			L\colon \PSh(\Aff) \rightleftarrows \Shv_{\fpqc}(\Aff) : \iota
		\]
		be sheafification and the inclusion. By the proof of
		\Cref{lem:Affines_Aleph1_Compact_fpqc}, the inclusion $\iota$
		preserves $\aleph_1$-filtered colimits. The sheafification functor $L$
		preserves finite limits. The presheaf category is
		$\aleph_1$-compactly generated, and its $\aleph_1$-compact objects are
		closed under finite limits by
		\Cref{lem:Aleph1_Compact_Presheaves_Finite_Limits}. Therefore
		\Cref{lem:Compact_Finite_Limits_Localization}, applied with
		$\kappa=\aleph_1$, gives the claim.
	\end{proof}
