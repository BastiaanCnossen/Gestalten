...
\begin{document}
	...
	Recall that we denote $1\Pr := \PrL_{\aleph_1}$. Given $C \in \CAlg(1\Pr)$, we have the adjunction of \Cref{nota:M_E_Adjunction},
	\[
		M_C\colon \CAlg(C) \rightleftarrows \CAlg(\Mod_C(1\Pr)) \simeq \CAlg(1\Pr)_{C/} \colon E_C,
	\]
	where $M_C$ is a fully faithful symmetric monoidal functor sending $B$ to $\Mod_B(C)$, and $E_C$ sends a commutative algebra $M$ over $C$ to the endomorphism object $\End_M(\unit)$.

	Applying this to $1\Pr \in \CAlg(1\Pr)$, we inductively define
	\[
		(n+1)\Pr := \Mod_{n\Pr}(1\Pr).
	\]
	We then define the presentable category $\StRing$ as the colimit
	\[
		\StRing := \colim(\CAlg(1\Pr) \xhookrightarrow{M_{1\Pr}} \CAlg(2\Pr) \xhookrightarrow{M_{2\Pr}} \dots)
	\]
	in $\CAlg(1\Pr)$. Equivalently, $\StRing$ is a limit of $\CAlg(n\Pr)$ along the endomorphism algebra functors $E_{n\Pr}\colon \CAlg((n+1)\Pr) \to \CAlg(n\Pr)$.

	\section{Stefanich rings associated to ring spectra}

	Now consider an arbitrary ring spectrum $A \in \CAlg(\Sp)$. We may then consider $0\Pr_A := \Mod_A(\Sp) \in \CAlg(1\Pr)$. Applying $M_{\Sp}$ from \Cref{nota:M_E_Adjunction} gives a fully faithful functor
	\[
		\CAlg(\Sp) \xhookrightarrow{M_{\Sp}} \CAlg(\Mod_{\Sp}(1\Pr)) \hookrightarrow \CAlg(1\Pr) \hookrightarrow \StRing.
	\]
	The image of this composite on $A$ is the sequence $(n\Pr_A)_{n \geq 0}$, where we inductively define
	\[
		n\Pr_A := \Mod_{(n-1)\Pr_A}(n\Pr) \in \CAlg((n+1)\Pr) = \CAlg(\Mod_{n\Pr}(1\Pr)) \simeq \CAlg(1\Pr)_{n\Pr/}
	\]
	for $n \geq 1$.

	\begin{remark}
		The choice of starting with commutative ring spectra is somewhat arbitrary and mostly done for expository purposes. More generally, we could replace $\CAlg(\Sp)$ by some other $\infty$-category $C$ of suitable ``derived rings'', e.g.\ animated rings, connective commutative $\Z$-algebras, or connective commutative $\S$-algebras, as long as we have some reasonable notion of the module category $\Mod_A(C) \in \CAlg(1\Pr)$. The main advantage of $\CAlg(\Sp)$ is that the functor to $\CAlg(1\Pr)$ is fully faithful, as being an $\Sp$-module in $1\Pr$ is the \emph{property} of being stable.
	\end{remark}

	\begin{observation}
		Consider $D \in \CAlg(1\Pr)$, and consider the inclusion $M_{\Mod_D(1\Pr)}$ of \Cref{nota:M_E_Adjunction},
		\[
			\CAlg(\Mod_D(1\Pr)) \hookrightarrow \CAlg(\Mod_{\Mod_D(1\Pr)}(1\Pr))
		\]
		induced by the symmetric monoidal inclusion $M_{1\Pr}\colon \CAlg(1\Pr) \hookrightarrow \CAlg(\Mod_D(1\Pr))$. By the fact that commutative algebras in the category of $D$-modules are the same as commutative $D$-algebras, and similarly for the right-hand side reidentifying the left-hand side and the right-hand side, we may rewrite this as a functor
		\[
			\CAlg(1\Pr)_{D/} \to \CAlg(1\Pr)_{\Mod_D(1\Pr)/}.
		\]
		Given an object $D \to B$ on the left-hand side, the corresponding algebra on the right-hand side is the map
		\[
			\Mod_D(1\Pr) \to \Mod_B(\Mod_D(1\Pr)) \simeq \Mod_B(1\Pr).
		\]
		So we may think of this functor as the effect of $M_{1\Pr}\colon \CAlg(1\Pr) \to \CAlg(1\Pr)$ on morphisms, given by base change along $B \to D$.
	\end{observation}

	By applying this to the map $n\Pr \to n\Pr_A$, we inductively see that
	\[
		n\Pr_A \simeq \Mod_{(n-1)\Pr_A}(1\Pr)
	\]
	for every $n \geq 1$, with the structure map $n\Pr \to n\Pr_A$ inductively induced by the maps $(n-1)\Pr \to (n-1)\Pr_A$ by passing to modules in $1\Pr$.

	We write
	\[
		\Aff := \CAlg(\Sp)\catop.
	\]
	For $A \in \CAlg(\Sp)$, we denote the corresponding object of $\Aff$ by $\Spec(A)$. Thus a map $\Spec(B) \to \Spec(A)$ in $\Aff$ is the same thing as a map $A \to B$ in $\CAlg(\Sp)$.
	...
	The goal of today is to show the following:
	...
	This functor is supposed to send a span in $\Aff$, equivalently a cospan in $\CAlg(\Sp)$ of the form
	\[
		A \xrightarrow{f} B \xleftarrow{f'} C
	\]
	to the composite functor
	\[
		n\Pr_A \xrightarrow{f_n^*} n\Pr_B \xrightarrow{f_{n,*}} n\Pr_C,
	\]
	where $f_{n,*}$ is right adjoint to $f_n^*$. Here $f_n^*$ is defined inductively: for $n = 1$ it is extension by scalars along $f\colon A \to B$, and for $n > 1$ it is extension by scalars along $f_{n-1}^*\colon (n-1)\Pr_A \to (n-1)\Pr_B$. We now discuss this in more detail.

	\section{Some formal nonsense}
	...
	The goal of today is the following theorem:
	...
	Let us now provide a proof sketch/rough plan for the proof:
	\begin{itemize}
		\item As we will see at the end of the talk, the claim may be reduced to the case of countably presented maps. So the main part of the talk focuses on that special case.
		\item We then first treat the case $n = 0$.
		\item And then we prove the claim for other $n$ by induction.
	\end{itemize}
	...
	The goal of this talk is to extend the discussion from the previous two talks from affine schemes to fpqc-stacks.

	In Talk 2 we introduced the functors
	\[
	...
	The goal of this talk is as follows:
	\begin{itemize}
	...
	We will now extend the assignment $X \mapsto \PrL_X$ to a six-functor formalism. We may think of this as a `higher' version of the six-functor formalism on $X \mapsto D(X)$. A first question is: what are the $!$-able maps? It turns out that at the higher categorical level, the class of $!$-able maps contains essentially \emph{all} maps. The relevant class of maps is the following:
