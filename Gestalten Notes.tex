...
\begin{document}
	...
	\begin{remark}
		In general $X \setminus Y$ need not be representable. If $f$ lies in $\Gest^{n\text{-ét}}$ for some $n$, then perhaps it might follow from abstract topos-theoretic arguments that $X \setminus Y$ is representable.
	\end{remark}
	...
	\begin{corollary}
		Consider a 1-presentable stable category $E \in 1\Pr_{\st}$. Then there are (possibly order-reversing) bijections between the following four posets:
		\begin{enumerate}
	...
	\emph{During the seminar, we did not discuss the material of this chapter as a separate talk; instead, \Cref{prop:Affineness_And_Diagonals} was proved directly as part of \Cref{chap:Examples}. The material was expanded into a separate chapter for the sake of clarity of exposition.}
	...
	The main result of this section shows that, under suitable hypotheses, passing to the diagonal drops the level of affineness by precisely one. Part (2) generalizes results of Gaitsgory on 1-affineness \cite{Gaitsgory_1Affineness}, and related results for the general case appeared in Stefanich's thesis \cite{Stefanich_HigherQCoh}.
	...
	\end{proposition}
	\begin{proof}
		Part (1) is an instance of \Cref{lem:Reduce_Affineness}, applied to the map $\Delta_f^*\colon A = \Oo(Y \times_X Y) \to B = \Oo(Y)$. Indeed, $\Delta_f^*$ is still $(n+1)$-affine and is $n$-prim by assumption, and has an $(n+1)$-affine section $\pr^*\colon \Oo(Y) \to \Oo(Y \times_X Y)$.
		...
		\]
		We need to show that $h$ is an isomorphism. Since $f$ is countably presented, \Cref{cor:Surjective_Affine_Truncation}, applied to the $(n+1)$-prim, eventually suave map $f$, shows that $h$ is surjective. It therefore suffices by \Cref{cor:Surjective_And_Injective_Implies_Iso} to show that $h$ is a monomorphism, i.e.\ that $\Delta_h$ is an isomorphism.

		By construction, $h$ is $(n+1)$-coaffine. We now check that $h$ is also $(n+1)$-prim. The map $g$ is $(n+1)$-affine and, by assumption, countably presented. Hence \Cref{prop:Proper_Properties}(1) shows that $g$ is $(n+1)$-proper. Applying the cancellation statement \Cref{prop:Proper_Properties}(4) to the factorization $f=g \circ h$, the $(n+1)$-primness of $f$ implies the $(n+1)$-primness of $h$. It then follows from \Cref{lem:Prim_Coaffine_Diagonal_Coaffine} that $\Delta_h$ is $n$-coaffine, and so it remains to show that $\Delta_h$ is also $n$-affine.

		Note that the classifying algebra of $g$ is
		\[
			(\Oo(Y)/\Oo(X))_{n+1} \qin \CAlg(\Oo(X)_{n+2}).
		\]

		Since $\Delta_f$ is assumed $n$-affine and $n$-affine maps are closed under right cancellation and base change, it suffices to show that $\Delta_g$ is $n$-affine in the following commutative diagram:
		\[
		\begin{tikzcd}
		...
		\end{tikzcd}
		\]
		Now, since the map $f\colon Y \to X$ is $(n+1)$-prim, the Künneth formula for prim maps (\Cref{lem:Kunneth_Prim}) implies that we have
		\[
			(\Oo(Y \times_X Y)/\Oo(X))_{n+1} \iso (\Oo(Y)/\Oo(X))_{n+1} \otimes_{\Oo(X)_{n+2}} (\Oo(Y)/\Oo(X))_{n+1}.
		\]
		This identifies the $(n+1)$-affinization of $Y \times_X Y$ over $X$ with $Y' \times_X Y'$. Indeed, under the equivalence
		\[
			\CAlg(\Oo(X)_{n+2}) \simeq \bigl(\Gest_X^{(n+1)\text{-affine}}\bigr)\catop,
		\]
		the object $Y'$ corresponds to $(\Oo(Y)/\Oo(X))_{n+1}$, and fiber products of $(n+1)$-affine Gestalten over $X$ correspond to tensor products of the corresponding algebras in $\CAlg(\Oo(X)_{n+2})$.

		Under this identification, $\Delta_g\colon Y' \to Y'\times_X Y'$ is the image of the map $\Delta_f\colon Y \to Y \times_X Y$ under the functor
		\[
			(-)^{(n+1)\text{-affine}}\colon \Gest_{/X} \to \Gest_{/X}^{(n+1)\text{-affine}}.
		\]
		This image is again $n$-affine: the $(n+1)$-affinization does not change the $(n+1)$-st categories, and hence does not change the relative algebra in degree $n$ for this map. Thus the comparison
		\[
			\Mod_{(\Oo(Y)/\Oo(Y \times_X Y))_n}\bigl(\Oo(Y \times_X Y)_{n+1}\bigr) \to \Oo(Y)_{n+1}
		\]
		for $\Delta_g$ is the same equivalence as for the $n$-affine map $\Delta_f$. Since $\Delta_g$ is already $(n+1)$-affine by \Cref{prop:Affineness_Properties}(4), this verifies the recursive criterion for $n$-affineness. This concludes the proof.
	\end{proof}
	...
	\begin{convention}
		Throughout this chapter, we will fix $n$ large enough, and work inside $\Gest_{/X}^{n\text{-}\et}$ for some Gestalt $X$, mostly $X = *$ or $X = \Gest(\D(A))$ for some $A \in \CAlg(\Sp)$. Since the inclusions $\Gest_{/X}^{n\text{-}\et} \hookrightarrow \Gest_{/X}^{(n+1)\text{-}\et}$, it doesn't matter which precise choice of $n$ we pick as long as all objects that are involved are $n$-étale.
	\end{convention}
	...
	\begin{example}[The empty set]
		...
		\[
			\emptyset = \Gest(*, *, *, \dots).
		\]
		We claim that this is always 1-affine. For each $n \geq 0$, we have $(\Oo(\emptyset)/\Oo(X))_n = *$. But for $n \geq 2$, the category $\Oo(X)_{n}$ is a module over the pointed category $1\Pr$, hence $\Oo(X)_n$ itself is pointed, so that $\Mod_*(\Oo(X)_n) = *$ as desired.
	...
	\begin{example}[Finite sets]
		...
		\]
		If $X = \Gest(A)$, we may express this as $\Gest(\prod_{I} A_0, \prod_I A_1, \prod_I A_2, \dots, \dots)$, since colimits of Gestalten correspond to limits of Stefanich rings.

		Note that $I$ is 0-étale over $X$, as a colimit of 0-étale Gestalten. We claim it is also 1-affine: we have $(\Oo(I)/\Oo(X))_n = \prod_I \unit \in A_n$, and so
		\[
			\Mod_{(\Oo(I)/\Oo(X))_{n-1}}(\Oo(X)_n) = \Mod_{\prod_I \unit}(A_n) \simeq \prod_I \Mod_{\unit} A_n \simeq \prod_{I} A_n = (\Oo(I)/\Oo(X))_{n}
		\]
	...
	\begin{example}[Countable animae]
		...
		We first show that it is $0$-proper: it is 0-étale hence 1-proper, and its diagonal has fibers by $B^n(\Z/2)$, so it remains to show it is $0$-prim. In other words, we need to show that the functor
		\[
			\D(\Q) \to \D(\Q)^{B^{n+1}(\Z/2)} \simeq \Mod_{\Q[\Omega(B^{n+1}(\Z/2))]}(\D(\Q)) \simeq \Mod_{\Q[B^n(\Z/2)]}(\D(\Q))
		\]
		has a $\D(\Q)$-linear right adjoint. For $n = 1$, we know that $H_*(B^n(\Z/2);\Q) \iso \Q$, so that $\Q[B^n(\Z/2)]$ is the monoidal unit in $\D(\Q)$, hence this map is even an isomorphism. It thus remains to show that the right adjoint of the functor $\D(\Q) \to \Fun(B(\Z/2),\D(\Q))$ preserves colimits. This functor is given by taking $\Z/2$-fixed points. But since $\D(\Q)$ is rational, the norm map $(-)_{h\Z/2} \to (-)^{h\Z/2}$ is an isomorphism, and so this is also a left adjoint, hence preserves colimits.

		We may now use \Cref{lem:Reduce_Affineness} to inductively show that $B^{n+1}(\Z/2)$ is not $n$-affine for any $n$. To see this, assume for contradiction that $B^{n+1}(\Z/2)$ is $n$-affine.Then its diagonal is $(n-1)$-affine, hence by the pullback square
		\[
		\begin{tikzcd}
	...
	Recall that for a Gestalt of the form $X = \Gest(\D(A))$ for some ring spectrum $A \in \CAlg(\Sp)$, we defined a functor
	\[
		\Gest(-)\colon \Shv_{\fpqc}(\Aff_A)^{\aleph_1} \to \Gest_{/X}, \qquad Y \mapsto \Gest(\Oo(Y), 1\Pr_Y, 2\Pr_Y, \dots).
	\]
	...
	\begin{example}[Affine line]
		...
		\[
			\Hom_{\Gest_{/X}}(Y,\bbA^1_X) \simeq \Hom_{\StRing_{A/}}(\St_A(A_0\{x\}), \Oo(Y)/\Oo(X)) \simeq \Hom_{\CAlg(A_1)}(A_0\{x\}, (\Oo(Y)/\Oo(X))_0) \simeq \Oo(Y)_0.
		\]
		Note that $\bbA^1_X$ is $0$-affine, hence $0$-proper and $1$-étale.
	...
	While may already define $\bbP^1_X$ over the absolute base, it turns out that before passing to the stable setting of $\D(\S)$, this is not well-behaved, and is different from the object one might expect. For concreteness, we explain the situation over a field $k$.
	...
	\begin{remark}
		...
		\]
		using ordinary polynomial algebras rather than free $E_{\infty}$-algebras, then the resulting Gestalt is also $0$-suave. Over $\D(\Q)$, the two choices of polynomial algebra agree, so the distinction between these two projective lines disappears.
	\end{remark}
	...
	\begin{example}
		...
		\[
			\pic(-)\colon \Gest_{/\D(\Q)} \to \An, X \mapsto \Oo(X)_1^{\times}.
		\]
