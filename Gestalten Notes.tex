...
\begin{document}
	...
	\begin{construction}
		...
		We now construct an internal form of the module-endomorphism adjunction. Namely, for a Stefanich ring $A$ and $m \geq 1$, we will produce an adjunction
		\[
			\begin{tikzcd}
	...
	\end{proposition}
	\begin{proof}
		Since $\Mod^A_m$ is a morphism in $1\Pr$, \Cref{lem:Fully_Faithful_On_Aleph1_Compacts} shows that it suffices to check that the unit of the adjunction $\Mod^A_m \dashv \End^A_m$ is an equivalence on $\aleph_1$-compact objects, i.e.\ on countably presented objects of $\CAlg(A_m)$. Let $R \in \CAlg(A_m)$ be countably presented. Since $\Mod^A_m(R) = L^A_m(\Mod_R(A_m))$, we get
		\[
