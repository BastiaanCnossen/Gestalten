...
\begin{document}
	...
	\begin{lemma}\label[lemma]{lem:Two_Sided_Bar_Relation_Object}
		Let $C$ be a small $\infty$-category, let $I$ be an ordinary small
		category, let $F\colon I \to \PSh(C)$ be a diagram, and let
		$i,j\in I$. There is a simplicial object
		\[
			B^{i,j}_{\bullet}(F)\in \Fun(\simp\catop,\PSh(C))
		\]
		with a natural equivalence
		\[
			\abs{B^{i,j}_{\bullet}(F)}
			\simeq
			F(i)\times_{\colim_I F}F(j).
		\]
		Moreover, each $B^{i,j}_q(F)$ is a coproduct, indexed by the
		$q$-simplices in the Bousfield--Kan category of finite zigzags in
		$I$ from $i$ to $j$, of objects of the form
		\[
			\lim_{K} F,
		\]
		where $K\to I$ is a finite diagram whose underlying graph is a finite
		zigzag from $i$ to $j$ together with the finitely many coherences
		appearing in a $q$-simplex, and $\lim_K F$ denotes the limit of the
		composite $K\to I\xrightarrow{F}\PSh(C)$. In particular, if $I$ is
		countable, then each $B^{i,j}_q(F)$ is a countable coproduct of finite
		limits of values of $F$.
	\end{lemma}
	\begin{proof}
		It is enough to check after evaluation at each object of $C$, since
		evaluation preserves limits and colimits. Thus we may assume
		$F\colon I\to\An$. Let
		\[
			D:=\int_I F
		\]
		be the category of elements. Then $\colim_I F$ is the groupoid
		completion $\abs{D}$. For points $x\in F(i)$ and $y\in F(j)$, the
		fiber of the left-hand side over $(x,y)$ is the mapping anima
		\[
			\Map_{\abs{D}}((i,x),(j,y)).
		\]
		The standard Bousfield--Kan path model for the classifying anima
		$\abs{D}$ identifies this mapping anima with the realization of the
		simplicial anima of finite zigzags in $D$ from $(i,x)$ to $(j,y)$.
		Letting $(x,y)$ vary gives the simplicial object
		$B^{i,j}_{\bullet}(F)$. A fixed $q$-simplex in the zigzag category
		still involves only finitely many objects of $D$ and morphisms between
		them, even though the set of such $q$-simplices may be infinite. After
		projecting this finite diagram to $I$, the space of lifts to
		$D=\int_I F$ is the corresponding finite limit of values of $F$. Here
		we use that the fibers of $D\to I$ are animae: whenever an intermediate
		vertex is allowed to vary over one such fiber, the usual co-Yoneda
		lemma contracts that variable to evaluation, so no additional colimit
		remains. This gives the stated description of the simplicial degrees
	...
	\end{proposition}
	\begin{proof}
		Apply \Cref{lem:Two_Sided_Bar_Relation_Object} to the ordinary
		category $I=\simp\catop$, the two objects $i=j=[0]$, and the diagram
		$Y_{\bullet}\colon \simp\catop\to\PSh(C)$. Since $\simp\catop$ is
		countable, the relation object is the geometric realization of a
		simplicial object whose $q$-simplices are countable coproducts of
		finite limits of the $Y_n$. By
		\Cref{lem:Finite_Limits_Kappa_Coproducts_Representables}, each such
		finite limit is a $\kappa$-small coproduct of representables. Since
		$\kappa$ is uncountable regular, countable coproducts and countable
		geometric realizations of $\kappa$-small coproducts of representables
		are $\kappa$-small colimits of representables. This proves the claim.
	\end{proof}
