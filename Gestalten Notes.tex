...
\begin{document}
	...
	\begin{enumerate}
		...
		\item[(2)] \emph{$\kappa$-presentable categories}: A category $C$ which has small colimits, for which the subcategory $C^{\kappa}$ of $\kappa$-compact objects is essentially small, and such that every object is a $\kappa$-filtered colimit of objects of $C^{\kappa}$;
	...
	\begin{example}
		The categories $\An$, $\Sp$, and $D(R)$ are all in $\PrL_{\aleph_0}$. If $X$ is a second-countable locally compact Hausdorff space, then $\Shv(X) \in \PrL_{\aleph_1}$. Categories of almost modules arising from standard countably presented almost setups are also $\aleph_1$-presentable. Without such countability hypotheses, these last two examples are presentable but need not be $\aleph_1$-presentable.
	\end{example}
	...
	\begin{definition}
		Let $\Rex_{\kappa}$ be the category of essentially small categories with $\kappa$-small colimits and functors preserving them. In the case $\kappa = \aleph_0$, we additionally require the categories to be idempotent complete.
	\end{definition}
	...
	\begin{theorem}
		\label[theorem]{thm:PrL_Kappa_Presentable}
	...
	\end{proposition}
	\begin{proof}[Sketch]
		This is \cite[Proposition 2.11]{Scholze_Gestalten}. If $\kappa$ is an uncountable regular limit cardinal, then
		\[
			\PrL_{\kappa} \simeq \colim_{\substack{\lambda < \kappa \\ \lambda\ \mathrm{regular}}} \PrL_{\lambda}
		\]
		is a $\kappa$-filtered colimit in $\PrL_{\kappa}$, but $\PrL_{\kappa}$ is not a retract of any stage, so it is not $\kappa$-compact. If $\kappa = \lambda^+$, the data of $\kappa$-small colimits can be encoded by an initial object, pushouts, and coproducts indexed by sets of cardinality at most $\lambda$, subject to fewer than $\kappa$ compatibilities. This presents $\Rex_{\kappa} \simeq \PrL_{\kappa}$ as a $\kappa$-compact object of itself. The case $\kappa = \aleph_0$ requires a separate argument.
	\end{proof}
	...
	\begin{remark}
		...
		The analogous statement in $\PrL_{\kappa}$ holds whenever $|I| < \kappa$: the coproduct $\coprod_{i \in I}^{\PrL_{\kappa}} C_i$ is always the naive product $\prod_i C_i$, whereas the product $\prod_{i \in I}^{\PrL_{\kappa}} C_i = \Ind_{\kappa}(\prod_i C_i^{\kappa})$ generally differs from it. The two agree when $|I| < \kappa$, but may also agree for larger $I$ in degenerate cases.
	\end{remark}
	...
	\begin{proof}
		By \cite[Proposition 5.5.3.8]{lurie2009HTT}, the category $\FunL(C,D)$ is presentable. A colimit-preserving functor $E \to \FunL(C,D)$ is the same as a functor $E \times C \to D$ which preserves colimits separately in both variables, hence, by the universal property of the tensor product, the same as a colimit-preserving functor $E \otimes C \to D$. This proves the first assertion.

		For the second assertion, Gabriel--Ulmer duality identifies $\PrL_{\kappa}$ with $\Rex_{\kappa}$. The internal hom in $\Rex_{\kappa}$ is $\Fun^{\kappa\text{-}\rex}(C^{\kappa},D^{\kappa})$, since a functor into it is precisely a functor of two variables preserving $\kappa$-small colimits separately. Applying $\Ind_{\kappa}$ gives the displayed internal hom in $\PrL_{\kappa}$.
	\end{proof}
	...
	\begin{proof}
		...
		\]
		preserves $\kappa$-compact objects. The monoidal unit $\An = \lra{[0]}^{\kappa}$ is $\kappa$-compact. Moreover, the $\kappa$-compact objects of $\Rex_{\kappa}$ are retracts of $\kappa$-small colimits of the generators $\lra{[n]}^{\kappa}$. Since the tensor product preserves $\kappa$-small colimits separately and preserves retracts, it is enough to observe that
		\[
			\lra{[n]}^{\kappa} \otimes \lra{[m]}^{\kappa} \simeq \lra{[n] \times [m]}^{\kappa},
		\]
		which is $\kappa$-compact.
	\end{proof}
	...
	\begin{theorem}
		...
		\]
		which is moreover a morphism in $\PrL_{\kappa}$. Its right adjoint
		\[
	...
	\end{theorem}
	\begin{proof}
		Full faithfulness and the description of the right adjoint are \cite[Corollary 4.8.5.21 and Remark 4.8.5.12]{lurie2016HA}. The displayed functor is a morphism in $\PrL_{\kappa}$ precisely because its right adjoint preserves $\kappa$-filtered colimits; this final assertion is \cite[Theorem 3.11]{Scholze_Gestalten}.
	\end{proof}
	...
	The first stage $\CAlg(\An)$ is redundant: the inclusion of the tail beginning with $\CAlg(1\Pr)$ is cofinal, and in the limit description the degree-zero term is uniquely determined as the endomorphism algebra of the unit in degree one. We retain it to make the degree-zero component of a Stefanich ring explicit.
	...
	For the present discussion the reader may take $\Cc$ to be an object of $2\Pr$, though the definition makes sense for any reasonable notion of $(\infty,2)$-category. If $F$ is a left adjoint, its right adjoint $F^R$ is unique, and the left adjoint of $F^R$ is again $F$ (with the same $\eta$ and $\epsilon$).
	...
	\begin{proof}
		...
		\]
		The left-hand side is equivalent to $(n-1)\Pr_{B \otimes_A B}$, and under this equivalence the map agrees with $\nabla^*_{n-1}$ induced by the codiagonal map $\nabla\colon B \otimes_A B \to B$. The map $B \to B \otimes_A B$ is obtained from $A \to B$ by base change, and $\nabla$ is a retraction of it. Thus $\nabla$ is countably presented by \Cref{lem:Affine_Countably_Presented_Calculus}, so $\nabla^*_{n-1}$ admits a $(n-1)\Pr_{B \otimes_A B}$-linear right adjoint.

		For the claim about base change, we apply the same result inside the 3-category $\Ar((n+1)\Pr_A)$, the arrow category of $(n+1)\Pr_A$. Since adjoints in this arrow category are precisely the adjointable squares, this gives the claim. Finally, the projection formula for $f_{n,\sharp}$ follows from the identification $f_{n,\sharp} \simeq f_{n,*}$ and the projection formula for $f_{n,*}$ proved in \Cref{rmk:Basic_Properties_Extension_Scalars}\eqref{it:Restriction_Scalars_Projection_Formula}.
	...
	\end{lemma}
	\begin{proof}
		The homotopy category of $\D(A)$ is a Brown category, generated by the perfect $A$-modules, so the cited theorem applies.
	\end{proof}
	...
	\begin{claim}\label[claim]{cl:CountablyGeneratedFlat}
		For each $m \geq 0$, there is a countably presented flat connective $A$-module $Q_m$ such that
		\[
			C_m := \cofib(A \to \lim_{\simp_{\leq m}} A_\bullet) \simeq Q_m[-m].
		\]
	\end{claim}
	...
	\begin{theorem}[{Lazard, \cite[Theorem 7.2.2.15]{lurie2016HA}}]
	...
	Combining this with \Cref{cl:CountablyGeneratedFlat}, we may write $C_{m+1} \simeq \colim_{i \geq 0} A^{\oplus n_i}[-m-1]$. If $Z$ is any perfect $A$-module and $Z \to C_{m+1}$ is any map, then it factors through some finite stage $Z \to A^{\oplus n_i}[-m-1]$. It therefore suffices to show that every map $A[-m-1] \to C_m$ is nullhomotopic. Such maps are classified by $\pi_{-m-1}(C_m)$, which vanishes because $C_m \simeq Q_m[-m]$ with $Q_m$ connective and flat. Thus $C_{m+1} \to C_m$ is phantom.

	We now prove \Cref{cl:CountablyGeneratedFlat}. Since each $A_r$ is flat over the connective ring spectrum $A$, there is a canonical equivalence
	\[
		A_r \simeq A \otimes_{\pi_0 A} \pi_0 A_r.
	\]
	The exact functor $A \otimes_{\pi_0 A} -$ commutes with the finite limits defining the partial totalizations. It therefore suffices to compute the cofiber after replacing $A \to A_\bullet$ by the fpqc-hypercover of static rings $\pi_0 A \to \pi_0 A_\bullet$.

	Let $N^{\bullet}$ be the normalized cochain complex of the augmented cosimplicial $A$-module $A \to A_{\bullet}$:
	\[
		A \to N^0 \to N^1 \to \dots.
	\]
	After the faithfully flat base change $A \to A_0$, the augmented hypercover is split, so this complex becomes contractible. Faithful flatness therefore implies that it is exact. The same argument applies after every base change in $A$.

	The Dold--Kan description of partial totalizations now gives
	\[
		C_m \simeq \coker(N^{m-1} \to N^m)[-m],
	\]
		where $N^{-1}:=A$. Each $N^m$ is a direct summand of the countably presented flat $A$-module $A_m$, hence is countably presented and flat. Exactness after arbitrary base change identifies the displayed cokernel with a submodule of $N^{m+1}$ whose inclusion remains injective after arbitrary base change. Since $N^{m+1}$ is flat, the equational criterion for flatness shows that this cokernel is flat. It is countably presented because it is a cokernel of a map between countably presented modules. Base-changing this description from $\pi_0 A$ back to $A$ proves \Cref{cl:CountablyGeneratedFlat}.
	...
	\[
		...
		\end{tikzcd}
	\]
	After base change along $A \to A_0$, the hypercover is split. Hence
	\[
	...
	\]
	which is the special case of \v{C}ech descent. Applying these two \v{C}ech-descent equivalences levelwise and then interchanging the two resulting limits identifies descent for $A \to A_{\bullet}$ with descent for the \v{C}ech nerve of $A \to A_0$.
	...
	Set $L := \colim_* \D(A_{\bullet})$ and let $u\colon L \to \D(A)$ be the augmentation. By Step 1, the right adjoint $\D(A) \to L$ is fully faithful, so $u$ is a localization and admits a $\D(A)$-linear section. Thus $\D(A)$ is a retract of $L$ in $\PrL_{\D(A)}$. On the other hand, the splitting after base change to $A_0$ shows that $u$ becomes an isomorphism after tensoring with every $\D(A_i)$ and hence after tensoring with $L$. Therefore $u = u \otimes_{\D(A)} \D(A)$ is a retract of the isomorphism $u \otimes_{\D(A)} L$, and is itself an isomorphism. Equivalently, the composite
	\[
		\D(A) \to \colim_* \D(A_{\bullet}) \to \D(A)
	\]
	is the identity in $\Pr_{\D(A)}$.

	This finishes the proof in the countably presented case, for $n = 0$.

	\begin{lemma}[{\cite[Lemma 4.11]{Scholze_Gestalten}}]\label[lemma]{lem:Countable_Colimits_Dualizable_Left_Adjoints}
		Let $\Vv \in \CAlg(1\Pr)$ and let $I$ be a countable category. Suppose that $C\colon I \to \Mod_{\Vv}(1\Pr)$ takes values in dualizable $\Vv$-linear categories and that every transition functor is a left adjoint internal to $\Mod_{\Vv}(1\Pr)$. Then $\colim_I C$ is dualizable.
	\end{lemma}
	...
	For $n \geq 2$, the object $F_{n,\sharp}F_n^*(\unit)$ is the countable colimit of the objects $(f_i)_{n,\sharp}(\unit)$ along internal left adjoints. The affine dualizability statement \Cref{prop:Affine_Ambidexterity}, together with \Cref{lem:Countable_Colimits_Dualizable_Left_Adjoints}, shows that it is dualizable. We may therefore dualize the counit
	\[
		F_{n,\sharp}F_n^*(\unit) \longrightarrow \unit.
	\]
	Under affine ambidexterity, its dual is precisely the comparison map one categorical level lower,
	\[
		(n-1)\Pr_A \longrightarrow \lim_{[i]\in\simp}(n-1)\Pr_{A_i},
	\]
	which is an isomorphism by the induction hypothesis. Hence the counit is an isomorphism and $F_n^*$ is fully faithful.

	For essential surjectivity, the reduction of Step 2a again reduces us to the \v{C}ech nerve of $A \to A_0$. We must show that the unit $\id \to F_n^*F_{n,\sharp}$ is an isomorphism. After base change along $A \to A_0$, the augmented \v{C}ech nerve is split, so the unit becomes an isomorphism. The pullback $n\Pr_A \to n\Pr_{A_0}$ is conservative: this follows from the full faithfulness just proved, applied to the same \v{C}ech nerve. Thus the unit was already an isomorphism over $A$.

	This finishes the proof.

	\section{Reduction to the countably presented case}
	\label[section]{sec:Reduction_Countably_Presented_Hypercovers}
	...
	\begin{proof}
		...
		The commutativity, associativity, and unitality of this multiplication each amount to an equality of two maps out of a countably presented module ($M_{i_n}^{\otimes 2}$, resp.\ $M_{i_n}^{\otimes 3}$, resp.\ $M_{i_n}$) that holds after composing into the commutative $A$-algebra $B$. By the second consequence above, each such identity holds after passing to a later stage; as there are only countably many of them, they can all be arranged to hold on a single cofinal subsystem. On such a subsystem $M$ is a countably presented flat $A$-algebra equipped with a map to $B$, and these algebras form an $\aleph_1$-filtered system with colimit $B$. This writes $B$ in the desired form.

		In general, write the map $A \to B$ as the $\aleph_1$-filtered colimit of all countably presented $A$-algebras $B_i$ mapping to $B$. We want to see that the set of $i$ for which $A \to B_i$ is faithfully flat is cofinal. We get a corresponding presentation of $\pi_0 B$ as the colimit of the $\pi_0 B_i$, and by the static case, for any $i$ there is some $j$ so that $\pi_0 B_i \to \pi_0 B_j$ factors through a countably presented faithfully flat $\pi_0 A$-algebra mapping to $\pi_0 B$. Taking a sequence $i_0, i_1, \ldots$ which successively incorporates these factorizations, the colimit $B_j = \colim(B_{i_0} \to B_{i_1} \to \ldots)$ is still countably presented and has the property that $\pi_0 A \to \pi_0 B_j$ is faithfully flat. For any fixed $n \in \Z$, the same approximation argument arranges that
		\[
			\pi_0 B_j \otimes_{\pi_0 A} \pi_n A \longrightarrow \pi_n B_j
		\]
		is an isomorphism. Since there are only countably many $n$, one further diagonal sequence arranges these isomorphisms simultaneously for all $n$.
	\end{proof}

	As a consequence, every fpqc-hypercover is an $\aleph_1$-filtered colimit of countably presented fpqc-hypercovers. One constructs the approximation inductively in the simplicial degree, using that each matching object is a finite limit and applying \Cref{lem:Faithfully_Flat_Approximation} to the matching map; a diagonal countable refinement makes the construction compatible in all degrees.
	...
	\]
	where we use that $A \mapsto n\Pr_A$ preserves $\aleph_1$-filtered colimits and that, in $1\Pr$, $\aleph_1$-filtered colimits commute with countable limits. This finishes the proof of the general form of the theorem.
	...
	\]
	for all $n \geq 0$, and showed they canonically extend to six-functor formalisms in which the countably presented maps are taken as the $!$-able maps, with $f_! = f_*$. Denote by
	\[
	...
	\begin{proof}
		...
		\]
		Since $n \geq 1$ and all transition maps are countably presented, the result from Talk 2 shows that all the pullback functors involved admit linear left adjoints. Here countable presentation of a transition map follows from \Cref{lem:Affine_Countably_Presented_Calculus}(iii), since both its source and target are countably presented over $A$. In particular, the limit defining $n\Pr_Y$ is along right adjoints, and hence agrees with the colimit along the corresponding left adjoints:
		\[
		...
		For (iv), we evaluate at the unit. In the affine case, we have $(g_i)_{n,\sharp} \simeq (g_i)_{n,*}$ by ambidexterity from Talk 2, so
		\[
			f_{n,\sharp}(\unit) \simeq \colim_{i \in I} (g_i)_{n,*}(\unit) \simeq \colim_{i \in I,*} (n-1)\Pr_{B_i},
		\]
		where the colimit is along the lower-$\ast$ functors. For $n \geq 2$ (so that $n - 1 \geq 1$), the lower-$\ast$ functors $(n-1)\Pr_{B_i} \to (n-1)\Pr_{B_j}$ are themselves linear left adjoints. Hence the colimit above is a countable colimit of dualizable objects along internal left adjoint functors, and is dualizable by \Cref{lem:Countable_Colimits_Dualizable_Left_Adjoints}.
	...
	\begin{proof}
		...
		\end{aligned}
		\]
		For fixed compact $K$, both sides preserve $\aleph_1$-filtered
		colimits in $E$: on the left this uses that $G$ preserves colimits and
		that $K$ is $\aleph_1$-compact, while on the right it uses that
		$f_n^*K$ is $\aleph_1$-compact. Thus the equivalence extends from the
		compact generators represented by the stages to every
		$E \in n\Pr_Y$. It then extends from compact $K$ to arbitrary $K$ by
		writing $K$ as a colimit of compact objects. Hence $G$ is right adjoint
		to $f_n^*$, so $G \simeq f_{n,*}$.
	\end{proof}
	...
	\begin{proof}
		...
		\[
			\CAlg(\Mod_{A_{m+1}}((m+1)\Pr)) \xrightarrow{\End_{(-)}(\unit)} \CAlg(A_{m+1})
		\]
		and
		\[
			\CAlg(A_{m+1}) \xrightarrow{U^A_m} \CAlg(\Mod_{A_m}(m\Pr)),
		\]
	...
	\begin{proof}
		...
		Finally, $L^A_m$ preserves $\aleph_1$-compact objects, so its right adjoint $U^A_m$ preserves $\aleph_1$-filtered colimits. The standard compact-generation argument of \Cref{lem:Fully_Faithful_On_Aleph1_Compacts} therefore extends the preceding calculation from dualizable objects to their $\aleph_1$-filtered colimits.
	\end{proof}
	...
	\begin{proof}
		...
		\]
		is an equivalence. Since $Y$ lies in $\StRing(A)$, the map $\eta_Y\colon Y \to TY$ is an equivalence. An inverse to $\eta_{X^{(\alpha)}}^*$ is therefore given by applying $T$ to a map $X^{(\alpha)} \to Y$ and then using $\eta_Y^{-1}$ to identify $TY$ with $Y$. The two composites are the identity by naturality of $\eta$ and the identity $T\eta = \eta T$, which follows directly from the definition of $\eta$ by the structure maps.
	...
	\begin{lemma}\label[lemma]{lem:Prim_Relative_Algebras_Countably_Presented}
		Let $f\colon A \to B$ be an $n$-prim map of Stefanich rings. Then, for every $m \geq n$, the relative algebra
		\[
			(B/A)_m \in \CAlg(A_{m+1})
		\]
		is countably presented. Consequently, the $m$-affine truncation of $B$ over $A$ is countably presented. In particular, if $f$ is also $m$-affine, then $f$ is countably presented and $m$-proper.
	\end{lemma}
	\begin{proof}
		By $n$-primness, the relative pullback
		\[
			(f/A)^*_m\colon \unit \to (B/A)_{m+1}
		\]
		admits an internal right adjoint $(f/A)_{m,*}$ in $A_{m+2}$. The underlying functor of this internal right adjoint is a morphism in the relevant $\aleph_1$-presentable category and therefore preserves $\aleph_1$-compact objects. The monoidal unit of $(B/A)_{m+1}$ is $\aleph_1$-compact, and \Cref{obs:Endomorphism_Object_Via_Right_Adjoint} identifies
		\[
			(f/A)_{m,*}(\unit) \simeq (B/A)_m
		\]
		as an object of $A_{m+1}$. Thus the underlying $A_{m+1}$-module of $(B/A)_m$ is $\aleph_1$-compact. By \Cref{lem:Kappa_Compact_Algebra_Iff_Module}, applied with $\kappa=\aleph_1$, the algebra $(B/A)_m$ is countably presented.

		The fully faithful functor $\CAlg(A_{m+1}) \hookrightarrow \StRing_{A/}$ defining $m$-affine Stefanich algebras is a morphism in $1\Pr$, so it carries $(B/A)_m$ to a countably presented $m$-affine truncation. If $f$ itself is $m$-affine, it agrees with this truncation and is therefore countably presented; it is then $m$-proper by \Cref{prop:Proper_Properties}(1).
	\end{proof}

	\begin{remark}
		The lemma does not assert that an arbitrary $n$-prim map is countably presented as an object of $\StRing_{A/}$. It asserts that all of its relative algebras in the prim range, and hence all affine truncations in that range, are countably presented. Countable presentation of the map itself follows when the map is affine in one of these degrees.
	\end{remark}
	...
	\begin{proposition}\label[proposition]{prop:Prim_Properties}
		...
		\begin{enumerate}
			...
			\item The $n$-prim Stefanich $A$-algebras are closed under all countable colimits. Moreover, for every $m \geq n$, the formation of these colimits commutes with the functor
			\[
				\StRing_{A/} \to \CAlg(A_{m+1}), \qquad B \mapsto (B/A)_m.
			\]
	...
	\end{proposition}
	\begin{proof}
		(1) Since $B/A$ is $n$-affine and countably presented, $(B/A)_n$ is countably presented in $\CAlg(A_{n+1})$. The functor $R \mapsto \Mod_R(A_{m+1})$ preserves countably presented objects by \Cref{lem:Mod_Preserves_Countably_Presented}, and $L^A_{m+1}$ preserves them because it is a morphism in $(m+2)\Pr$. Induction along the $n$-affine presentation therefore shows that $(B/A)_m$ is countably presented for every $m \geq n$.

		The map $(f/A)^*_m\colon (A/A)_{m+1} \to (B/A)_{m+1}$ factors as
		\[
			(A/A)_{m+1} \to \Mod^A_{m+1}((B/A)_m) \xrightarrow{L^{B/A}_m} (B/A)_{m+1}.
		\]
		If $B/A$ is $n$-affine, then the second map is an equivalence for $m \geq n$, and so this composite has a right adjoint in $A_{m+2}$ if and only if the first map admits a right adjoint. By definition, the first map is the image under the 2-functor $L^A_{m+1}\colon \Mod_{A_{m+1}}((m+1)\Pr) \to A_{m+2}$ of the free module functor
		\[
			A_{m+1} \to \Mod_{(B/A)_m}(A_{m+1}).
		\]
		If $(B/A)_m \in \CAlg(A_{m+1})$ is countably presented, then this free module functor admits a right adjoint in $\Mod_{A_{m+1}}((m+1)\Pr)$ by part \eqref{it:Restriction_Scalars_Internal_Right_Adjoint} of \Cref{rmk:Basic_Properties_Extension_Scalars}. Applying the 2-functor $L^A_{m+1}$ preserves this adjunction and supplies the required right adjoint in $A_{m+2}$.
		...
		(2) Every countable colimit can be assembled from finite coproducts and sifted countable colimits. Finite coproducts are covered by (3) and (4): the coproduct of $B$ and $C$ in $\StRing_{A/}$ is $B \otimes_A C$, and the map $A \to B \otimes_A C$ factors as an $n$-prim map followed by a cobase change of an $n$-prim map. The levelwise assertion follows at the same time from the base-change isomorphism in (3).

		It remains to consider a countable sifted category $I$ and a diagram $i \mapsto B(i)$ of $n$-prim Stefanich $A$-algebras. For $m \geq n$, set
		\[
			R_m := \colim_{i \in I}(B(i)/A)_m \qin \CAlg(A_{m+1}).
		\]
		Since $I$ is sifted, this colimit is also the colimit of the underlying objects of $A_{m+1}$. Apply the lemma below to the countable category $I^{\lhd}$, with value $\unit$ at its initial object and value $(B(i)/A)_{m+1}$ at $i \in I$. It follows that the unit map $u_m\colon \unit \to R_{m+1}$ admits a right adjoint and that the endomorphism $u_m^R u_m$ is the colimit of the corresponding endomorphisms for the $B(i)$. By \Cref{obs:Endomorphism_Object_Via_Right_Adjoint}, evaluation at the unit identifies this colimit with $R_m$. Consequently
		\[
			R_m \iso \End^A_{m+1}(R_{m+1})
		\]
		for every $m \geq n$. Thus these levels already satisfy the Stefanich condition, and spectrification does not change them. They are therefore the levels of $\colim_{i \in I}B(i)$, the displayed adjunctions show that this colimit is $n$-prim, and the formula for $R_m$ proves the asserted compatibility with the functors $B \mapsto (B/A)_m$.
	\end{proof}

	\begin{lemma}\label[lemma]{lem:Filtered_Colimit_Of_Adjoints}
		Let $I$ be a countable category with an initial object $0$, let $\bbC \in 2\Pr$, and consider a functor $X\colon I \to \bbC$. Assume that each $F_i\colon X_0 \to X_i$ has a right adjoint $F_i^R$. Then the functor $F\colon X_0 \to \colim_I X$ admits a right adjoint $F^R\colon \colim_{i \in I} X_i \to X_0$, and we have
		\[
			F^R F = \colim_{i \in I} F_i^R F_i \qin \bbC(X_0, X_0).
		\]
		The unit $\id_{X_0} \to F^R F$ is induced by the initial term $F_0^R F_0 = \id_{X_0}$.
	\end{lemma}
	\begin{proof}
		...
		In $1\Pr$ this is the standard comparison between the colimit presentation $\colim_{i \in I} X_i$ in $\PrL$ and the limit presentation $\lim_{i \in I^{\catop}} X_i$ formed along the right adjoints. Writing $\iota_i\colon X_i \to \colim_I X$ for the structure maps, each $\iota_i$ admits a right adjoint $\iota_i^R$; since $0 \in I$ is initial we have $F = \iota_0$, and hence a right adjoint $F^R := \iota_0^R$. Under the limit presentation $F^R$ is computed by the partial right adjoints, and the Beck--Chevalley condition required above holds by construction. Tracing through this description, the composite $F^R F$ is computed as the colimit
		\[
			F^R F = \colim_{i \in I} F_i^R F_i \qin \bbC(X_0, X_0),
		\]
		and the unit $\id \to F^R F$ is the inclusion of the initial index $i = 0$, for which $F_0 = \id_{X_0}$. Alternatively, one writes down the unit and counit of the adjunction explicitly and checks the triangle identities directly.
	\end{proof}
	...
	\begin{lemma}[Künneth for prim maps]\label[lemma]{lem:Kunneth_Prim}
		...
		\[
			(B \otimes_A C/A)_n \simeq (B/A)_n \otimes_{A_{n+1}} (C/A)_n.
		\]
	\end{lemma}
	\begin{proof}
	...
	\begin{proposition}\label[proposition]{prop:Proper_Rigid}
		...
		\begin{enumerate}
			...
			\item The multiplication map $(B/A)_1 \otimes_{A_2} (B/A)_1 \to (B/A)_1$ admits a right adjoint in
			\[
				\Mod_{(B/A)_1 \otimes_{A_2} (B/A)_1}(A_2).
			\]
		\end{enumerate}
	\end{proposition}
	\begin{proof}
	...
	\[
		(f/A)_{m-1,\sharp} \simeq ((f/A)^!_{m-1})^{\vee}
	\]
	...
	\begin{lemma}[Künneth for suave maps]\label[lemma]{lem:Kunneth_Suave}
		...
		\[
			(B \otimes_A C/A)^!_n \longrightarrow (B/A)^!_n \otimes_{A_{n+1}} (C/A)^!_n
		\]
		is an isomorphism, where the displayed map is the canonical Künneth map obtained by base change and the projection formula.
	\end{lemma}
	\begin{proof}
		...
		\[
			(B \otimes_A C/A)^!_n = (\pr \circ g)_{n,\sharp}(\unit) \iso g_{n,\sharp}\bigl(\pr_{n,\sharp}(\unit)\bigr) = g_{n,\sharp}\bigl((B \otimes_A C/C)^!_n\bigr).
		\]
		Since $\pr$ is the cobase change of the $n$-suave map $f$ along $g$, \Cref{prop:Suave_Properties}(1) gives $(B \otimes_A C/C)^!_n \iso g_n^*\bigl((B/A)^!_n\bigr)$, and therefore
		\[
			(B \otimes_A C/A)^!_n \iso g_{n,\sharp} g_n^*\bigl((B/A)^!_n\bigr) \iso (B/A)^!_n \otimes_{A_{n+1}} g_{n,\sharp}(\unit) = (B/A)^!_n \otimes_{A_{n+1}} (C/A)^!_n,
		\]
		using the projection formula for the left adjoint $g_{n,\sharp}$. By the definition of the base-change and projection maps as mates, this composite is the canonical Künneth map in the statement.
	\end{proof}
	...
	\begin{proposition}\label[proposition]{prop:Ambidexterity_Stefanich_Rings}
		...
		\end{enumerate}
		Moreover, in any of these cases, we have an identification $f_{m,\sharp} \simeq f_{m,*}$ for each $m \geq n+1$.
	\end{proposition}
	\begin{proof}
	...
	\begin{lemma}\label[lemma]{lem:Linear_Adjoint_Preserves_Dualizable}
		Let $\varphi\colon C \to D$ be a morphism in $\CAlg(\PrL)$, and suppose $\varphi$ admits both a $C$-linear left adjoint $\varphi_{\sharp}$ and a $C$-linear right adjoint $\varphi_{*}$. Then both $\varphi_{\sharp}$ and $\varphi_{*}$ preserve dualizable objects.
	\end{lemma}
	\begin{proof}
		Let $N \in D$ be dualizable, with dual $N^{\vee}$. Since $\varphi$ is symmetric monoidal, it preserves dualizable objects, and $C$-linearity of the adjoints yields the projection formulas
		\[
		...
		\]
		for $Y \in D$ and $M \in C$. Using the adjunction $\varphi_{\sharp} \dashv \varphi \dashv \varphi_{*}$ together with these formulas, we compute, naturally in $M \in C$,
		\[
			\Hom_C(\varphi_{\sharp} N, M)
			\simeq \varphi_{*}\Hom_D(N, \varphi(M))
			\simeq \varphi_{*}\bigl(N^{\vee} \otimes \varphi(M)\bigr)
			\simeq \varphi_{*}(N^{\vee}) \otimes M.
		\]
		Setting $M = \unit$ gives $\Hom_C(\varphi_{\sharp} N, \unit) \simeq \varphi_{*}(N^{\vee})$, so the displayed natural equivalence becomes $\Hom_C(\varphi_{\sharp} N, M) \simeq \Hom_C(\varphi_{\sharp} N, \unit) \otimes M$. Hence $\varphi_{\sharp} N$ is dualizable, with dual $\varphi_{*}(N^{\vee})$. Applying this conclusion to $N^{\vee}$ shows that $\varphi_{\sharp}(N^{\vee})$ is dualizable with dual $\varphi_{*}(N)$, so $\varphi_{*}(N)$ is dualizable as well.
	\end{proof}
	...
	\begin{lemma}\label[lemma]{lem:Adjoints_Countable_Limits}
		...
		\begin{enumerate}
			...
			\[
				F_\sharp F \simeq \colim_{i \in I\catop} F_{i,\sharp} F_i \qin \End_{\bbC}(Y).
			\]
	...
	\begin{proposition}\label[proposition]{prop:Suave_Countable_Limits}
		...
		\begin{enumerate}
			...
			\[
				(B/A)^!_m \simeq \colim_{i \in I\catop} (B_i/A)^!_m.
			\]
	...
	\begin{proof}
		...
		\]
		is an equivalence. By the base-change formula of \Cref{prop:Suave_Properties}(1) and the fact that $g_m^*$ preserves colimits, we have for every $m \geq 0$
		\[
			(B \otimes_A C/C)^!_m
			\simeq g_m^*(B/A)^!_m
			\simeq g_m^* \colim_{i \in I\catop} (B_i/A)^!_m
			\simeq \colim_{i \in I\catop} (B_i \otimes_A C/C)^!_m.
		\]
		On the other hand, applying part (1) to the diagram $i \mapsto B_i \otimes_A C$ of $0$-suave $C$-algebras gives
		\[
			\Bigl(\bigl(\lim_{i \in I} (B_i \otimes_A C)\bigr)/C\Bigr)^!_m \simeq \colim_{i \in I\catop} (B_i \otimes_A C/C)^!_m.
		\]
	...
	\begin{proof}
		...
		$(3) \Rightarrow (4)$: Let $L\colon \PSh(C)^{\aleph_1} \to \Yy$ be a left exact localization as in (3). First, $\PSh(C)^{\aleph_1}$ satisfies the countable Giraud axioms: indeed $\PSh(C) \simeq \Ind_{\aleph_1}(\PSh(C)^{\aleph_1})$ is a presheaf topos, hence satisfies the Giraud axioms, and these restrict to the $\aleph_1$-compact objects. Here countable colimits of $\aleph_1$-compact objects remain $\aleph_1$-compact, and the latter are closed under finite limits by \Cref{prop:Kappa_Compact_Presheaves_Finite_Limits}. It then suffices to know that the countable Giraud axioms are stable under left exact localizations, which is \Cref{lem:Countable_Giraud_Localization} below.

		$(4) \Rightarrow (1)$: Since $\Ind_{\aleph_1}(\Yy)$ is presentable, by \Cref{thm:Topos_Characterization} it is enough to verify the Giraud axioms for $\Ind_{\aleph_1}(\Yy)$. Every small colimit in $\Ind_{\aleph_1}(\Yy)$ is an $\aleph_1$-filtered colimit of countable colimits of objects of $\Yy$, while finite limits commute with $\aleph_1$-filtered colimits. Thus universality of colimits and disjointness of coproducts propagate from $\Yy$ to $\Ind_{\aleph_1}(\Yy)$. For effectivity, since $\simp\catop$ is countable, the canonical functor
		\[
			\Ind_{\aleph_1}\bigl(\Fun(\simp\catop, \Yy)\bigr) \longrightarrow \Fun(\simp\catop, \Ind_{\aleph_1}(\Yy))
		\]
		is an equivalence. The full subcategory of groupoid objects is defined by finite-limit conditions, so it is likewise generated under $\aleph_1$-filtered colimits by the groupoid objects in $\Yy$. Geometric realization commutes with these filtered colimits, as do the finite limits forming the Čech nerve. Effectivity in $\Yy$ therefore implies effectivity in $\Ind_{\aleph_1}(\Yy)$.
	\end{proof}
	...
	\begin{proof}[Proof of \Cref{thm:Sheaf_Valued_In_Etale}]
		Since the original functor sends the terminal object of $C$ to $A$ and fiber products in $C$ to tensor products, the finite-colimit assertion for its extension will follow from the Künneth formula proved below. We must also show that for every $\aleph_1$-compact $X \in \Shv(C)^{\aleph_1}$ the algebra $\Oo(X)$ is $n$-\'etale over $A$. Our strategy is to show:
		\begin{itemize}
		...
		\]
		Both $A \to \Oo(X)$ and $A \to \Oo(Y_i)$ are $n$-\'etale, so the second-out-of-three property of \Cref{prop:Etale_Properties}(2) shows that each relative map $\Oo(X) \to \Oo(Y_i)$ is $n$-\'etale, hence $n$-suave. By \Cref{prop:Suave_Countable_Limits}(1) the countable limit $\Oo(X) \to \Oo(Y)$ is again $n$-suave, with
		\[
			(\Oo(Y)/\Oo(X))^!_m \simeq \colim_{i \in I\catop} (\Oo(Y_i)/\Oo(X))^!_m \qquad (m \geq n).
		\]
		...
		\]
		We claim that the value at the unit is $B^!_{m-1}$, showing that $B^!_{m-1} \simeq (\Oo(Y)/\Oo(X))^!_{m-1}$. As $m \geq n+1$, this identifies the dualizing objects in every degree at least $n$. Together with the right adjoints to the counits $B^!_m \to \unit$ for $m \geq n+1$, this proves both conditions for $n$-suaveness.
		...
		By descent, this is
		\[
			h_{m-1,!}h^!_{m-1}\unit \simeq \lim_{g\colon X' \to X} g_{m-1,!}g_{m-1}^!\unit \simeq B^!_{m-1}.
		\]
	...
	\end{corollary}
	\begin{proof}
		The ``only if'' direction is stability under base change. Conversely, write $Y'_\bullet$ for the Čech nerve of $Y' \to Y$. Since $Y' \to Y$ is an effective epimorphism, for every $m$ we have
		\[
			\Oo(Y)_m \simeq \lim_{[k] \in \simp} \Oo(Y'_k)_m,
		\]
		and the analogous descent descriptions hold for the relative objects of $f$. If $f'$ is $n$-affine, its relative commutative algebras in all degrees at least $n$ descend through these limits, as do the identifications with their internal module categories, so $f$ is $n$-affine. If $f'$ is $n$-prim, the objects $f'_{m-1,*}(\unit)$ and the units $\unit \to f'_{m-1,*}(\unit)$ form descent data for $m \geq n+1$; their internal right adjoints descend as well, proving that $f$ is $n$-prim. The same argument with $f'_{m-1,\sharp}(\unit)$ and the adjoints to the counits proves the $n$-suave case. Finally, $n$-properness and $n$-étaleness reduce to the preceding cases by applying the same argument to all iterated diagonals. Hence each listed property descends.
	\end{proof}
	...
	\begin{proof}
		...
		So let $i\colon Z \hookrightarrow X$ be an $n$-proper injection with $\unit \iso (\Oo(Z)/\Oo(X))_n$. We prove by induction on $m \geq n$ that $\unit \iso (\Oo(Z)/\Oo(X))_m$; the case $m = n$ is the hypothesis. Suppose $m>n$ and assume the claim for $m - 1$. By assumption on $i$, the map $i_{m-1}^*\colon \unit_X \to (\Oo(Z)/\Oo(X))_m$ has an internal right adjoint $i_{m-1,*}$ in $\Oo(X)_{m+1}$, and by the inductive hypothesis the unit of this adjunction, $\unit \to (\Oo(Z)/\Oo(X))_{m-1}$, is an isomorphism in $\End_{\Oo(X)_{m+1}}(\unit) = \Oo(X)_m$. Hence $i_{m-1}^*$ is fully faithful. On the other hand, idempotence of the target algebra implies that $i_{m-1,*}$ is fully faithful. Thus the adjunction is an equivalence, and $\unit \to (\Oo(Z)/\Oo(X))_m$ is an isomorphism. This completes the induction and the proof.
	\end{proof}
	...
	\begin{proof}
		...
		\]
		is, as an object of $\End_{\Oo(X)_{m+2}}(\unit) = \Oo(X)_{m+1}$, given by $(\Oo(U)/\Oo(X))^!_m$, while the counit of the adjunction is the isomorphism $j_{m-1,!}\colon (\Oo(U)/\Oo(X))^!_m \iso \unit_X$ furnished by the inductive hypothesis. Hence the right adjoint $j^!_m$ is fully faithful. Coidempotence of $(\Oo(U)/\Oo(X))^!_{m+1}$ implies that the counit map $j_{m,!}$ is fully faithful as well, by the dual of the idempotent-algebra argument above. Thus the adjunction $j_{m,!} \dashv j^!_m$ is an equivalence, so $j_{m,!}$ is an isomorphism, completing the induction.
	\end{proof}
	...
	\begin{proposition}\label[proposition]{prop:Complement_Construction}
		...
		\]
		Assume that $n \geq 1$, or that $\Oo(X)_1$ is pointed. Then $A$ is an idempotent algebra, and the $n$-proper injection $\Gest_X(A) \hookrightarrow X$ is the complement of $U$.
	\end{proposition}
	\begin{proof}
	...
	\begin{corollary}\label[corollary]{cor:Complement_Open_Closed}
		If $\Oo(X)_1$ is pointed, then the complement of an open immersion into $X$ is a closed immersion. In particular, this holds for Gestalten over $\Gest(\An_*)$. The converse holds for Gestalten over $\Spec(\S)$. \qed
	\end{corollary}
	...
	\begin{proof}
		...
		\[
			T \times_X \colim_{[m]} Y^{\times_X(m+1)} \simeq \colim_{[m]} (T \times_X Y)^{\times_T(m+1)}.
		\]
		Applying this with $T = A$ and using surjectivity of $f$:
		\[
			A \simeq A \times_X \colim_{[m]} Y^{\times_X(m+1)} \simeq \colim_{[m]} (A \times_X Y)^{\times_A(m+1)},
		\]
		and likewise $B \simeq \colim_{[m]} (B \times_X Y)^{\times_B(m+1)}$. In simplicial degree $m$ the induced map between these Čech nerves identifies with the base change of $f^*\varphi$ along $Y^{\times_X(m+1)} \to Y$, and is therefore an isomorphism. Taking colimits, $\varphi$ induces an isomorphism $A \iso B$ over $X$.
	\end{proof}
	...
	\begin{proof}
		...
		Base change along $U \to X$ also commutes with the simplicial colimit, and therefore
		\[
			U \times_X U \simeq \colim_{[m]} U \times_X Y^{\times_X(m+1)} \simeq \colim_{[m]} Y^{\times_X(m+1)} = U.
		\]
		Thus $U \to X$ is an injection. It follows that the Čech nerves of $Y \to U$ and $Y \to X$ agree, so $Y \to U$ is surjective. Moreover, $Y \to U$ is the base change of the $n$-suave map $Y \to X$, hence is $n$-suave.
	\end{proof}
	...
	\end{proposition}
	\begin{proof}
		(1) $\Leftrightarrow$ (2): Consider the epi-mono factorization $Y \twoheadrightarrow U \hookrightarrow X$ of $f$ from \Cref{prop:Epi_Mono_Factorization_Suave}. The dualizing coalgebra of $U \hookrightarrow X$ is
		\[
			(\Oo(U)/\Oo(X))^!_n \simeq \colim_{[m]} (\Oo(Y^{\times_X(m+1)})/\Oo(X))^!_n.
		\]
		By \Cref{prop:Suave_Etale_Idempotent_Coalgebras}, the injection $U \to X$ is an isomorphism precisely when this coalgebra is the unit. This is condition (2), while $U = X$ is equivalent to surjectivity of $f$.

		(2) $\Leftrightarrow$ (3) is \Cref{lem:Kunneth_Suave}.

		(1) $\Rightarrow$ (4): Surjectivity and descent give
		\[
			\Oo(X)_{n+1} \simeq \lim_{[m] \in \simp} \Oo(Y^{\times_X(m+1)})_{n+1}.
		\]
		A map in this limit is an isomorphism if its pullback to $Y$ is an isomorphism, since all of its further pullbacks to the terms of the Čech nerve are then isomorphisms. Thus $f_n^*$ is conservative.

		(4) $\Rightarrow$ (3): By conservativity it suffices to check the map of (3) after base change to $\Oo(Y)_{n+1}$. After base change it is the corresponding augmented bar construction for the projection $Y \times_X Y \to Y$. This projection is split by the diagonal, so the augmented simplicial object has extra degeneracies and its colimit is the unit.
	\end{proof}
	...
	\begin{proof}
		...
		\[
			\Hom_Y(Gx_1,Gx_2) \xrightarrow{F} \Hom_X(FGx_1,FGx_2) \xrightarrow{G} \Hom_Y(GFGx_1,GFGx_2) \to \Hom_Y(Gx_1,Gx_2),
		\]
		where the last map uses the two structure maps of the retraction $\id_Y \Rightarrow GF \Rightarrow \id_Y$, is the identity. Thus $\Hom_Y(Gx_1,Gx_2)$ is the image of the resulting idempotent on $\Hom_X(FGx_1,FGx_2)$. The identical construction for $Y'$ exhibits $\Hom_{Y'}(G'x_1,G'x_2)$ as the image of the same idempotent, because $F'G' = FG$ and the retraction data on $Y'$ is obtained by applying $i^R$. Hence $H$ induces an isomorphism
		\[
			\Hom_{Y'}(G'x_1,G'x_2) \iso \Hom_Y(Gx_1,Gx_2).
		\]
		As every object of $Y'$ and every object of $Y$ is a retract of an object in the image of $G'$ and $G$, respectively, this proves that $H$ is fully faithful. It is therefore an equivalence.
	\end{proof}
	...
	\begin{corollary}\label[corollary]{cor:Surjective_Affine_Truncation}
		Let $f\colon Y \to X$ be an $n$-prim, eventually suave map of Gestalten. Then the map
		\[
	...
	\begin{proof}
		...
		\emph{Induction step.} Assume the claim for $(m-1)$-suave maps, and let $f$ be $n$-prim and $m$-suave. Let
		\[
			Y' := \Gest_X\bigl((\Oo(Y)/\Oo(X))_{m-2}\bigr)
		\]
		be the $(m-2)$-affine truncation of $Y$. By the base case, $Y \to Y'$ is surjective. Since $f$ is $n$-prim, \Cref{lem:Prim_Relative_Algebras_Countably_Presented} shows that $(\Oo(Y)/\Oo(X))_{m-2}$, and hence the $(m-2)$-affine map $g\colon Y' \to X$, is countably presented. The map $g$ is still $n$-prim: in degrees $k \leq m-2$ its relative pullback maps agree with those of $f$, while in degrees $k>m-2$ the required right adjoints follow from \Cref{prop:Prim_Properties}(1), applied to the countably presented $(m-2)$-affine map $g$. By \Cref{prop:Proper_Properties}(1), the map $g$ is $(m-2)$-proper, and hence $(m-1)$-suave by ambidexterity (\Cref{prop:Ambidexterity_Stefanich_Rings}). The induction hypothesis shows that $g$ is surjective. As $Y \to Y'$ is surjective as well, so is the composite $Y \to X$.
	\end{proof}
	...
	\begin{corollary}\label[corollary]{cor:Descendable_Implies_Surjective}
		Let $f\colon Y \to X$ be a $0$-prim, eventually suave map of Gestalten over $\Spec(\S)$. If $f_{0,*}(\unit) \in \Oo(X)_1$ is descendable, then $f$ is surjective.
	\end{corollary}
	\begin{proof}
	...
	\begin{corollary}\label[corollary]{cor:Surjective_Iff_Descendable}
		Let $X$ be a $0$-prim Gestalt over $\Spec(\S)$, and let $f\colon Y \to X$ be a $0$-prim, eventually suave map. Then $f$ is surjective if and only if $f_{0,*}(\unit)$ is descendable in $\Oo(X)_1$.
	\end{corollary}
	\begin{proof}
	...
	\emph{During the seminar, the contents of \Cref{chap:Affine_Coaffine_Maps_Gestalten,chap:Examples} were discussed in the same talk, which came before \Cref{chap:Injections_Surjections_Gestalten}. The material has been expanded and reorganized into two separate chapters for the sake of clarity of exposition.}
	...
	\begin{lemma}\label[lemma]{lem:Affinization_Preserves_Affineness}
		Let $X$ be a Gestalt, let $m, n \geq 0$, and let $\varphi\colon Z \to W$ be an $m$-affine map between Gestalten over $X$. Then its $n$-affinization over $X$,
		\[
			\varphi^{n\text{-affine}}\colon (Z/X)^{n\text{-affine}} \to (W/X)^{n\text{-affine}},
		\]
		is again $m$-affine.
	\end{lemma}
	\begin{proof}
		Write $Z' := (Z/X)^{n\text{-affine}}$ and $W' := (W/X)^{n\text{-affine}}$, both $n$-affine over $X$ by construction. Thus $\varphi^{n\text{-affine}}\colon Z' \to W'$ is a map between $n$-affine Gestalten over $X$, and is therefore itself $n$-affine by the cancellation property of \Cref{prop:Affineness_Properties}(4).
		...
		\]
		are equivalences in the remaining degrees $m \leq i < n$. In the inverse-limit description of Stefanich algebras from \Cref{cor:Inclusion_n_Affine_A_Algebras}, $n$-affinization leaves the relative algebra data and their transition maps through degree $n$ unchanged. Consequently, in the displayed range the relative structure maps for $\varphi^{n\text{-affine}}$ identify with those for $\varphi$. They are equivalences because $\varphi$ is $m$-affine. This verifies the criterion, so $\varphi^{n\text{-affine}}$ is $m$-affine.
	\end{proof}
	...
	\end{proposition}
	\begin{proof}
		Write $Y' := (Y/X)^{n\text{-affine}}$, and let $g\colon Y'\to X$ be the structure map. By construction, $g$ is $n$-affine and the map $Y \to Y'$ identifies the relative algebra of $Y'$ over $X$ in degree $n$ with that of $Y$ over $X$. By transitivity of relative right adjoints, applying $g_{n,*}$ to the unit map
		\[
			\unit_{\Oo(Y')_{n+1}} \to (\Oo(Y)/\Oo(Y'))_n
		\]
		gives precisely this identification over $X$. Since $g$ is $n$-affine, $g_{n,*}$ is the forgetful functor from a module category and is conservative. The displayed unit is therefore an isomorphism, i.e.\ $Y \to Y'$ is $n$-coaffine.
		...
		\[
			\Hom_{\Gest_{/X}}(T,W)
			\simeq
			\Hom_{\CAlg(\Oo(X)_{n+1})}\bigl(R,(\Oo(T)/\Oo(X))_n\bigr)
		\]
		naturally in $T \in \Gest_{/X}$. Applying this to $T=Z$ and $T=Y$, the map
		\[
			\Hom_{\Gest_{/X}}(Z,W) \to \Hom_{\Gest_{/X}}(Y,W)
		\]
	...
	\begin{proposition}
		...
		Let $n \geq 0$, and let $f\colon Y \to X$ be a map of Gestalten.
		\begin{enumerate}
			\item If $f$ is $(n+1)$-affine and $\Delta_f$ is $n$-prim, then $\Delta_f$ is $n$-affine.
			\item If $\Delta_f$ is $n$-affine and $f$ is $(n+1)$-prim, then $f$ is $(n+1)$-affine.
		\end{enumerate}
	...
	\begin{proof}
		...
		\]
		We first verify that \Cref{cor:Surjective_Affine_Truncation} applies to $f$. Set
		\[
			A:=\Oo(X), \qquad R:=(\Oo(Y)/A)_{n+1}\in\CAlg(A_{n+2}).
		\]
		By \Cref{lem:Prim_Relative_Algebras_Countably_Presented}, the algebra $R$ is countably presented. The K\"unneth formula for the $(n+1)$-prim map $f$ gives
		\[
			(\Oo(Y\times_XY)/A)_{n+1}\simeq R\otimes_{A_{n+2}}R.
		\]
		The relative algebra of $\Delta_f$ in degree $n+1$ is $R$, regarded as an algebra over $R\otimes_{A_{n+2}}R$ through multiplication. Both $R$ and $R\otimes_{A_{n+2}}R$ are countably presented as $A_{n+2}$-algebras, so \Cref{lem:Kappa_Compact_Algebra_Closure_Properties}(4), with $\kappa=\aleph_1$, shows that $R$ is countably presented as an $R\otimes_{A_{n+2}}R$-algebra. Since $\Delta_f$ is $n$-affine and hence $(n+1)$-affine, it follows that $\Delta_f$ is a countably presented $(n+1)$-affine map. Thus $\Delta_f$ is $(n+1)$-prim by \Cref{prop:Prim_Properties}(1). Ambidexterity (\Cref{prop:Ambidexterity_Stefanich_Rings}(1)) now shows that $f$ is $(n+2)$-suave.

		Consequently \Cref{cor:Surjective_Affine_Truncation}, applied with $n+1$ in place of $n$, shows that $h$ is surjective. Moreover, the algebra $R$ is countably presented, so the $(n+1)$-affine map $g$ is countably presented and hence $(n+1)$-proper by \Cref{prop:Proper_Properties}(1). Applying the cancellation statement \Cref{prop:Proper_Properties}(4) to $f=g\circ h$, the $(n+1)$-primness of $f$ implies that $h$ is $(n+1)$-prim.

		By construction, $h$ is $(n+1)$-coaffine. It follows from \Cref{lem:Prim_Coaffine_Diagonal_Coaffine} that $\Delta_h$ is $n$-coaffine, and so it remains to show that $\Delta_h$ is also $n$-affine.
	...
	\begin{example}[Finite sets]
		...
		\]
		for all $n \geq 2$; here we use that $A_n$ is semiadditive, since it is a module over $1\Pr$, which is semiadditive. If $A_1$ is semiadditive, then $I$ is even 0-affine.
	\end{example}
	...
	\begin{example}[Countable animae]
		...
		We first show, by induction on $n$, that it is $0$-proper. It is $0$-étale, hence $1$-proper. Its diagonal is the projection
		\[
			B^n(\Z/2)\times B^{n+1}(\Z/2)\longrightarrow B^{n+1}(\Z/2),
		\]
		which is $0$-proper by the induction hypothesis and base change. The initial case uses that the finite anima $\Z/2$ is $0$-proper over $\Gest(\D(\Q))$. It therefore remains only to show that $B^{n+1}(\Z/2)$ is $0$-prim. In other words, we need to show that the functor
		\[
		...
		We may now use \Cref{prop:Affineness_And_Diagonals}(1) to inductively show that $B^{n+1}(\Z/2)$ is not $n$-affine. First, $B(\Z/2)$ is not $0$-affine: if it were, then the category $\D(\Q)^{B(\Z/2)}$ of rational $\Z/2$-representations would be equivalent to
		\[
			\Mod_{\Q^{B(\Z/2)}}(\D(\Q))\simeq\D(\Q),
		\]
		because $B(\Z/2)$ has trivial rational cohomology. This contradicts the existence of nontrivial rational $\Z/2$-representations.

		Now let $n\geq1$ and assume for contradiction that $B^{n+1}(\Z/2)$ is $n$-affine. Since its diagonal is $(n-1)$-prim, \Cref{prop:Affineness_And_Diagonals}(1), applied with $n-1$, shows that the diagonal is $(n-1)$-affine. Hence the pullback square
		\[
		\begin{tikzcd}
		...
		\end{tikzcd}
		\]
		would imply that $B^n(\Z/2)$ is $(n-1)$-affine, contradicting the induction hypothesis.
	\end{example}
	...
	\begin{proof}
		...
		However, the mapping anima from any object in this image to $k[T] \in \D_{\geq 0}(k[T])$ is contractible. Indeed, because $k[T]$ is $0$-truncated, every $k[T]$-linear map $M \to k[T]$ with $M$ connective is detected on $\pi_0(M)$, and the mapping anima is discrete. If $M \in \D_{\geq 0}(k[T^{\pm 1}])$, the image of such a map in $k[T]$ is divisible by all powers of $T$, hence lies in $\bigcap_{r \geq 0} T^r k[T] = 0$. Similarly, maps from objects of $\D_{\geq 0}(k[T,\frac{1}{1-T}])$ have image in $\bigcap_{r \geq 0} (1-T)^r k[T] = 0$. The full subcategory of objects whose mapping anima to $k[T]$ is contractible is closed under colimits, so $k[T]$ cannot be generated by the displayed image.
	\end{proof}
	...
	\begin{proof}
		...
		\]
		We will now show that the map $* \to \fib(\Pic \to \mu_2)$ is surjective, i.e.\ that any $L \in \Oo(X)^{\times}_1$ such that the swap map $c\colon L \otimes L \to L \otimes L$ is trivial is already locally trivial. Write $A = \Oo(X)$. The action of $\Sigma_n$ on $L^{\otimes n}$ is uniquely trivial for all $n \geq 1$. Indeed, this coherent action is given by a map
		\[
			B\Sigma_n \to B\Aut(L^{\otimes n}) \simeq B\Aut(\unit) = B A_0^{\times}.
		\]
		Since $X$ lies over $\D(\Q)$, the commutative algebra $A_0$ refines to an object of $\CAlg(\D_{\geq 0}(\Q))$. Thus $A_0^{\times}$ is a connective $E_{\infty}$-group whose higher homotopy groups are $\Q$-vector spaces. The induced homomorphism $\Sigma_n \to \pi_0(A_0^{\times})$ is trivial because transpositions act trivially by assumption, and transpositions generate $\Sigma_n$. All remaining obstruction and ambiguity groups are positive-degree group cohomology groups of the finite group $\Sigma_n$ with coefficients in rational vector spaces, hence vanish. Therefore the displayed map is uniquely nullhomotopic.
	...
	\emph{Talk given by Ou. The material from \Cref{sec:Translation,sec:Extension_To_Stacks,sec:6FF_On_Gest} was not treated during the seminar and was added afterwards for completeness.}
	...
	\begin{corollary}[Künneth implies affineness]
		...
		\[
			...
			\D^{(n)}(X\times_SY)
		\]
		is an equivalence. Then $[S]_{\D}$ is $n$-affine for every $S\in C$. Consequently every structure map $[S]_{\D}\to[*]_{\D}$ is $n$-affine.
	\end{corollary}
	\begin{proof}
		...
		For every $S\in C$, the Stefanich ring of $[S]_{\D}$ has $m$-th level $\D^{(m)}(S)$ for $m\geq1$. The equivalences
		\[
			\D^{(m+1)}(S)\simeq \Mod_{\D^{(m)}(S)}(m\Pr)
			\qquad(m\geq n)
		\]
		are precisely the affine comparison maps in degrees $m\geq n$. Hence $[S]_{\D}$ is $n$-affine. Applying the cancellation property of \Cref{prop:Affineness_Properties}(4) to the $n$-affine Gestalten $[S]_{\D}$ and $[*]_{\D}$ proves the final assertion.
	\end{proof}
	...
	\begin{proof}
		...
		(1) Dually, $0$-suaveness of $[f]_{\D}$ asks that each $(f/A)^*_m$ admit a \emph{left} adjoint, subject to the secondary adjointability condition of \Cref{def:N_Suave}. By the reduction above and \Cref{thm:Transmutation} every requirement is automatic except the left adjoint at $m=0$ and the secondary condition at $m=1$. The latter holds because the level-$1$ dualizing object $(B/A)^!_1 = f_{1,\sharp}(\unit)$ is dualizable: the ambidextrous level-$1$ adjoints preserve dualizable objects by \Cref{lem:Linear_Adjoint_Preserves_Dualizable}. As explained immediately after \Cref{def:N_Suave}, dualizability identifies the required right adjoint with the dual of the already available left adjoint. As in~(2), the left adjoint at $m=0$ says that $\unit_Y$ is a \emph{right} adjoint $1$-morphism $[X]\to[Y]$ in $K_{\D,X}$. Via the self-duality $K_{\D,X}\simeq K_{\D,X}\catop$, this is equivalent to saying that $\unit_Y$ is a left adjoint $[Y]\to[X]$, i.e.\ that $\unit_Y\in\Suave_f(Y)$. We conclude that $[f]_{\D}$ is $0$-suave if and only if $f$ is $\D$-suave.
	\end{proof}

	\begin{corollary}[\'Etale and proper maps]\label[corollary]{cor:Dictionary_Etale_Proper}
	...
	Let $\D\colon \Span(C) \to 1\Pr$ be a presentable six-functor formalism. We constructed a pullback-preserving functor
	\[
		[-]_{\D}\colon C \to \Gest
	\]
	fitting in a commutative triangle as follows:
	\[
	\begin{tikzcd}[column sep=huge]
		C\catop \rar[dashed,"{[-]_{\D}}"] \arrow[dr,swap,"{\D^{*,\otimes}}"] & \Gest\catop \dar["{\Oo(-)_1}"] \\
		& \CAlg(1\Pr).
	...
	\begin{proof}
		...
		\[
			\id \xrightarrow{f_{n,!}} f_{n+1,\sharp}f_{n+1}^*
		\]
		...
		On the other hand, we can also apply this reasoning to the right adjoint $g_{n,*}\colon g_{n+1,*}\unit \to \unit$ of $\unit \to g_{n+1,*}\unit$. Applying $f_{n+1,*}f_{n+1}^*$ to it, this becomes $g'_{n,*}\colon h_{n+1,*}\unit \to f_{n+1,*}\unit$, and then $f_{n,\sharp}\colon f_{n+1,*}f_{n+1}^* \to \id$ gives the other composite from $h_{n+1,*}\unit$ to $\unit$. The displayed square identifies these two composites. Under the internal adjunction correspondence they are precisely the two sides of the double Beck--Chevalley transformation, proving that it is an isomorphism in $\Oo(X)_{n+2}$ and hence also on the underlying functors.
	\end{proof}
	...
	\end{proposition}
	\begin{proof}
		First fix a Gestalt $X$, put $A:=\Oo(X)$, and let
		\[
			C_X:=(E_{\mathrm{tr}})_{/X}
		\]
		be the category of truncated, $1$-\'etale and $1$-proper Gestalten over $X$. This category has finite limits, and every morphism in $C_X$ again lies in $E_{\mathrm{tr}}$: the relevant properties are stable under base change and composition, and the cancellation properties for the $1$-\'etale and $1$-proper conditions are part of \Cref{prop:Etale_Properties,prop:Proper_Properties}. Working in a larger universe if necessary, we may therefore apply \Cref{thm:Span_2_Universal} with all three classes equal to $C_X$.

		The relative algebras define a symmetric monoidal functor to $\CAlg(A_3)$; after forgetting the algebra structures, we regard this as a lax symmetric monoidal functor
		\[
		...
		We now assemble these functors. For $Y\to X$ in $(E_{\mathrm{tr}})_{/X}$ and $T\to X$ in $\Gest_{/X}$, define
		\[
		...
		\]
		The construction of $D_T$ is natural under pullback, since both the relative algebra functor and the extension supplied by \Cref{thm:Span_2_Universal} are natural. Hence these functors are natural in $T$, and we obtain functors
		\[
			\D_{/X}\colon\Span((E_{\mathrm{tr}})_{/X})\longrightarrow\Fun(\Gest_{/X}\catop,1\Pr).
		\]
		The contravariant coefficient functor $\Oo(-)_1\colon\Gest\catop\to\CAlg(1\Pr)$ and the lax symmetric monoidal structures of the $D_T$ supply natural external products
		\[
			\D_{/X}(Y)(T)\otimes\D_{/X'}(Y')(T')
			\longrightarrow
			\D_{/(X\times X')}(Y\times Y')(T\times T').
		\]
		The naturality of the Künneth equivalences under pullback makes these products compatible with the span actions and with the following compatibility equivalences.

		Their contravariant compatibility in $X$ is the canonical base-change identification
		\[
			T'\times_{X'}(X'\times_XY)\simeq T'\times_XY.
		\]
		If $e\colon X'\to X$ lies in $E_{\mathrm{tr}}$, the covariant compatibility required in \Cref{thm:Slicewise_Reconstruction} is checked objectwise by the same pullback identification:
		\[
			\D_{/X}(e_{\sharp}Y)(T)
			=\Oo(T\times_XY)_1
			\simeq
			...
			=(e_*\D_{/X'}(Y))(T).
		\]
		The symmetric monoidal reconstruction result \Cref{cor:Slicewise_Reconstruction_Monoidal} therefore assembles the $D_X$ into a lax symmetric monoidal functor
		\[
			\Oo(-)_1\colon\Span(\Gest,E_{\mathrm{tr}})\longrightarrow1\Pr.
		\]
	...
	\begin{remark}
		...
		For the non-monoidal argument below we only need projective morphisms between quasiprojective schemes. In this setting, \citet[Corollary~3.11]{Aoki_PhD_Thesis} proves projective right adjoints and projective base change for functors valued in an arbitrary stable presentable $2$-category. No non-monoidal assertion for all proper morphisms is needed.
	\end{remark}
	...
	\begin{proof}
		...
		It remains to prove that the image of $p_1^*$ generates $\SH^{(2)}(X)$. The corepresentables associated to objects $Y\to X$ of $\Sch_{/X}$ generate $\SH^{(2)}(X)$. A finite affine open cover of $Y$ and the successive closed--open recollements express $[Y]$ in the localizing subcategory generated by the corepresentables of affine opens and their finite intersections. We may therefore assume that $Y$ is affine. Choose a closed immersion $Y\hookrightarrow\bbA^n_{\Z}$. Since $X$ is separated over $S$, the graph of $Y\to X$ is a closed immersion; composing it with the base change of $Y\hookrightarrow\bbA^n_{\Z}$ gives a closed immersion
		\[
	...
	The result of the previous section shows that the Gestalt $[\Spec(\Z)]_{\SH}$ is the 2-affine Gestalt associated to the presentably symmetric monoidal 2-category $\SH^{(2)}(S)$. We now discuss the universal property established by \citet{Aoki_PhD_Thesis}.

	In this section $S$ denotes a noetherian finite-dimensional base scheme, $\Sch_S$ denotes the category of separated finite type $S$-schemes, and $\operatorname{QProj}_S\subseteq\Sch_S$ denotes the full subcategory of quasiprojective schemes. Aoki proves the theorem using $\operatorname{QProj}_S$, where proper morphisms are projective. His corrected thesis, \citet[Remark~1.3]{Aoki_PhD_Thesis}, states that replacing $\operatorname{QProj}_S$ by $\Sch_S$ gives the same universal property. We use the quasiprojective presentation in the proof. For the rest of this chapter one may keep $S=\Spec(\Z)$ in mind.
	...
	\[
		F\colon\operatorname{QProj}_S\catop\longrightarrow\Cc
	\]
	...
	\begin{lemma}
		...
		\[
			F\colon\operatorname{QProj}_S\catop\longrightarrow\Cc
		\]
		be motivic. Then $F$ is biadjointable with respect to open immersions and projective maps. Consequently, by \Cref{thm:Span_2_Universal}, it extends canonically to a symmetric monoidal $2$-functor
		\[
			\SpanTwoP{\operatorname{QProj}_S}{\operatorname{QProj}_S}{\mathrm{open}}{\mathrm{projective}}
			\longrightarrow\Cc.
		\]
	\end{lemma}
	\begin{proof}
		The motivic axioms already give the required left adjoints and Beck--Chevalley equivalences for open immersions, since open immersions are smooth. For projective maps, \citet[Corollary~3.11]{Aoki_PhD_Thesis} gives right adjoints and projective base change in every stable presentable $2$-category. Aoki's complementary closed-immersion argument gives the Beck--Chevalley equivalence for a square whose horizontal maps are open and whose vertical maps are projective. This is precisely the double Beck--Chevalley condition required in \Cref{def:Biadjointable_Functor}.

		It remains to explain the projection formulas. They are a formal consequence of symmetric monoidality together with the relevant Beck--Chevalley equivalence. We spell this out for a projective map $p\colon X\to Y$; the argument for open immersions and left adjoints is the same, with left adjoints in place of right adjoints. Consider the pullback square
		\[
		\begin{tikzcd}
		...
		Thus $F$ is biadjointable with respect to open immersions and projective maps, and the final assertion is precisely \Cref{thm:Span_2_Universal}.
	\end{proof}

	We will also use the following concrete form of the universal property of the kernel category. Write
	\[
		\mathcal P_S:=\Fun(\Span(\operatorname{QProj}_S),1\Pr_{\st})
	\]
	with its Day convolution symmetric monoidal structure. Aoki's quasiprojective construction, compared with the separated finite type construction in \citet[Remark~1.3]{Aoki_PhD_Thesis}, gives an equivalence
	\[
		\SH^{(2)}(S)=\bbK_{\SH,S}
		\simeq
		\Mod_{\SH}(\mathcal P_S).
	...
	\begin{lemma}
		...
		\begin{enumerate}
			...
			\[
				T\colon\Span(\operatorname{QProj}_S)\longrightarrow\Cc;
			\]
			\item a morphism of commutative algebra objects in $\Fun(\Span(\operatorname{QProj}_S),1\Pr_{\st})$
			\[
	...
	\begin{proof}
		...
		\]
		of commutative algebra objects in $\Fun(\Span(\operatorname{QProj}_S),1\Pr_{\st})$. Finally, $\SH^{(2)}(S)$ is generated under colimits by the free $\SH$-modules on representables, which are the corepresentables $[X]$; this gives both the formula $H([X])\simeq T(X)$ and the assertion about natural transformations.
	\end{proof}
	...
	\begin{theorem}[{\citet[Theorem~A]{Aoki_PhD_Thesis}}]
		...
		\[
			[-]\colon\operatorname{QProj}_S\catop\longrightarrow\SH^{(2)}(S),
		...
		\[
			...
			\simeq
			\Fun^{\otimes}_{\mathrm{mot}}\bigl(\operatorname{QProj}_S\catop,\Cc\bigr).
		\]
		The same statement holds with $\operatorname{QProj}_S$ replaced throughout by $\Sch_S$.
	\end{theorem}

	\begin{proof}
		Let $\Mot^{(2)}(S)$ denote the universal stable presentably symmetric monoidal $2$-category equipped with a motivic functor
		\[
			[-]_{\Mot}\colon\operatorname{QProj}_S\catop\longrightarrow\Mot^{(2)}(S).
		\]
		...
		\]
		under $\operatorname{QProj}_S\catop$. We have to construct an inverse.
		...
		\[
			T\colon\Span(\operatorname{QProj}_S)\longrightarrow\Mot^{(2)}(S).
		\]
		...
		\[
			...
			\qquad
			D\colon\Span(\operatorname{QProj}_S)\longrightarrow1\Pr_{\st}.
		\]
		...
		It remains only to check that $F$ and $G$ are inverse equivalences. The composite $G F$ is a symmetric monoidal colimit-preserving endofunctor of $\Mot^{(2)}(S)$ whose restriction to $\operatorname{QProj}_S\catop$ is the identity. By the universal property defining $\Mot^{(2)}(S)$, this identifies $G F$ with the identity. For the other composite, use the last assertion of \Cref{lem:Maps_Out_Of_SH2}. The endofunctor $F G$ of $\SH^{(2)}(S)$ induces the identity on the span-valued functor of corepresentables, and its induced endomorphism of the coefficient system $\SH$ is the identity by Drew--Gallauer's initiality. Thus the two pieces of data attached to $F G$ agree with those attached to the identity functor of $\SH^{(2)}(S)$, so $F G\simeq\id$. This proves that $F$ is an equivalence, which is precisely the asserted universal property of $\SH^{(2)}(S)$ for quasiprojective schemes. The separated finite type version is \citet[Remark~1.3]{Aoki_PhD_Thesis}.
	\end{proof}

	\begin{remark}
		The use of quasiprojective schemes in the proof is essential to the stated projective-base-change input. The extension of the universal property to separated finite type schemes is a separate result, recorded in \citet[Remark~1.3]{Aoki_PhD_Thesis}; it does not require a non-monoidal proper-base-change theorem in the generality excluded by \Cref{rmk:Nonmonoidal_ACD_Proper}.
	\end{remark}
	...
	\[
		\RingGest_{\mot}(\Cc)
	\]
	denote the category of sutured, homologically trivial, and smooth ring Gestalten in \(\Gg_{\Cc}\); morphisms are morphisms of ring Gestalten. Such a ring Gestalt is automatically local: the open and closed maps in \Cref{def:Sutured_Ring_Gestalt} are $1$-étale and $1$-affine, so \Cref{lem:Recollement_Gives_Cover} applies.
	...
	\begin{theorem}[Aoki's ring-Gestalt theorem]
		...
		\]
		Equivalently, the stable presentably symmetric monoidal $2$-category $\SH^{(2)}(\Spec(\Z))$ is freely generated by a homologically trivial, smooth, sutured ring Gestalt.
	\end{theorem}
	\begin{proof}
		...
		\[
			X\longmapsto H([X])
		\]
		is motivic, where $[X]$ denotes the corepresentable object of $\SH^{(2)}(\Spec(\Z))$. Evaluating at $\bbA^1$ therefore gives a ring Gestalt with exactly the properties isolated above. The localization axiom for the decomposition $\bbA^1=\bbG_m\cup\{0\}$ gives suturedness, and homotopy invariance gives homological triviality. The smooth axiom gives weak suaveness of the projection $H(R_{\SH})\to*$, while the Tate axiom gives stability; by \Cref{prop:Aoki_Weakly_Suave_Stable_Smooth}, this is the same as smoothness. Locality then follows from suturedness by \Cref{lem:Recollement_Gives_Cover}.

		Conversely, let $R$ be a sutured, homologically trivial, smooth ring Gestalt over $\Gest(\Cc)$. By the geometric clauses in the definition of suturedness and \Cref{lem:Recollement_Gives_Cover}, $R$ is local. Hence \Cref{prop:Transmutation_Determined_By_Affine_Line} gives the transmutation
		\[
	...
	For a Gestalt $X$, let $\RingGest_{\mot}(X)$ denote the category of sutured, homologically trivial, and smooth ring Gestalten over $X$, i.e.\ such ring Gestalten in $\Gest^{1\text{-}\et}_{/X}$ whose affine-line object is \(1\)-affine over \(X\). If $X=\Gest(\Cc)$, this recovers the notation $\RingGest_{\mot}(\Cc)$ above. Locality is automatic by suturedness, as above.
	...
	\begin{example}[Fourier]\label[example]{ex:Fourier_Circle}
		...
		\[
			\begin{aligned}
				C^{\infty}(S^1)&\iso \{\text{sequences } (a_n)_n \text{ of rapid decay}\},\\
				\mathcal{D}'(S^1)&\iso \{\text{sequences } (a_n)_n \text{ of polynomial growth}\},
			\end{aligned}
		\]
		where $\mathcal{D}'(S^1)$ is the space of distributions on $S^1$, namely the continuous strong dual of the nuclear Fréchet space $C^{\infty}(S^1)$; the Hilbert space isomorphism is recovered by passing to completions.
	...
	\begin{example}[The multiplicative group]\label[example]{ex:Gm_Fourier}
		...
		\]
		with inverse $N \mapsto p_!(q^*N \otimes m^*\Ll)$. Here $q_!$ plays the role of integration over $*/\bbG_m$, while $p^*M \otimes m^*\Ll^{-1}$ is the analogue of $f(t)t^{-n}$. Note that $q$ is proper in the quasicoherent six-functor formalism, so $q_! \simeq q_*$, while $p$ is étale, so $p_! \simeq p_\sharp$.
	...
	\begin{example}[Mukai]\label[example]{ex:Mukai}
		...
		\[
			...
			\end{tikzcd}
		\]
		induces a Fourier equivalence $\D_{\qc}(A) \simeq \D_{\qc}(A^*)$, $M \mapsto q_*(p^*M \otimes \mathcal{P})$; see \citet{FourierMukai2006Huybrechts}. In contrast to the previous examples, here one does \emph{not} pass to classifying spaces.
	\end{example}
	...
	\begin{example}[Deligne]\label[example]{ex:Deligne_Fourier}
		...
		\[
			...
			\end{tikzcd}
		\]
		(with $m$ the evaluation pairing) induces an equivalence $\D_{\et}(V, \overline{\Q}_{\ell}) \simeq \D_{\et}(V^*, \overline{\Q}_{\ell})$, $M \mapsto q_!(p^*M \otimes m^*\Ll_{\psi})$.
	\end{example}
	...
	\begin{remark}
		...
		\[
			(\Oo(M^*_{m-k})/\Oo(X))_m \otimes (\Oo(M_k)/\Oo(X))_m \iso (\Oo(M^*_{m-k} \times M_k)/\Oo(X))_m
		\]
	...
	\begin{proof}
		...
		\]
		using the symmetry of the tensor pairing. Under these identifications, the biduality map corresponds to the map $\Hom(N, M) \to \Hom(M^*, N^*)$ given by contravariant functoriality of $(-)^*$. As $(-)^*$ is an anti-self-equivalence of the union (\Cref{thm:Cartier_Duality}), this map of animae is an equivalence. The Yoneda lemma therefore identifies $M \to (M^*)^*$ as an isomorphism.
	\end{proof}
	...
	\]
	the monoid of $\aleph_1$-compact objects of $\Oo(X')_n$ under tensor product. Note that we have an inclusion $(\GL_1)_n \subseteq (\bbA^1)_n$ of commutative monoids. We then have the following result:
	...
	\begin{theorem}\label[theorem]{thm:Internal_Hom_GL1}
		...
		\]
		is representable by the $n$-affine Gestalt $\Gest_X\bigl((\Oo(G)/A)^!_n\bigr)$. More generally, for every $G \in \CMon(\Gest^{n\text{-suave}}_{/X})$ one has
		\[
	...
	\begin{construction}\label[construction]{con:Suave_Dualizing_Symmetric_Monoidal}
		Put $C:=\Gest^{n\text{-suave}}_{/X}$. By \Cref{prop:Suave_Properties}, this full subcategory of $\Gest_{/X}$ is closed under finite products. We construct a symmetric monoidal functor
		\[
			(C, \times_X) \longrightarrow A_{n+1},
			\qquad
			Y \longmapsto (\Oo(Y)/A)^!_n.
		\]

		We first record the coherent mate construction that we use. Let $K$ be an $(\infty,2)$-category with a chosen object $\unit$, and write $\operatorname{RAdj}_{\unit/}(K)$ for the category of right-adjoint $1$-morphisms $u\colon\unit\to B$, with commutative squares that are the identity on $\unit$ as morphisms. Haugseng's mate equivalence and its compatibility with composition \cite[Theorem~1.1, Corollary~4.9, and Remark~4.11]{Haugseng_Lax_Transformations} give a functor
		\[
			\Theta_K\colon \operatorname{RAdj}_{\unit/}(K)\catop\longrightarrow\End_K(\unit),
			\qquad
			(u\colon\unit\to B)\longmapsto u^L u.
		\]
		On a commutative square from $u$ to $v$, the functor $\Theta_K$ takes the left mate and then whisks it with the appropriate right adjoint. Thus this formula specifies a functor, including its higher coherences, rather than only its values on objects and morphisms.

		We now make the symmetric monoidality of this construction formal. Encode a symmetric monoidal $(\infty,2)$-category $K$ by the finite-product-preserving functor \cite[Appendix~C]{BachmannHoyois2021Norms}
		\[
			K^\otimes\colon\Span(\Fin)\longrightarrow\Cat_{(\infty,2)},
			\qquad I\longmapsto K^I.
		\]
		The assignments
		\[
			K\longmapsto\operatorname{RAdj}_{\unit/}(K)\catop
			\qquad\text{and}\qquad
			K\longmapsto\End_K(\unit)
		\]
		are functorial under unit-preserving $(\infty,2)$-functors and preserve finite products. Every structure functor of $K^\otimes$ preserves the tuple of unit objects, and the functors $\Theta_K$ are natural in $K$ by Haugseng's result. Applying these two assignments and $\Theta$ to the entire functor $K^\otimes$, not separately to its individual operations, therefore gives a natural transformation between finite-product-preserving functors $\Span(\Fin)\to\Cat$. Equivalently, $\Theta_K$ is a symmetric monoidal functor. This supplies in one construction the comparisons for every operation, their symmetric-group actions, and all compatibilities among them.

		Write $\CAlg(K)^{\mathrm{radj}}$ for the full subcategory of commutative algebras whose unit is a right adjoint. It is closed under tensor products. The unit-arrow construction is natural in $K$ and preserves finite products, so applying it to the entire $\Span(\Fin)$-diagram gives a morphism of finite-product-preserving functors and hence a symmetric monoidal functor
		\[
			\eta\colon\CAlg(K)^{\mathrm{radj}}\longrightarrow\operatorname{RAdj}_{\unit/}(K),
			\qquad B\longmapsto(\unit\to B).
		\]

		Take $K$ to be the underlying symmetric monoidal $(\infty,2)$-category of $A_{n+2}$. The lax symmetric monoidal right adjoint
		\[
			F_X:=(\Oo(-)/A)_{n+1}\colon
			C\catop\longrightarrow\CAlg(K)
		\]
		to the $(n+1)$-affine inclusion lands in $\CAlg(K)^{\mathrm{radj}}$: for $f\colon Y\to X$, the unit of $F_X(Y)$ is the relative pullback
		\[
			\eta_Y=(f/A)^*_n\colon\unit\longrightarrow(\Oo(Y)/A)_{n+1}.
		\]
		It admits the left adjoint $(f/A)_{n,\sharp}$ because $f$ is $n$-suave. The composite of $F_X$ with the unit-arrow functor is lax symmetric monoidal. Passing to opposites and composing with the symmetric monoidal functor $\Theta_K$ therefore gives an oplax symmetric monoidal functor
		\[
			D_X\colon C\longrightarrow\End_{A_{n+2}}(\unit)\simeq A_{n+1}.
		\]
		Here the structural equivalence $\End_{A_{n+2}}(\unit)\simeq A_{n+1}$ of the Stefanich ring $A$ is symmetric monoidal. Under it, the value at $Y$ is
		\[
			D_X(Y)=(f/A)_{n,\sharp}(f/A)^*_n\unit=(\Oo(Y)/A)^!_n.
		\]
		For orientation, if $r\colon Y\to Z$ is a morphism over $X$, naturality of $\eta$ gives $\eta_Y\simeq(r/A)^*_n\eta_Z$. Its coherent left mate is
		\[
			(f/A)_{n,\sharp}(r/A)^*_n\longrightarrow(g/A)_{n,\sharp},
		\]
		where $g\colon Z\to X$ is the structure map. Whiskering with $\eta_Z$ gives $D_X(Y)\to D_X(Z)$. This describes the resulting map, but its functoriality, including all higher coherences, comes from the mate equivalence above.

		It remains to show that the oplax structure maps are isomorphisms. We first identify the already constructed binary comparison, without using this identification to define its coherences. Write
		\[
			\mu_{Y,Z}\colon F_X(Y)\otimes F_X(Z)\longrightarrow F_X(Y\times_XZ)
		\]
		for the lax product map. The binary oplax comparison is the composite
		\begin{align*}
			\eta_{Y\times_XZ}^L\eta_{Y\times_XZ}
			&\simeq \eta_{Y\times_XZ}^L\mu_{Y,Z}(\eta_Y\otimes\eta_Z)\\
			&\longrightarrow (\eta_Y^L\otimes\eta_Z^L)(\eta_Y\otimes\eta_Z)\\
			&\simeq D_X(Y)\otimes_{A_{n+1}}D_X(Z),
		\end{align*}
		where the middle map is the left mate of the square $\mu_{Y,Z}(\eta_Y\otimes\eta_Z)\simeq\eta_{Y\times_XZ}$. The preceding $\Span(\Fin)$-construction is what makes these maps coherent as $Y$ and $Z$ and the operation vary.

		Let $p\colon Y\times_XZ\to Y$ be the base change of $g$, and put $h=fp$. Compatibility of mates with composition identifies this comparison with the pasted base-change and projection maps
		\begin{align*}
			D_X(Y\times_XZ)
			&=h_{n,\sharp}(\unit)\\
			&\simeq f_{n,\sharp}p_{n,\sharp}(\unit)\\
			&\simeq f_{n,\sharp}f_n^*D_X(Z)\\
			&\simeq D_X(Y)\otimes_{A_{n+1}}D_X(Z).
		\end{align*}
		Here the second isomorphism is suave base change, and the third is the suave projection formula. This is precisely the Künneth isomorphism of \Cref{lem:Kunneth_Suave}, not merely some isomorphism between the same two objects. The unit comparison is an isomorphism since $D_X(X)=\unit$. Hence $D_X$ is symmetric monoidal.

		Passing to commutative algebra objects now gives
		\[
			\CMon(C)
			\simeq
			\CAlg(C,\times_X)
			...
			\CAlg(A_{n+1}),
		\]
		which endows $(\Oo(G)/A)^!_n$ with the commutative algebra structure induced by the commutative monoid structure on $G$.
	\end{construction}
	...
	\begin{proposition}\label[proposition]{prop:Cartier_Dual_Suave_Group}
		...
		\[
			G^*_n = \Hh om_{\CGrp(\Gest_{/X})}(G, (\GL_1)_n) = \Gest_X\bigl((\Oo(G)/A)^!_n\bigr).
		\]
	\end{proposition}
	...
	\begin{example}\label[example]{ex:Z_Gm_Cartier}
		...
		\[
			R = \bigoplus_{*} R \to \bigoplus_{\Z} R, \qquad \bigoplus_{\Z} R \otimes \bigoplus_{\Z} R \iso \bigoplus_{\Z^2} R \to \bigoplus_{\Z} R,
		\]
		so that $\Oo(\bbG_{m,R})_0 = R[t^{\pm 1}]$, as expected.
	\end{example}

	\begin{example}\label[example]{ex:Finite_Flat_Cartier}
		Let $G$ be a finite flat group scheme over $R$. By Raynaud's theorem, the structural morphism $G\to\Spec(R)$ is a finite local complete intersection, hence $G$ is $0$-suave. Taking $n = 0$ and using that $\Oo(G)$ is finite projective,
		\[
			(\Oo(G)/R)^!_0 = \Oo(G)^* = \Oo(G^*),
		\]
	...
	\begin{proof}
		...
		\[
			\Hh om(\Z, \tau_{\leq 0}\GL_1) \simeq \tau_{\leq 0}\bbG_m,
		\]
	...
	\]
	corresponding to the ring Gestalt $\underline{\R}$ over $\Gest(\D(\S))$. The pullback of the motivic affine line is
	\[
		[\bbA^1_{\Z}]_{\SH}\times_{[\Spec(\Z)]_{\SH}}\Gest(\D(\S))
		\simeq
		\underline{\R}=\Gest(\Shv(\R,\D(\S))).
	\]
	Under this realization, the torsor $\widetilde{X} \to X$ yields an $\underline{\R}^{\times}$-torsor over $\Gest(\D(\S))$. After removing a trivial $\{\pm 1\}$-torsor, this is the $\underline{\R}_{> 0}$-torsor $\Gest(W) \to \Gest(\D(\S))$ of Witt vectors discussed by \citet{Scholze_Wild_Betti}.

	\chapter{The proof of Cartier duality}
	\label[chapter]{chap:Cartier_Duality_Proof}

	\emph{Talk given by Sil.}

	In this chapter we prove the Cartier duality theorem for Gestalten (\Cref{thm:Cartier_Duality}), whose statement was discussed in the previous chapter.

	\section{Recollections}
	\label[section]{sec:Cartier_Proof_Recollections}

	Throughout, fix a Gestalt $X = \Gest(A)$ as base.

	\begin{definition}\label[definition]{def:GL1_A1_Levels}
		Recall that $(\GL_1)_n$ is the Gestalt over $X$ defined by
		\[
			(\GL_1)_n := \Gest_X(\S \otimes \Oo(X)_n),
		\]
		where $\S$ is the free commutative group on a single generator. In light of the adjunction $\CMon(\An) \rightleftarrows \CAlg(\Oo(X)_{n+1})$, the Gestalt $(\GL_1)_n$ represents the functor
		\[
			\Gest_{/X}\catop \to \An, \qquad Y \mapsto \Oo(Y)^{\times}_n.
		\]
		Since this canonically lifts to a functor valued in $\CGrp(\An)$, the Gestalt $(\GL_1)_n$ is canonically a commutative group in $\Gest_{/X}$. We similarly define
		\[
			(\bbA^1)_n := \Gest_X(\Fin^{\simeq} \otimes \Oo(X)_n),
		\]
		which represents the functor $Y \mapsto \Oo(Y)^{\simeq, \aleph_1}_n$ and is canonically a commutative monoid in $\Gest_{/X}$.
	\end{definition}

	\begin{lemma}\label[lemma]{lem:Loops_GL1_Levels}
		There are isomorphisms of commutative groups $\Omega(\GL_1)_n \simeq (\GL_1)_{n-1}$, so that the sequence $((\GL_1)_0, (\GL_1)_1, (\GL_1)_2, \dots)$ assembles into a spectrum object $\GL_1 \in \Sp(\Gest_{/X})$.
	\end{lemma}
	\begin{proof}
		Naturally in $Y$ we have
		\[
			\Omega \Oo(Y)^{\times}_n = \Aut_{\Oo(Y)_n}(\unit) = \End_{\Oo(Y)_n}(\unit)^{\times} = (\Oo(Y)_{n-1})^{\times},
		\]
		which gives the claim.
	\end{proof}

	The goal of this talk is to prove the Cartier duality theorem of Scholze and Stefanich (\Cref{thm:Cartier_Duality}). Recall its content: for $M \in \Sp(\Gest^{n\text{-}\et}_{/X})$, the functor
	\[
		\Sp(\Gest_{/X})\catop \to \An, \qquad N \mapsto \Hom(N \otimes M, \GL_1)
	\]
	is representable by $M^* = \Hh om(M, \GL_1) \in \Sp(\Gest^{n\text{-prop}}_{/X})$; the assignment $M \mapsto M^*$ induces an equivalence
	\[
		\Sp(\Gest^{n\text{-}\et}_{/X})\catop \simeq \Sp(\Gest^{n\text{-prop}}_{/X});
	\]
	and for all $m, k \geq 0$ with $m + k \geq n$ there is an equivalence
	\[
		(\Oo(M^*_k)/\Oo(X))_{m+k} \simeq (\Oo(M_m)/\Oo(X))^!_{m+k},
	\]
	induced by a form of the Fourier--Makkai transform.

	\section{The key computation}
	\label[section]{sec:Cartier_Proof_Key_Computation}

	The heart of the proof is the following computation, which refines \Cref{thm:Internal_Hom_GL1} to a statement over the slice.

	\begin{theorem}\label[theorem]{thm:Cartier_Key_Computation}
		Let $X = \Gest(A)$ and let $G \in \CGrp(\Gest^{n\text{-suave}}_{/X})$. Then the functor
		\[
			\Gest_{/X}\catop \to \An, \qquad Y \mapsto \Hom_{\CGrp(\Gest_{/Y})}(G \times_X Y, (\GL_1)_n)
		\]
		is representable by the $n$-affine Gestalt
		\[
			\Gest_X\bigl((\Oo(G)/A)^!_n\bigr)
		\]
		over $X$. The analogous statement holds with $(\GL_1)_n$ replaced by $(\bbA^1)_n$ and $G \in \CMon(\Gest^{n\text{-suave}}_{/X})$.
	\end{theorem}

	\begin{remark}\label[remark]{rem:Suave_Dualizing_Monoidal}
		By \Cref{con:Suave_Dualizing_Symmetric_Monoidal}, the functor
		\[
			(\Oo(-)/A)^!_n \colon \Gest^{n\text{-suave}}_{/X} \to A_{n+1}
		\]
		is canonically symmetric monoidal for the cartesian structure on the source. In particular, for $G \in \CMon(\Gest^{n\text{-suave}}_{/X})$ the object $(\Oo(G)/A)^!_n$ acquires a commutative algebra structure in $A_{n+1}$.
	\end{remark}

	\begin{definition}\label[definition]{def:Internal_Hom_GL1}
		We write
		\[
			\Hh om(G, (\GL_1)_n) := \Gest_X\bigl((\Oo(G)/A)^!_n\bigr)
		\]
		for the representing object of \Cref{thm:Cartier_Key_Computation}. It is canonically a commutative group, and as such represents the functor $G' \mapsto \Hom(G \otimes G', (\GL_1)_n)$; in other words, $\Hh om(G, (\GL_1)_n)$ is the internal hom of commutative groups in $\Gest_{/X}$.
	\end{definition}

	\begin{corollary}\label[corollary]{cor:Eventually_Suave_Dual}
		If $G \in \CGrp(\Gest_{/X})$ is eventually suave (i.e.\ $n$-suave for some $n$), then $\Hh om(G, (\GL_1)_n)$ exists for all $n$. Likewise, if $M \in \Sp(\Gest_{/X})$ is such that each $M_n$ is eventually suave, then
		\[
			M^* = \Hh om(M, \GL_1)
		\]
		exists.
	\end{corollary}
	\begin{proof}
		Assume first that $G$ is $n$-suave. Then $\Hh om(G, (\GL_1)_m)$ exists for all $m \geq n$ by \Cref{thm:Cartier_Key_Computation}, and by taking loops we obtain the claim for $m < n$ as well. For the spectral statement, use that
		\[
			\tau_{\geq 0}\Hh om(M, \GL_1) = \lim_n \Hh om(M_n, (\GL_1)_n). \qedhere
		\]
	\end{proof}

	\begin{proof}[Proof of \Cref{thm:Cartier_Key_Computation}]
		The $(\bbA^1)_n$-statement implies the $(\GL_1)_n$-statement by passing to units, so we prove the former.

		Let first $H$ be an $n$-suave Gestalt over $X$. For a map $f\colon Y \to X$ we compute
		\[
			\Hom_{\Gest_{/Y}}(H \times_X Y, (\bbA^1)_n) \simeq \Oo(H \times_X Y)^{\simeq, \aleph_1}_n \simeq \Hom_{\Oo(Y)_{n+1}}\bigl(\unit, (\Oo(H \times_X Y)/\Oo(Y))_n\bigr) \simeq \Hom\bigl((\Oo(H \times_X Y)/\Oo(Y))^!_n, \unit\bigr).
		\]
		Recall that the assignment $(\Oo(-)/\Oo(-))^!_n$ commutes with base change along $n$-suave maps, and so
		\[
			\Hom\bigl((\Oo(H \times_X Y)/\Oo(Y))^!_n, \unit\bigr) = \Hom\bigl(f_n^*(\Oo(H)/\Oo(X))^!_n, \unit\bigr) = \Hom\bigl((\Oo(H)/\Oo(X))^!_n, (\Oo(Y)/\Oo(X))_n\bigr).
		\]
		If now $H = G \in \CMon(\Gest^{n\text{-suave}}_{/X})$ is a commutative monoid, one deduces
		\[
			\Hom_{\CMon(\Gest_{/Y})}(G \times_X Y, (\bbA^1)_n) \simeq \Hom_{\CAlg(\Oo(X)_{n+1})}\bigl((\Oo(G)/\Oo(X))^!_n, (\Oo(Y)/\Oo(X))_n\bigr),
		\]
		which is exactly representability by $\Gest_X\bigl((\Oo(G)/A)^!_n\bigr)$.
	\end{proof}

	We have thus constructed the spectral dual out of the non-spectral duals. It is not yet clear how to describe this spectral dual directly. We now show that the limit computing the spectral dual can be understood stage by stage, with the various stages iteratively fixing the $n$-affinizations of the spectral dual.

	\begin{theorem}\label[theorem]{thm:Iterated_Affinization}
		Let $X = \Gest(A)$ and $M \in \Sp(\Gest_{/X})$, and assume that for all $k \geq n$ the map $* \to M_{k+1}$ is $k$-suave. (Note that this implies that each $M_k$ is $k$-suave.) Then $M^*_0$ is $n$-prim, and the natural map
		\[
			M^*_0 \to \Hh om(M_n, (\GL_1)_n) = \Gest_X\bigl((\Oo(M_n)/A)^!_n\bigr)
		\]
		exhibits its target as the $n$-affinization of its source.
	\end{theorem}

	This follows from the following lemma.

	\begin{lemma}\label[lemma]{lem:Affinization_Loop}
		Let $G \in \CGrp(\Gest_{/X})$ and assume that $G$ is $(n+1)$-suave and that the unit $i\colon * \to G$ is $n$-suave, so that $G' := \Omega G$ is $n$-suave. Then $\Hh om(G, (\GL_1)_{n+1})$ is $n$-prim, and the map
		\[
			\Hh om(G, (\GL_1)_{n+1}) \xrightarrow{\Omega} \Hh om(G', (\GL_1)_n)
		\]
		exhibits its target as an $n$-affinization of its source.
	\end{lemma}
	\begin{proof}
		The target is the $n$-affine Gestalt $\Gest_X\bigl((\Oo(G')/A)^!_n\bigr)$ and the source is the $(n+1)$-affine Gestalt $\Gest_X\bigl((\Oo(G)/A)^!_{n+1}\bigr)$. We must show the following two statements.
		\begin{enumerate}
			\item The map $\unit \to (\Oo(G)/A)^!_{n+1}$ in $A_{n+2}$ admits a right adjoint.
			\item The map $(\Oo(G')/A)^!_n \to \End_{(\Oo(G)/A)^!_{n+1}}(\unit)$ is an isomorphism.
		\end{enumerate}
		For (1), the map $\unit \to (\Oo(G)/A)^!_{n+1}$ is obtained by applying $f_{n+1,\sharp}$ to the map $i_{n,!}\colon (\Oo(*)/\Oo(G))^!_{n+1} \to \unit$ in $\Oo(G)_{n+2}$, where $f\colon G \to *$. Since $i$ is $n$-suave, the map $i_{n,!}$ admits a right adjoint $i_n^!$; as $f_{n+1,\sharp}(-)$ preserves adjunctions, this produces the required right adjoint, hence primness.

		For (2), we may use (1) to identify the endomorphism object $\End_{(\Oo(G)/A)^!_{n+1}}(\unit) \in A_{n+1} = \End_{A_{n+2}}(\unit)$ with the composite
		\[
			\unit \xrightarrow{f_{n+1,\sharp}(i_{n,!})} (\Oo(G)/A)^!_{n+1} \xrightarrow{f_{n+1,\sharp}(i_n^*)} \unit.
		\]
		Since $\Hom(-, \unit)$ acts as the identity on the unit, this composite may alternatively be computed as
		\[
			\unit \xrightarrow{f_{n+1,*}(i_{n,\sharp})} (\Oo(G)/A)_{n+1} \xrightarrow{f_{n+1,*}(i_n^*)} \unit.
		\]
		Now $f_{n+1,*}(i_n^* i_{n,\sharp}) = f_{n+1,*}(g_{n,\sharp} g_n^*)$ by the pullback square
		\[\begin{tikzcd}
			G' \rar["g"] \dar["g"'] \drar[pullback] & * \dar["i"] \\
			* \rar["i"'] & G.
		\end{tikzcd}\]
		But the map $g_{n,\sharp} g_n^*\colon \unit \to \unit$ in $A_{n+2}$ is precisely $g_{n,\sharp}(\unit) = (\Oo(G')/A)^!_n \in A_{n+1} = \End_{A_{n+2}}(\unit)$, which finishes the proof.
	\end{proof}

	\section{Continuing the proof}
	\label[section]{sec:Cartier_Proof_Continuing}

	Now let $M \in \Sp(\Gest^{n\text{-}\et}_{/X})$. Applying \Cref{thm:Iterated_Affinization} to $\Omega^k M$ for all $k$ shows that each map $M^*_k \to X$ is $n$-prim. It remains to see that it is $n$-proper. The diagonal of $M^*_k$ is a base change of the zero section $* \to M^*_k$: indeed, for any commutative group $G$ the composite
	\[
		G \xrightarrow{\Delta} G \times G \xrightarrow{\mathrm{shear}} G \times G
	\]
	equals $(\id, e)$, which is a base change of $e\colon * \to G$. Consequently the higher diagonals are base changed from the zero sections of the iterated loop groups $\Omega^j G$. (Note that the diagonal of the zero section of $G$ is the zero section of $\Omega G$.)

	\begin{proposition}\label[proposition]{prop:Zero_Section_Prim}
		In the situation of \Cref{thm:Iterated_Affinization}, the map $* \to M^*_1$ is $(n+1)$-prim.
	\end{proposition}
	\begin{proof}
		Assume for simplicity that $n = 0$. For $m \geq 1$, consider the map $i^*_{m-1}\colon (\Oo(M^*_1)/A)_m \to \unit$ in $A_{m+1}$; we show that it admits a right adjoint. As before, it is identified with the map $f_{m-1,!}\colon (\Oo(M_{m-1})/A)^!_m \to \unit$, where $f\colon M_{m-1} \to X$. Since $f$ is $(m-1)$-suave, the functor $f_{m-1,!}$ admits a right adjoint $f^!_{m-1}$, and hence $i^*_{m-1}$ admits a right adjoint $i_{m-1,*}$ in $A_{m+1}$ for each $m \geq 1$. However, we need this right adjoint in $\Oo(M^*_1)_{m+1}$.

		Consider the map $\id \to i_{m,*} i_m^*$ in $\End_{A_{m+2}}\bigl((\Oo(M^*_1)/A)_{m+1}\bigr)$. Applying $\Hom(\unit, -)$ sends it to the unit map $\unit \to i_{m,*}\unit$ in $\Oo(M^*_1)_{m+1}$, so it suffices to show that this unit map admits a right adjoint.

		Consider the map
		\[
			\Oo(M_m \times M_m)_{m+1} \to \End_{A_{m+2}}\bigl((\Oo(M_m)/A)^!_{m+1}\bigr)
		\]
		sending $K$ to the composite
		\[
			(\Oo(M_m)/A)^!_{m+1} \xrightarrow{p_m^!} (\Oo(M_m \times M_m)/A)^!_{m+1} \xrightarrow{-\otimes K} (\Oo(M_m \times M_m)/A)^!_{m+1} \xrightarrow{q_{m,!}} (\Oo(M_m)/A)^!_{m+1},
		\]
		where $p, q\colon M_m \times M_m \to M_m$ are the two projections. The tensoring $-\otimes K$ exists because $(\Oo(M_m \times M_m)/A)^!_{m+1}$ is the image of the unit under the left adjoint $\Oo(M_m \times M_m)_{m+2} \to A_{m+2}$, so that the endomorphisms $\Oo(M_m \times M_m)_{m+1} = \End_{\Oo(M_m \times M_m)_{m+2}}(\unit)$ act on this object.

		One computes that the map $\Delta_{f, m-1, !}$ in $\Oo(M_m \times M_m)_{m+1}$ is sent to the unit $\id \to f_m^! f_{m,!}$. So to prove the claim, it remains to show that $\Delta_{f, m-1, !}$ admits a right adjoint. But this follows from the fact that $* \to M_{m-1}$ is $(m-1)$-suave.
	\end{proof}

	\begin{corollary}\label[corollary]{cor:Dual_Is_Proper}
		Given $M \in \Sp(\Gest^{n\text{-}\et}_{/X})$, we have $M^* \in \Sp(\Gest^{n\text{-prop}}_{/X})$.
	\end{corollary}

	In fact, the argument dualizes, which implies the following.

	\begin{proposition}\label[proposition]{prop:Dual_Is_Etale}
		Given $M \in \Sp(\Gest^{n\text{-prop}}_{/X})$, we have $M^* \in \Sp(\Gest^{n\text{-}\et}_{/X})$.
	\end{proposition}

	\begin{proof}[Proof of \Cref{thm:Cartier_Duality}]
		Let $M \in \Sp(\Gest^{n\text{-}\et}_{/X})$. Then $M^*$ is representable and lies in $\Sp(\Gest^{n\text{-prop}}_{/X})$ by \Cref{cor:Dual_Is_Proper}, and moreover
		\[
			(\Oo(M_k)/\Oo(X))^!_{m+k} \cong (\Oo(M^*_m)/\Oo(X))_{m+k}.
		\]
		To conclude, it suffices to show that the canonical map $M \to (M^*)^*$ is an isomorphism. But for $m + k > n + 1$ we have
		\[
			(\Oo(M^{**}_k)/A)_{m+k} \cong (\Oo(M^*_m)/A)_{m+k} \cong (\Oo(M_k)/A)_{m+k},
		\]
		which gives the claim.
	\end{proof}
	...
	\begin{lemma}\label[lemma]{lem:Ex_Infinity_Countable_Control}
		...
		\begin{enumerate}[(1)]
			...
			in $\PSh(C)$, i.e.\ for every $n\geq 1$ and $0\leq i\leq n$ the maps
			\[
				\Ex^{\infty}(Y)(\Delta^n)\to
				\Ex^{\infty}(Y)(\Lambda_i^n)
			\]
	...
	\end{lemma}
	\begin{proof}
		The simplicial set $\sd^r\Delta^q$ is finite. Although its full simplex
		category contains infinitely many degenerate simplices, the right Kan
		extension can be computed from the finite diagram of nondegenerate
		simplices and their face maps. Thus the formula defining
		$(\Ex^{r}(Y))_q$ expresses it as a finite limit of the objects $Y_n$.

		The remaining assertions are the usual internal properties of the
		$\Ex^{\infty}$ fibrant replacement in an $\infty$-topos; this is the
		countability argument used in \citet[the proof of Proposition~7.3]{Scholze_Gestalten}.
	...
	\begin{proof}
		...
		\url{https://rezk.web.illinois.edu/i-hate-the-pi-star-kan-condition.pdf}.}
	...
	\begin{proof}
		...
		$Y_0\to\abs{Y_{\bullet}}$ is an effective epimorphism. Evaluating at the
		objects representing $X$ and $Z$ gives effective epimorphisms of animae,
		whose homotopy fibers are nonempty. We may therefore choose lifts of the
		two maps $X,Z\to\abs{Y_{\bullet}}$ to $Y_0$, together with the required
		homotopies over $\abs{Y_{\bullet}}$.
	...
	\begin{construction}[Endomorphisms of the unit]
		...
		Assume that $E=C$. We may then describe the endomorphism category of $\ast$ in $\SpanTwoP{C}{E}{I}{P}$ as follows: an object of $\Hom(\ast,\ast)$ is a span of the form $* \leftarrow X \rightarrow *$ corresponding to an object $X\in C$, while a morphism $X\to Y$ corresponds to a span $X\xleftarrow{p}W\xrightarrow{i}Y$ with $p\in P$ and $i\in I$. In other words, we have
		\[
	...
	The six operations make it possible to express, purely in terms of the six-functor formalism, several geometric properties of a morphism: cohomological smoothness, suaveness, primness, \'etaleness, and properness. We explicitly record this vocabulary here, so that we can compare it to the analogous notions in the Gestalten formalism under the transmutation functor $[-]_{\D}$; see \Cref{sec:Translation}. Throughout, $\D$ denotes a six-functor formalism on $(C,E)$ in the sense of \Cref{def:Six_Functor_Formalism}. For the kernel constructions below, we additionally assume that $E$ is closed under relative diagonals. Equivalently, the wide subcategory $C_E$ admits fiber products and its inclusion into $C$ preserves them, as in the notion of a geometric setup used by \citet[Definition~2.1.1]{Heyer_Mann}. Morphisms are assumed to lie in $E$ unless stated otherwise.
	...
	\begin{definition}[Cohomologically smooth morphism]
		...
		\begin{enumerate}
			...
			\item\label{def:Cohom_Smooth_BC} For every pullback square
			\[
			\begin{tikzcd}
				X' \rar{g'} \dar[swap]{f'} \drar[pullback] & X \dar{f} \\
				Y' \rar[swap]{g} & Y,
			\end{tikzcd}
			\]
			with $g$ arbitrary, the morphism $f'$ also satisfies conditions (1) and (2), and the canonical map $g'^* f^!(\unit_Y) \to f'^!(\unit_{Y'})$ is an isomorphism.
		\end{enumerate}
	\end{definition}
	...
	\begin{definition}[$2$-category of kernels]
		...
		\end{itemize}
		The extra diagonal hypothesis above ensures that the identity kernels and all exceptional pushforwards in the composition formula are defined. Formally, one equips $\Span(E_{/Y})$ with the closed symmetric monoidal structure induced by the cartesian monoidal structure on $E_{/Y}$, regards it as self-enriched via the internal homs $[X,X']=X\times_YX'$ supplied by self-duality in a span category, and transfers this enrichment along the lax symmetric monoidal functor $\D_Y\colon \Span(E_{/Y}) \to \Cat$.
	...
	\begin{proposition}[{\citet[\textsection 4.4]{Heyer_Mann}}]
		...
		\begin{enumerate}
			\item If $A \in \D(X)$ is $f$-suave, then $\SD_f(A)$ is also $f$-suave (with right adjoint $A$ in $K_{\D, Y}$), and there is a natural equivalence
			\[
				\SD_f(A) \otimes f^*(-) \;\simeq\; \uHom\bigl(A, f^!(-)\bigr) \colon \D(Y) \to \D(X).
			\]
			Setting $- = \unit_Y$ yields $\SD_f(A) \simeq \uHom(A, f^!(\unit_Y))$, exhibiting $\SD_f(A)$ as a relative Verdier dual of $A$. The bidual identity $\SD_f(\SD_f(A)) \simeq A$ then holds, and $\SD_f$ defines a contravariant equivalence
			\[
			...
			\]
			and the bidual $\PD_f(\PD_f(A)) \simeq A$ holds. Consequently $\PD_f$ defines a contravariant equivalence
			\[
	...
	\begin{definition}[Cohomologically \'etale and proper morphisms]
		...
		Let $\D$ be a six-functor formalism on $(C, E)$, and let $f \colon X \to Y$ be a truncated morphism in $E$ (i.e.\ $n$-truncated for some integer $n\geq -2$).
		\begin{itemize}
	...
	Suppose that $\Delta_f$ is $\D$-étale. This hypothesis already implies that $f$ is truncated: by definition $\Delta_f$ is truncated, and a morphism with truncated diagonal is truncated. We may therefore argue inductively on the truncation level of $f$. Write $\pi_1,\pi_2\colon X\times_YX\to X$ for the projections, so that
	\[
	\begin{tikzcd}
	...
	\end{tikzcd}
	\]
	The base-change isomorphism $f^*f_!\iso\pi_{1,!}\pi_2^*$ has a double mate
	\[
		\Ex^{*!}\colon \pi_1^!f^*\longrightarrow\pi_2^*f^!.
	\]
	By induction, the étale comparison $c_{\Delta_f}\colon\Delta_f^*\to\Delta_f^!$ is already an isomorphism. We define the \emph{étale comparison} for $f$ to be the composite
	\[
	\begin{aligned}
		c_f\colon f^*
		&\iso \Delta_f^!\pi_1^!f^*
		\xrightarrow{\Delta_f^!\Ex^{*!}} \Delta_f^!\pi_2^*f^!
		\xrightarrow{c_{\Delta_f}^{-1}} \Delta_f^*\pi_2^*f^!
		\iso f^!.
	\end{aligned}
	\]
	Equivalently, under $f^*\dashv f_*$ this comparison is the mate
	\[
		\id\longrightarrow f_*f^!,
	\]
	while under $f_!\dashv f^!$ it is the mate of the trace map
	\[
		\tr_f\colon f_!f^*\longrightarrow\id.
	\]
	In terms of the base-change description above, these mates are obtained by inserting the unit for the adjunction associated with $\Delta_f$, after identifying $\Delta_f^*$ with $\Delta_f^!$ by $c_{\Delta_f}$. Thus the construction uses only the six-functor structure and the comparison already constructed for the diagonal; no comparison is asserted when $\Delta_f$ is not known to be $\D$-étale.

	The following proposition shows that $f$ is $\D$-étale if and only if this inductively defined comparison is an isomorphism. A dual inductive construction produces a map $f_! \to f_*$ whenever $\Delta_f$ is known to be $\D$-proper.
	...
	\begin{proposition}[Characterization of \'etaleness and properness, {cf.\ \citet[\textsection 4.6]{Heyer_Mann}}]
		...
		Let $\D$ be a six-functor formalism on $(C, E)$ and let $f \colon X \to Y$ be a morphism in $E$. In either part below, the hypothesis on $\Delta_f$ already implies that $f$ is truncated.
		\begin{enumerate}
			\item Suppose $\Delta_f$ is $\D$-\'etale. Then the following are equivalent:
			\begin{enumerate}
				\item[(a)] $f$ is $\D$-\'etale;
				\item[(b)] The comparison $f^*\unit_Y \to f^!\unit_Y$ is an isomorphism in $\D(X)$;
				\item[(c)] The comparison $f^*\to f^!$ is an isomorphism of functors.
			\end{enumerate}
	...
	\end{itemize}
	These two compatibilities are closely related. The adjunction $e_{\sharp}\dashv e^*$ induces an adjunction
	\[
		\Span(e^*)\dashv \Span(e_{\sharp}).
	\]
	Similarly, restriction along $e^*$ is right adjoint to restriction along $e_{\sharp}$. The contravariant compatibility is therefore the mate of the covariant compatibility. In particular, if we remember the covariant compatibility relations as coherent data, the contravariant compatibilities can be stated as properties.
	...
	\end{theorem}
	\begin{proof}
		The proof is given in \citet[\textsection 8]{CLL_Span}; there, the data in the second bullet point occurs under the name \emph{$E$-commutative monoid in the $C$-category $X \mapsto \Fun(C_{/X},A)$}.
	...
	\]
	in $E_{/Y}$ and then evaluating at $\id_Y$. A general span $X \xleftarrow{f} U \xrightarrow{e} Y$ is sent to the composite $e_!f^*$. The covariant compatibility in \Cref{thm:Slicewise_Reconstruction}, whose symbol $e_*$ denotes restriction between presheaf categories rather than a pushforward in a six-functor formalism, is precisely what makes this construction functorial under composition of spans.

	\begin{corollary}[Symmetric monoidal slicewise reconstruction]
		\label[corollary]{cor:Slicewise_Reconstruction_Monoidal}
		Let $A$ be a symmetric monoidal category. In the situation of \Cref{thm:Slicewise_Reconstruction}, suppose that the slicewise data are equipped with coherent external products
		\[
			\D_{/X}(Z)(W)\otimes\D_{/X'}(Z')(W')
			\longrightarrow
			\D_{/(X\times X')}(Z\times Z')(W\times W')
		\]
		and a unit, satisfying the lax symmetric monoidal coherences and compatible with the span actions and with the contravariant and covariant compatibility equivalences. Then the reconstructed functor
		\[
			\D\colon\Span(C,E)\longrightarrow A
		\]
		admits a unique lax symmetric monoidal structure inducing the given slicewise structures. In particular, for $A=\Cat$, compatible families of slicewise three-functor formalisms assemble into a three-functor formalism on $(C,E)$. The same applies to $1\Pr$ and $\PrL$.
	\end{corollary}
	\begin{proof}
		We refine the localization proof of \Cref{thm:Slicewise_Reconstruction}. In the notation of \citet[Propositions~8.18 and 8.21]{CLL_Span}, let
		\[
			K:=\Span_{\mathrm{fw}}(\Ar_E(C))
		\]
		be the span category obtained from the arrow category of $E$-maps. Slicewise data are equivalently functors $K\to A$ which invert the class $W$ encoding the covariant compatibility, and the source functor induces a localization
		\[
			s\colon K\longrightarrow\Span(C,E)
		\]
		at $W$.

		Cartesian product, computed pointwise in the arrow category, makes the adequate triple defining $K$ into a commutative algebra among adequate triples. Since the span construction preserves finite products, $K$ is symmetric monoidal and $s$ is symmetric monoidal. The class $W$ is closed under tensoring with arbitrary objects: its generators arise from composable maps $f,q\in E$, and tensoring such a generator with an arrow in $E$ gives the generator associated with the corresponding product maps. Consequently $s$ is a symmetric monoidal localization. Thus precomposition with $s$ identifies lax symmetric monoidal functors $\Span(C,E)\to A$ with lax symmetric monoidal functors $K\to A$ whose underlying functors invert $W$.

		Finally, the straightening and unstraightening equivalences used to identify functors $K\to A$ with the slicewise data are compatible with cartesian products on the indexing categories. Under this identification, a lax symmetric monoidal structure on a functor $K\to A$ is precisely a coherent system of external products as in the statement. Applying the symmetric monoidal universal property of $s$ gives the asserted refinement and its uniqueness.
	\end{proof}
