...
\begin{document}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&\overset{(2)}{\simeq} g_m^*((B/A)^!_m),
		\end{align*}
		where (1) holds since $g_{m+1}^*$ restricts to $g_m^*$ on endomorphisms of the unit and (2) is \Cref{lem:Suave_Dualizing_Object_Via_Adjunction}. Under this equivalence, the counit map $(f'/C)_{m-1,!}\colon (B \otimes_A C/C)^!_m \to \unit$ is the image under $g_m^*$ of the counit map $(f/A)_{m-1,!}\colon (B/A)^!_m \to \unit$, and hence it admits a right adjoint. This shows that $f'$ is $n$-suave, and that the we have
		\[
	...
	\begin{corollary}
		Consider a 1-presentable stable category $E \in 1\Pr_{\st}$. Then there are (order-presering or order-reversing) bijections between the following four posets:
		\begin{enumerate}
	...
	\]
	assigning to each $X \in C$ a presentably symmetric monoidal category $\D(X)$ of coefficients, to each map $f$ an adjoint pairs $f^* \dashv f_*$, and to each map $f$ in a wide subcategory $E$ of $C$ closed under base change an adjunction $f_! \dashv f^!$, compatible via base change and the projection formula (see \Cref{chap:Six_Functor_Formalisms} for some recollections). As was observed in \cite[Lecture~X]{Scholze_Six_Functors}, it is an emperical fact that many six-functor formalisms on the category $C = \Sch$ of schemes (separated of finite type, say) can be factored as a composite
	\[
		\Span(\Sch) \xrightarrow{X \mapsto X^{?}} \Span(\AnStck) \xrightarrow{\D_{\qc}} 1\Pr,
	\]
	where $\AnStck$ is the category of analytic stacks and $\D_{\qc}$ is a six-functor formalism on $\AnStck$ given by quasi-coherent sheaves. The functor $X \mapsto X^{?}$ from schemes to analytic stacks is called a \emph{transmutation}, following \cite[Remark~2.3.8]{Bhatt2022FGauges}. Here are some examples:
	
	\begin{enumerate}
		\item For $\D(\Z)$-valued sheaves we have $\D(X,\Z) \simeq \D_{\qc}(X_{\mathrm{Betti}}),$ where $X_{\mathrm{Betti}}$ is the Betti stack of $X$
		\item For algebraic D-modules, we have $D\mathrm{mod}(X) = D_{\qc}(X_{\mathrm{dR}}),$ where $X_{\mathrm{dR}}$ is the de Rham stack of $X$.
	...
	Note that this definition of the 2-category of kernels is different from the one that usually appears in the literature, e.g.\ in \cite{Heyer_Mann}. Usually, one considers the canonical self-enrichment of $\Span(C)$ via its closed monoidal structure, and then transfers the enrichment via the lax monoidal functor $\D\colon \Span(C) \to \Cat$. The following result shows that $\bbK_{\D}$ is the $1\Pr$-enriched presheaves on this 2-category.
	...
	We will now show that the category of kernels is boring if $\D$ satisfies the categorical Künneth formula, then $\bbK_D$ is simply the module category $\Mod_{\D(*)}(1\Pr)$. we start by constructing a comparison functor.
	...
	\]
	For each map $f\colon Y \to X$ in $C$, left Kan extension along the symmetric monoidal pullback functor $\Span(f^*)\colon \colon \Span(C_{/X}) \to \Span(\C_{/Y})$ induces a symmetric monoidal restriction functor
	\[
	...
	\begin{proof}
		...
		\[
			...
			f_!B\times_XA,
		\]
		which follows directly from the defining pullback square for $f^*A$. The same computation shows that the map
		\[
			\Span(f^*)^*\bigl(N\otimes\Span(f_!)^*M\bigr)
			\xrightarrow{\ \sim\ }
			\Span(f^*)^*N\otimes M
		\]
		is an isomorphism. All in all, we obtain the desired pair of adjunctions
		\[
			f_{1,\sharp} \vdash f_1^* \vdash f_{1,*},
		\]
		where by construction we have an identification $f_{1,\sharp} = f_{1,*}$.
		The two projection-formula equivalences above are $\D_X$-linear, so they pass to the corresponding module categories and give the projection formula for $f_{1,\sharp}$ and $f_{1,*}$.
	...
	\begin{corollary}
		\label[corollary]{cor:Kernels_1_prim}
		Consider the functor
		\[
			C \to \Gest, \qquad X \mapsto \Gest_X(\bbK_{\D,X}).
		\]
		For each map $f\colon X \to Y$, the induced map $f^*\colon \Gest_X(\bbK_{\D,X}) \to \Gest_X(\bbK_{\D,Y})$ is $2$-proper and $1$-prim.
	\end{corollary}
	\begin{proof}
		Since $\Gest_X(\bbK_{\D,X})$ is 2-affine for every $X$, the map $f^*$ is automatically $2$-affine as well, hence in particular $2$-proper. Moreover, \Cref{prop:Adjoints_Category_Of_Kernels} provides the required right adjoint $f_{1,*}$ of $f_1^*$, so it is even $1$-prim.
	\end{proof}

	\begin{remark}\label[remark]{rem:Kunneth_Fails}
		As we will now explain, the map $f_1^*$ is very close to being $1$-suave as well. Since $f^*$ is $2$-proper, \Cref{prop:Ambidexterity_Stefanich_Rings} shows that it is $3$-\'etale, and hence in particular $3$-suave. In order to prove that $f^*$ is already $2$-suave, it remains to construct a left adjoint to the degree-$2$ pullback
		\[
			f_2^*\colon \Mod_{\bbK_{\D,X}}(2\Pr)\longrightarrow\Mod_{\bbK_{\D,Y}}(2\Pr)
		\]
		and to verify the first instance of the secondary adjointability condition in \Cref{def:N_Suave}. It is natural to try to apply our usual ambidexterity argument to this situation. The functor $f_2^*$ has a right adjoint by $2$-primness, and the unit of this adjunction admits a right adjoint because $f$ is $1$-prim. To apply \Cref{prop:General_Ambidexterity}, one would also have to show that the counit admits a right adjoint. The expected description of this counit is the degree-$2$ pullback associated with the diagonal of $f$; if this description were available, this counit would have a right adjoint as well, giving us a left adjoint $f_{2,\sharp}$ and an identification $f_{2,\sharp} \simeq f_{2,*}$. However, getting such a description requires the assignment $X \mapsto \Gest_X(\bbK_{\D,X})$ to satisfy the Künneth formula, which in general it doesn't.

		In fact, if the Künneth formula were to hold, all maps would be 1-proper and 1-étale, by applying the same reasoning to all iterated diagonals.
	\end{remark}

	In order to force the Künneth formula to hold, we will repeat the category of kernels construction.
	...
	Consider a presentable six-funtor formalism $\D\colon \Span(C) \to 1\Pr$, and consider a map $f\colon Y \to X$ in $C$. We have two competing notions of cohomological properties (suaveness, primness, étaleness, properness):
	\begin{itemize}
	...
	\emph{During the seminar, the content of this section was discussed as part of Talk \ref{chap:SH}; it has been inserted here for sake of exposition.}
	...
	\begin{proof}
		...
		Now consider the natural morphism $\unit \to g_{n+1,*}\unit$ in $\Oo(X)_{n+2}$. Applying $f_{n+1,*}f_{n+1}^*$ to it, this becomes $f_{n+1,*}\unit \to h_{n+1,*}\unit$, and the existence of $f_{n,\sharp}$ amounts to the commutative square
		\[\begin{tikzcd}
	...
	Restricting to truncated maps, we can however get the desired six-functor formalism:

	\begin{proposition}[{\citet{Aoki_PhD_Thesis}}]\label[proposition]{prop:6FF_Truncated_Gest}
		Fix some Gestalt $X$, and let $C \subseteq \Gest_{/X}$ be the small category of Gestalten over $X$ that are truncated, $1$-étale and $1$-proper. Then there is a six-functor formalism
		\[
			\Span(C) \to \Oo(X)_2\colon Y \mapsto (\Oo(Y)/\Oo(X))_1
		\]
		refining the above construction. In particular, forgetting along $\Oo(X)_2 \to 1\Pr$, this yields a six-functor formalism
		\[
			\Span(C) \to 1\Pr\colon Y \mapsto \Oo(Y)_1.
		\]
	\end{proposition}
	\begin{proof}
		In short, we apply \cite[Theorem~B]{CLL_Span2} (recalled in \Cref{thm:Span_2_Universal}) to obtain a once-categorified six-functor formalism $Y \mapsto \Oo(Y)_2$, and then pass to endomorphisms to get the desired functor $\Span(C) \to \Oo(X)_2$. We spell out the details.

		The category $C$ has finite limits: truncated, $1$-étale and $1$-proper maps are all stable under base change and composition, and the identity of $X$ has all three properties. Moreover, every morphism $f\colon Y \to Z$ in $C$ is truncated, since it is a morphism between truncated objects of $\Gest_{/X}$, and the cancellation properties in \Cref{prop:Etale_Properties,prop:Proper_Properties} guarantee it is again $1$-étale and $1$-proper. We may therefore apply the setup of \Cref{sec:2Cat_Enhancement} with
		\[
			I=P=E=C,
		\]
		where the notation means that all three wide subcategories contain every morphism of $C$. The base-change, factorization and left-cancellation hypotheses are then automatic, while the truncatedness hypothesis on $I\cap P$ holds by the preceding observation.

		Let $A := \Oo(X)$. In what follows, we regard $A_3$ as a $2$-category and $A_2$ as a category via the usual forgetful functors, without changing notation. Consider the symmetric monoidal functor
		\[
			\Phi\colon C\catop\longrightarrow \CAlg(A_3),
			\qquad
			Y\longmapsto (\Oo(Y)/A)_2,
		\]
		where the contravariant functoriality along $f\colon Y\to Z$ is via the relative map $(f/A)^*_1$. Its symmetric monoidal structure follows from the Künneth formula \Cref{lem:Kunneth_Prim}, applied in degree $2$.

		We claim that $\Phi$ is $(C,C)$-biadjointable in the sense of \Cref{def:Biadjointable_Functor}. Indeed, every $f$ in $C$ is $1$-étale and $1$-proper, so $(f/A)^*_1$ admits both the internal left adjoint $(f/A)_{1,\sharp}$ and the internal right adjoint $(f/A)_{1,*}$. The two Beck--Chevalley conditions are \Cref{prop:Prim_Suave_Base_Change}, the double Beck--Chevalley condition is \Cref{prop:Double_BC_Gest}, and the two projection formulae are \Cref{lem:Suave_Projection_Formula,lem:Prim_Projection_Formula}. Applying \Cref{thm:Span_2_Universal} now produces a lax symmetric monoidal $2$-functor
		\[
			\widetilde\Phi\colon \SpanTwoP{C}{C}{C}{C}\longrightarrow A_3
		\]
		extending $\Phi$.

		We have $\Phi(\ast)=(A/A)_2\simeq\unit_{A_3}$. Hence passing to endomorphisms of the unit, as in \Cref{con:Endo_Unit}, gives a lax symmetric monoidal functor
		\[
			\Span(C)
			\simeq \Hom_{\SpanTwoP{C}{C}{C}{C}}(\ast,\ast)
			\longrightarrow
			\Hom_{A_3}(\unit,\unit)
			\simeq A_2=\Oo(X)_2.
		\]
		It remains to identify its value on objects. An object $Y\in C$ corresponds on the left to the endomorphism span
		\[
			\ast\xleftarrow{p_Y}Y\xrightarrow{p_Y}\ast.
		\]
		By regarding the forward map as living in $P$, its image under $\widetilde\Phi$ is the endomorphism $(p_Y/A)_{1,*}(p_Y/A)^*_1$ of the unit in $A_3$. Under the equivalence $\End_{A_3}(\unit)\simeq A_2$, evaluation at the unit identifies this with
		\[
			(p_Y/A)_{1,*}(p_Y/A)^*_1(\unit)
			\simeq (\Oo(Y)/A)_1
		\]
		by \Cref{obs:Endomorphism_Object_Via_Right_Adjoint}. Thus the resulting functor has the asserted value on $Y$.

		Finally, for a morphism $f\colon Y\to Z$ in $C$, the forward span $Y\xleftarrow{\id}Y\xrightarrow{f}Z$ acts through the counit of $(f/A)_{1,\sharp}\dashv(f/A)^*_1$. Since $f$ belongs to $I\cap P$, the two factorizations $f=f\circ\id=\id\circ f$ in the formula for the exceptional pushforward of \Cref{thm:Span_2_Universal} give a canonical identification $(f/A)_{1,\sharp}\simeq(f/A)_{1,*}$. After evaluation at the unit, the above counit is precisely the map $f_{0,!}$ constructed before the proposition. Consequently the functor refines that construction. All its values and structure functors are presentable, so the required right adjoints exist by \Cref{rmk:Presentable_Targets}; hence it is a six-functor formalism. Forgetting along $A_2=\Oo(X)_2\to1\Pr$ sends $(\Oo(Y)/A)_1$ to $\Oo(Y)_1$ and gives the second displayed formalism.
	\end{proof}

	\begin{proposition}
		Let $(C,E,I,P)$ be a quadruple satifying the conditions of \Cref{thm:Construction_6FF}, and let $\D\colon \Span_2(C,E)_{P,I} \to 1\Pr$ be some 2-functor. Then $\D$ agrees with the composite
		\[
			\Span_2(C,E)_{P,I} \xrightarrow{[-]_{\D}} \Span_2(\Gest, \Gest^!) \xrightarrow{\Oo(-)_1} 1\Pr.
		\]
	\end{proposition}
	\begin{proof}
		By the universal property of the span 2-category from \Cref{thm:Span_2_Universal}, it suffices to check the agreement on $C\catop$, where it holds by \Cref{thm:Transmutation}.
	\end{proof}
	...
	\begin{definition}[Projection formula]
		...
		\begin{enumerate}
			...
			\item Dually, if $p^*$ admits a right adjoint $p_*$, then the \emph{dual projection map}
			\[
				N\otimes p_*(M)\to p_*\big(p^*N\otimes M\big)
			\]
			is required to be an isomorphism; we then say $p_*$ satisfies the \emph{dual projection formula}.
		\end{enumerate}
	\end{definition}
	...
	\begin{theorem}[{Mann \citep[Proposition~A.5.10]{mann2022sixFunctor}}]
		...
		\begin{enumerate}
			...
			\item for every $p\colon X\to Y$ in $P$, the pullback $p^*$ admits a right adjoint $p_*$ satisfying the dual Beck--Chevalley condition and the dual projection formula;
	...
	\begin{construction}[The span $2$-category]
		...
		\begin{itemize}
			...
			commute.
		\end{itemize}
		Composition of $2$-cells is again by pullback, and the underlying $1$-category is $\Span(C,E)$. Making this precise (as a $2$-fold complete Segal space) is the technical heart of \citet{CLL_Span2}, to which we refer for details.
	\end{construction}
	...
	\begin{definition}[$(I,P)$-biadjointable lax symmetric monoidal functor]
		...
		\begin{enumerate}
			...
			\item For every $p\in P$, the morphism $p^*$ admits a right adjoint $p_*$ satisfying the dual Beck--Chevalley condition and the dual projection formula.
			\item For every pullback square
			\[
			\begin{tikzcd}
				X'\rar{i}\dar[swap]{p}\drar[pullback] & X\dar{q}\\
