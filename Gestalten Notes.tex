...
\begin{document}
	...
	\begin{notation}\label[notation]{nota:Absolute_Transition_Adjunctions}
		For every $n \geq 0$, the module-endomorphism adjunction gives an adjunction
		\[
			\Mod^{\bbP}_n\colon \CAlg(n\Pr) \rightleftarrows \CAlg((n+1)\Pr) \noloc \End^{\bbP}_n,
		\]
		where
		\[
			\Mod^{\bbP}_n(R) := \Mod_R(n\Pr), \qquad \End^{\bbP}_n(D) := \End_D(\unit_D).
		\]
	\end{notation}
	...
	\begin{definition}
		...
		\[
			\StRing \; := \; \colim_{1\Pr}\bigl(\CAlg(\An) \xrightarrow{\Mod^{\bbP}_0} \CAlg(1\Pr) \xrightarrow{\Mod^{\bbP}_1} \CAlg(2\Pr) \xrightarrow{\Mod^{\bbP}_2} \dots\bigr).
		\]
		Equivalently, passing to the right adjoints gives
		\[
			\StRing \simeq \lim_{\CAT}\bigl(\CAlg(\An) \xleftarrow{\End^{\bbP}_0} \CAlg(1\Pr) \xleftarrow{\End^{\bbP}_1} \CAlg(2\Pr) \xleftarrow{\End^{\bbP}_2} \dots\bigr).
		\]
	\end{definition}
	...
	\begin{observation}
		...
		\[
			\Mod^{\bbP}_n\colon \CAlg(n\Pr) \hookrightarrow \CAlg((n+1)\Pr), \qquad R \mapsto \Mod_R(n\Pr),
		\]
		with right adjoint $\End^{\bbP}_n(D) := \End_D(\unit_D)$. Note that for $A \in \CAlg(\Sp)$, there is a canonical equivalence
		\[
			n\Pr_A = \Mod_{(n-1)\Pr_A}(1\Pr) \simeq \Mod_{(n-1)\Pr_A}(\Mod_{(n-1)\Pr}(1\Pr)) = \Mod_{(n-1)\Pr_A}(n\Pr) = \Mod^{\bbP}_n((n-1)\Pr_A).
		\]
		In particular, by fully faithfulness of $\Mod^{\bbP}_n$ there is a canonical equivalence
		\begin{equation}
		\label{eq:Structure_Maps_St(A)}
			\End^{\bbP}_n(n\Pr_A) \simeq (n-1)\Pr_A.
		\end{equation}
	\end{observation}
	...
	\begin{definition}
		...
		\[
			\St(A) \qin \StRing \; = \; \lim_{\CAT}\bigl(\CAlg(\An) \xleftarrow{\End^{\bbP}_0} \CAlg(1\Pr) \xleftarrow{\End^{\bbP}_1} \CAlg(2\Pr) \xleftarrow{\End^{\bbP}_2} \dots\bigr)
		\]
		to be the sequence
		\[
	...
	\end{lemma}
	\begin{proof}
		Since we have $n\Pr_A = \Mod^{\bbP}_n((n-1)\Pr_A)$ for all $n$, this assignment is equivalent to the following composite:
		\[
			\CAlg(\Sp) \xrightarrow{\Mod_{(-)}(\Sp)} \CAlg(1\Pr) \hookrightarrow \StRing,
		\]
		where the second functor is fully faithful as it is a colimit over the fully faithful functors
		\[
			\Mod^{\bbP}_n\colon \CAlg(n\Pr) \hookrightarrow \CAlg((n-1)\Pr).
		\]
	...
	\begin{theorem}
		The functor $n\Pr_{(-)}^!\colon \Aff_{\S} \to 1\Pr$ (defined on maps by the right adjoints $f^!$) is a hypersheaf with respect to the fpqc topology for each $n \geq 0$. (Note that for $n \geq 1$, the maps $f^!$ agree with $f^*$ by ambidexterity.)
	\end{theorem}
	...
	\[
		\St(X) := (\Omega^\infty\Oo(X), \D(X), 1\Pr_X, 2\Pr_X, \ldots),
	\]
	with $n$-th component $\St(X)_n := (n-1)\Pr_X$ (using the low-degree conventions $0\Pr_X := \D(X)$ and $(-1)\Pr_X := \Omega^\infty\Oo(X)$). This extends the affine assignment $A \mapsto \St(A)$ to a \emph{Stefanich ring functor}
	\[
		\St\colon \Shv_{\fpqc}(\Aff_{\S})\catop \to \StRing,
	\]
	restricting to $\St\colon \CAlg(\Sp) \hookrightarrow \StRing$ on affines.
	...
	\begin{corollary}\label[corollary]{cor:StRing_Functor_Properties}
		The functor
		\[
			\Shv\nolimits_{\textup{fpqc}}(C)^{\aleph_1} \to \StRing\catop_{\S/}, \qquad X \mapsto \St(X)
		\]
	...
	\begin{align*}
		\StRing &:= \colim_{1\Pr}(\CAlg(\An) \xrightarrow{\Mod^{\bbP}_0} \CAlg(1\Pr) \xrightarrow{\Mod^{\bbP}_1} \CAlg(2\Pr) \xrightarrow{\Mod^{\bbP}_2} \dots) \\
		& \simeq \lim_{\CAT}(\CAlg(\An) \xleftarrow{\End^{\bbP}_0} \CAlg(1\Pr) \xleftarrow{\End^{\bbP}_1} \CAlg(2\Pr) \xleftarrow{\End^{\bbP}_2} \dots),
	\end{align*}
	where we write $\Mod^{\bbP}_n(R) := \Mod_R(n\Pr)$ and $\End^{\bbP}_n(D) := \End_D(\unit_D)$.

	We denote the objects of this category as $A = (A_0,A_1,A_2, \dots)$, where $A_n \in \CAlg(n\Pr)$ and the structure isomorphisms $A_n \simeq \End^{\bbP}_n(A_{n+1})$ are left implicit in the notation. Similarly, morphisms from $A$ to $B$ are collections of maps $f_{n-1}^*\colon A_n \to B_n$ compatible with the structural isomorphisms.
	...
	\begin{notation}
		...
		\[
			...
			\end{tikzcd}
		\]
		where the left adjoint sends $R$ to $\Mod_R(C)$ and the right adjoint sends $D$ to $\End_D(\unit_D)$. Here $\Mod_C(n\Pr)$ agrees with $\Mod_C(1\Pr)$ after forgetting the ambient level, since the $C$-action restricts along $(n-1)\Pr \to C$ to the required $(n-1)\Pr$-linear structure. Now let $A$ be a Stefanich ring. Applying this adjunction with $C = A_m$, the structural equivalence
		\[
		...
		\]
		for the relative module transition, and
		\[
			\End^A_m := \End_{(-)}(\unit) \circ U^A_m\colon \CAlg(A_{m+1}) \to \CAlg(A_m)
		\]
		for its right adjoint. Explicitly, $\Mod^A_m(R) = L^A_m(\Mod_R(A_m))$ and $\End^A_m(D) = \End_{U^A_m(D)}(\unit)$.
	\end{notation}
	...
	\begin{proposition}\label[proposition]{prop:Slice_Description}
		...
		\[
			\StRing_{A/} \simeq \colim_{1\Pr}(\CAlg(A_1) \xrightarrow{\Mod^A_1} \CAlg(A_2) \xrightarrow{\Mod^A_2} \dots),
		\]
		or equivalently
		\[
			\StRing_{A/} \simeq \lim_{\CAT}(\CAlg(A_1) \xleftarrow{\End^A_1} \CAlg(A_2) \xleftarrow{\End^A_2} \dots).
		\]
	...
	\begin{proof}
		...
		\[
			\StRing_{A/} = \lim_{\CAT}(\dots \xrightarrow{\End^{\bbP}_2} \CAlg(2\Pr)_{A_2/} \xrightarrow{\End^{\bbP}_1} \CAlg(1\Pr)_{A_1/} \xrightarrow{\End^{\bbP}_0} \CAlg(\An)_{A_0/}).
		\]
		...
		Since each functor $\End^A_m$ admits the left adjoint $\Mod^A_m$, we may alternatively compute $\StRing_{A/}$ as a colimit in $1\Pr$ over these left adjoints:
		\[
			\StRing_{A/} \simeq \colim_{1\Pr}(\CAlg(A_1) \xrightarrow{\Mod^A_1} \CAlg(A_2) \xrightarrow{\Mod^A_2} \dots).
		\]

		\emph{Step 2:} We next show that each transition functor $\Mod^A_m\colon \CAlg(A_m) \to \CAlg(A_{m+1})$ in the colimit description is fully faithful. Since $\Mod^A_m$ is a morphism in $1\Pr$, \Cref{lem:Fully_Faithful_On_Aleph1_Compacts} shows that it suffices to check that the unit of the adjunction $\Mod^A_m \dashv \End^A_m$ is an equivalence on $\aleph_1$-compact objects, i.e.\ on countably presented objects of $\CAlg(A_m)$. Let $R \in \CAlg(A_m)$ be countably presented. Since $\Mod^A_m(R) = L^A_m(\Mod_R(A_m))$, we get
		\[
			\End^A_m(\Mod^A_m(R)) \simeq \End_{L^A_m(\Mod_R(A_m))}(\unit).
		\]
		...
		\]
		It admits an $A_m$-linear right adjoint $G_R\colon \Mod_R(A_m) \to A_m$, given by forgetting the $R$-module structure. Since $R$ is countably presented, $G_R$ preserves $\aleph_1$-compact objects, hence this adjunction lives inside $\Mod_{A_m}(m\Pr)$. Applying the functor of $2$-categories $L^A_m\colon \Mod_{A_m}(m\Pr) \to A_{m+1}$, we obtain an adjunction
		\[
			L^A_m(F_R) \colon L^A_m(A_m) \rightleftarrows L^A_m(\Mod_R(A_m)) \noloc L^A_m(G_R)
		\]
		in $A_{m+1}$. In particular, we get
		\begin{align*}
			\End^A_m(\Mod^A_m(R)) &= \End_{L^A_m(\Mod_R(A_m))}(\unit) \\
			& = \End_{L^A_m(\Mod_R(A_m))}(L^A_m(F_R)(\unit_{A_{m}})) \\
			& = \Hom_{L^A_m(A_{m})}(\unit_{A_m}, L^A_m(G_R)L^A_m(F_R)(\unit_{A_m})) \\
			& \simeq L^A_m(G_R \circ F_R)(\unit_{A_m}) \\
			& = L^A_m(R \otimes -)(\unit_{A_m}) \\
			& = R.
		\end{align*}
		Under this identification, the unit map $R \to \End^A_m(\Mod^A_m(R))$ is the map induced by the action of $R$ on the free rank-one $R$-module $F_R(\unit_{A_m})$. Under the adjunction $L^A_m(F_R) \dashv L^A_m(G_R)$, this action corresponds to the identity of $G_RF_R(\unit_{A_m}) \simeq R$. Thus the unit is an equivalence on countably presented $R$, and $\Mod^A_m$ is fully faithful.
		...
		\[
			\StRing_{A/} \; \simeq \; \lim_{\CAT}( \dots \xrightarrow{\End^A_{n+1}} \CAlg(A_{n+1}) \xrightarrow{\End^A_n}\CAlg(A_n))
		\]
		$p_n$ is left adjoint to the projection $q_n\colon \StRing_{A/} \to \CAlg(A_n)$. We must show that the unit map $R \to q_np_n(R)$ is an isomorphism for each $R \in \CAlg(A_n)$. Observe that by fully faithfulness of the functors $\Mod^A_m$, the sequence of objects
		\[
			X_j = \Mod^A_{j-1}\Mod^A_{j-2}\cdots \Mod^A_n(R) \qin \CAlg(A_j)
		\]
		...
		\[
			\begin{aligned}
				...
				&\simeq \lim_{j \geq n} \Hom_{\CAlg(A_n)}(R,\End^A_n \cdots \End^A_{j-1}((B/A)_{j-1})) \\
				&\simeq \Hom_{\CAlg(A_n)}(R,(B/A)_{n-1}).
			\end{aligned}
		\]
		Here the last equivalence uses the compatibility equivalences $\End^A_i((B/A)_i) \simeq (B/A)_{i-1}$ defining the object $B$ of the limit.
	...
	\begin{proof}
		...
		\]
		we see that it suffices to show that the unit map $\unit_C \to U^A_mL^A_m\unit_C = U^A_m\unit_D$ is an isomorphism. But this map is canonical comparison map $A_m \iso \End_{A_{m+1}}(\unit_{A_{m+1}})$, which is an isomorphism as $A$ is a Stefanich ring.
	\end{proof}
	...
	Limits and $\aleph_1$-filtered colimits in $\StRing$ are computed pointwise, since they are preserved by the transition functors $\End^{\bbP}_n$ in the limit defining $\StRing$. Arbitrary colimits are harder to describe directly, since they are not (necessarily) preserved by the endomorphism functors.
	...
	\begin{definition}\label[definition]{def:PreStRing}
		...
		\[
			\Theta\colon \mathcal{P} \to \mathcal{P}, \qquad (D_n)_{n \geq 0} \mapsto (\End^{\bbP}_n(D_{n+1}))_{n \geq 0}.
		\]
		...
		\[
			\alpha_n\colon D_n \to \End^{\bbP}_n(D_{n+1})
		\]
	...
	\begin{proof}
		...
		\]
		by setting $D'_n = \End^{\bbP}_n(D_{n+1})$, with structure maps obtained by applying the functors $\End^{\bbP}_n$ to the structure maps of $D$. This admits a natural transformation $\eta\colon \id \to T$, given by the structure maps of a preStefanich ring. For $X \in \PreStRing$, define $X^{(0)} := X$, $X^{(\alpha + 1)} := T(X^{(\alpha)})$, and
		\[
			X^{(\lambda)} := \colim_{\alpha < \lambda} X^{(\alpha)}
		\]
		for limit ordinals $\lambda$.

		We first show that $X^{(\omega_1)}$ lies in $\StRing$. Since each $\End^{\bbP}_n$ preserves $\aleph_1$-filtered colimits, the functor $T$ preserves $\aleph_1$-filtered colimits. Hence
		\[
			T(X^{(\omega_1)})
			\simeq
	...
	\begin{construction}\label[construction]{con:Tensor_Product_In_Slice}
		...
		\[
			\StRing_{A/}
			\simeq
			\lim_{\CAT}(\CAlg(A_1) \xleftarrow{\End^A_1} \CAlg(A_2) \xleftarrow{\End^A_2} \dots).
		\]
		Let
		\[
		...
		\[
			...
			\qquad
			(R_m)_{m \geq 0} \mapsto (\End^A_{m+1}(R_{m+1}))_{m \geq 0}.
		\]
	...
	Before we start, we recall how the transition maps look for a morphism of Stefanich rings. Let
	\[
		\bbP = (\ast, \An,1\Pr,2\Pr,\dots)
	\]
	denote the initial Stefanich ring, with $\bbP_n = (n-1)\Pr$. The absolute transition adjunctions are the adjunctions
	\[
		\Mod^{\bbP}_n\colon \CAlg(n\Pr)
		\rightleftarrows
		\CAlg((n+1)\Pr) \noloc
		\End^{\bbP}_n.
	\]
	A Stefanich ring $A$ is, among other things, a compatible system of algebras over these absolute transitions. Equivalently, its structural equivalences can be adjointed to the functors
	\[
		L^A_n\colon \Mod_{A_n}(n\Pr) \to A_{n+1}.
	\]

	Now fix a map $f\colon A \to B$. The map $\bbP \to A \to B$ gives a commutative diagram
	\[
		\begin{tikzcd}
			\bbP_n \rar \dar["\ast"'] & A_n \rar \dar["\ast"'] & B_n \dar["\ast"'] \\
			\bbP_{n+1} \rar & A_{n+1} \rar & B_{n+1},
		\end{tikzcd}
	\]
	in the Grothendieck construction of the absolute transitions $\Mod^{\bbP}$. The maps marked by $\ast$ are cartesian: after passing to the right adjoint transitions they induce the structural equivalences $A_n \iso \End^{\bbP}_n(A_{n+1})$ and $B_n \iso \End^{\bbP}_n(B_{n+1})$.

	Writing $a\colon \bbP \to A$ for the structure map, we also obtain an adjunction
	\[
		a_n^*\colon \CAlg(n\Pr) \rightleftarrows \CAlg(A_{n+1}) \noloc \fgt.
	\]
	By \Cref{con:Lift_To_Algebra}, the map $f$ determines relative algebras
	\[
		(B/A)_n = f_{n,*}(\unit_{B_{n+1}}) \qin \CAlg(A_{n+1}),
	\]
	whose underlying $n$-category is $B_n$. The relative transition functor
	\[
		\CAlg(A_{n+1}) \xhookrightarrow{\Mod_{(-)}(A_{n+1})} \CAlg(\Mod_{A_{n+1}}((n+1)\Pr)) \xrightarrow{L^A_{n+1}} \CAlg(A_{n+2}),
	\]
	is $\Mod^A_{n+1}$. The Stefanich-ring structure of $B$ over $A$ gives a canonical comparison map
	\[
		L^{B/A}_n\colon \Mod^A_{n+1}((B/A)_n) \to (B/A)_{n+1}.
	\]
	The index is chosen so that for the absolute map $\bbP \to A$, the map $L^{A/\bbP}_n$ identifies with the structural functor $L^A_n$. Concretely, $L^{B/A}_n$ is the lower vertical comparison in the diagram
	\[
		\begin{tikzcd}
			\unit = (A/A)_n \rar \dar["!"', "\ast"] & (B/A)_n \dar["!"', "\ast"] \\
			A_{n+1} = \Mod_{(A/A)_n}(A_{n+1}) \rar["\mathrm{free}"] \dar["!"', "\ast"] & \Mod_{(B/A)_n}(A_{n+1}) \dar["L^{B/A}_n"] \\
			\unit = (A/A)_{n+1} \rar & (B/A)_{n+1}.
		\end{tikzcd}
	\]

	The properties below are all conditions on the maps obtained from this diagram in high degrees. For each $i$, we will consider the composite in $\CAlg(A_{i+2})$
	\[
		(A/A)_{i+1} \to \Mod^A_{i+1}((B/A)_i) \xrightarrow{L^{B/A}_i} (B/A)_{i+1}.
	\]
	We denote this composite by
	\[
	...
	\begin{observation}
		...
		\]
		in $A_{i+2}$. We may describe this right adjoint as
		\[
			(f/A)_{i,*}(M) = \Hom_{(B/A)_{i+1}}(\unit, M).
		\]
		In particular, we see that
		\[
			(f/A)_{i,*} (f/A)^*_i \unit = \Hom_{(B/A)_{i+1}}(\unit, \unit) = \End_{(B/A)_{i+1}}(\unit).
		\]
		(Note that this is just $(B/A)_i$, so this allows us to think of $(B/A)$ as a relative Stefanich ring in $\StRing_{A/}$.)

		Under the equivalence $\End_{A_{i+2}}(\unit) \iso A_{i+1}$ (given by ``evaluating at the unit''), this means that we can think of $(B/A)_i$ as the endomorphism $(f/A)_{i,*} (f/A)^*_i$ of the unit of $(B/A)_{i+1}$.
	\end{observation}
	...
	\begin{proposition}\label[proposition]{prop:Prim_Properties}
		...
		\begin{enumerate}
			...
			\[
				...
				\end{tikzcd}
			\]
			If $f$ is $n$-prim, then so is $f'$, and we have $g^*(B/A)_m = (B \otimes_A C / C)_m$ for $m \geq n$. (In particular, we do not need to ``spectrify''.)
			\item The $n$-prim maps are closed under composition.
		\end{enumerate}
	\end{proposition}
	\begin{proof}
		(4) This is relatively straightforward and we omit the argument.
		...
		(3) We first show that in high degrees $g^*(B/A)_\bullet$ is already a Stefanich ring, i.e.\ that the canonical maps $g^*(B/A)_i \to \End_{g^*(B/A)_{i+1}}(\unit)$ are isomorphisms. This implies that $g^*(B/A)_\bullet$ already agrees with $(C \otimes_A B / C)_i$ in these degrees. To see this, we consider the internal adjunction $(f/A)^*_i \dashv (f/A)_{i,*}$ in $A_{i+2}$. The object $(B/A)_i$ then corresponds to the composite
		\[
			\unit \xrightarrow{(f/A)^*_i} (B/A)_{i+1} \xrightarrow{(f/A)_{i,*}} \unit.
		\]
		Under the $2$-functor $g_{i+1}^*\colon A_{i+2} \to C_{i+2}$, we get an induced adjunction in $C_{i+2}$. Then $g^*(B/A)_i$ corresponds to the composite
		\[
			\unit \to g^*(B/A)_{i+1} \to \unit
		\]
		in $C_{i+2}$. It then follows that this composite computes the endomorphisms of $g^*(B/A)_{i+1}$, showing what we wanted. It then also follows that the map $(C/C)_{i+1} \to g^*(B/A)_{i+1} = (C \otimes_A B / C)_{i+1}$ admits a right adjoint, so that $f'$ is $n$-prim.
	...
	Note that one missing feature of the previous proposition is that $n$-prim maps are not necessarily closed under diagonals. This motivates the following definition.
	...
	\begin{proposition}\label[proposition]{prop:Suave_Properties}
		\begin{enumerate}
			\item The $n$-suave maps are closed under cobase change.
	...
	\begin{proposition}\label[proposition]{prop:Etale_Properties}
		The $n$-\'etale maps satisfy cobase change and cancellation.
	\end{proposition}
	...
	\begin{proposition}\label[proposition]{prop:Ambidexterity_Stefanich_Rings}
		...
		\end{enumerate}
		Moreover, in any of these cases, we have an identification $f_{m,\sharp} \cong f_{m,*}$ for each $m \geq n+2$.
	\end{proposition}
